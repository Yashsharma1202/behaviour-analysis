...
OUTPUT_FUTURES_ONLY = BASE_DIR / 'Futures_Master_Only_v2.xlsx'
...
# ------------------- 3. Robust Price Lookup Helper -------------------
# Pre-cache available dates per stock in OI_DATA
oi_parquet_dates = {} # stock -> dict(date_str -> price)
for stock_dir in OI_ROOT.iterdir():
    if stock_dir.is_dir():
        ...
        # Load futures first
        if fut_dir.is_dir():
            for p_file in fut_dir.glob('*.parquet'):
                d_str = p_file.stem
                try:
                    df_p = pd.read_parquet(p_file)
                    if 'Time' in df_p.columns:
                        r = df_p[df_p['Time'] == '15:29:59']
                        price = float(r.iloc[0]['Close']) if not r.empty else float(df_p.iloc[-1]['Close'])
                    else:
                        price = float(df_p.iloc[-1]['Close'])
                    dates_dict[d_str] = price
                except Exception:
                    pass
        # Spot fallback if futures missing
        if spot_dir.is_dir():
            for p_file in spot_dir.glob('*.parquet'):
                d_str = p_file.stem
                if d_str not in dates_dict:
                    try:
                        df_s = pd.read_parquet(p_file)
                        if 'Time' in df_s.columns:
                            r = df_s[df_s['Time'] == '15:29:59']
                            price = float(r.iloc[0]['Close']) if not r.empty else float(df_s.iloc[-1]['Close'])
                        else:
                            price = float(df_s.iloc[-1]['Close'])
                        dates_dict[d_str] = price
                    except Exception:
                        pass
        oi_parquet_dates[stock] = dates_dict

def resolve_futures_price(stock: str, target_date: str, fallback_synthetic: float):
    # 1. Exact match in OI_DATA
    if stock in oi_parquet_dates:
        s_dict = oi_parquet_dates[stock]
        if target_date in s_dict:
            return s_dict[target_date]
        # 2. Nearest date within +/- 5 days
        target_dt = pd.to_datetime(target_date)
        valid_dates = []
        for d_str, price in s_dict.items():
            diff = abs((pd.to_datetime(d_str) - target_dt).days)
            if diff <= 5:
                valid_dates.append((diff, d_str, price))
        if valid_dates:
            valid_dates.sort(key=lambda x: x[0])
            return valid_dates[0][2] # return price of nearest date
    
    # 3. Fallback to synthetic/spot price from trades sheet
    return float(fallback_synthetic)

# ------------------- 4. Process Trades Data -------------------
processed_trades = []
...
for idx, row in df_trades.iterrows():
    ...
    # Synthetic fallback price calculation
    atm_strike = row.get('ATM Strike (₹)', 0.0)
    ...
    if syn_entry <= 0: syn_entry = atm_strike or 1000.0
    if syn_exit <= 0: syn_exit = syn_entry

    entry_price = resolve_futures_price(sym, entry_dt, syn_entry)
    exit_price  = resolve_futures_price(sym, exit_dt, syn_exit)
...
print(f"Fully processed {len(df_all_trades)} trades with 0 missing prices!", flush=True)

# List of Quarters ordered
quarters_order = [
...
# ------------------- 5. Fund Compounding & Quarterly Summaries -------------------
quarter_summaries = []
...
for qtr in quarters_order:
    ...
    end_cap = beg_cap + q_pnl # Profit / Loss booked then added to available fund
    q_return = (q_pnl / beg_cap) if beg_cap > 0 else 0.0
    
    # Capital allocation per trade in this quarter
...
# ------------------- 6. Stock Leaderboard & Best Results -------------------
stock_leaderboard = []
for sym, group in df_all_trades.groupby('Symbol'):
    ...
    # Count profitable quarters for this stock
    prof_qtrs = 0
    for qtr in quarters_order:
...
# ------------------- 8. Load Daily Futures Master -------------------
print("Loading daily futures close dataset...", flush=True)
try:
    df_daily = pd.read_excel(OUTPUT_FUTURES_ONLY, sheet_name='Futures_Master', engine='openpyxl')
except Exception:
    df_daily = pd.DataFrame()

# ------------------- 9. Build Master Workbook with 17 Sheets -------------------
print("Building Master Workbook with 17 sheets & executive formatting...", flush=True)
wb = Workbook()
# remove default active sheet
wb.remove(wb.active)

# Helper function to apply table headers and styling
...
ws_exec.cell(row=3, column=5, value=final_cap).number_format = '₹#,##0.00'
...
# --- SHEET 17: Futures_Master (Daily close dataset) ---
...
# Save output files with fallback
...
saved_master = safe_save_workbook(wb, OUTPUT_FUTURES_MASTER)
saved_only = safe_save_workbook(wb, OUTPUT_FUTURES_ONLY)

print(f"\n🎉 ULTIMATE FUTURES MASTER WORKBOOK CREATED IN {time.time() - t0:.1f}s!", flush=True)
