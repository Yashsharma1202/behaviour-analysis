...
print("==========================================================================================")
print("BUILDING CLEAN & EASY QUARTERLY PERFORMANCE MASTER V3 (TRUE FINANCIAL QUARTER PERIODS)")
print("==========================================================================================")
print("  • Q1 = April to June Period (Announced in July/August)")
print("  • Q2 = July to September Period (Announced in October/November)")
print("  • Q3 = October to December Period (Announced in January/February)")
print("  • Q4 = January to March Period (Announced in April/May)")
...
SECTOR_MAP = {
    ...
    "VOLTAS": "Consumer Durables & Retail", "CROMPTON": "Consumer Durables & Retail",
    "DIXON": "Consumer Durables & Retail", "AMBER": "Consumer Durables & Retail",
    "POLYCAB": "Consumer Durables & Retail", "KEI": "Consumer Durables & Retail",
    "ZOMATO": "Consumer Durables & Retail", "SWIGGY": "Consumer Durables & Retail",
    "NAUKRI": "Consumer Durables & Retail", "DELHIVERY": "Consumer Durables & Retail",
    "PGEL": "Consumer Durables & Retail", "KAYNES": "Consumer Durables & Retail",
    
    "ULTRACEMCO": "Cement & Building Materials", "GRASIM": "Cement & Building Materials",
    "AMBUJACEM": "Cement & Building Materials", "ACC": "Cement & Building Materials",
    "SHREECEM": "Cement & Building Materials", "DALBHARAT": "Cement & Building Materials",
    "ASTRAL": "Cement & Building Materials", "SUPREMEIND": "Cement & Building Materials",
    
    "LT": "Capital Goods & Infrastructure", "BHARATFORG": "Capital Goods & Infrastructure",
    "BEL": "Capital Goods & Infrastructure", "HAL": "Capital Goods & Infrastructure",
    "BDL": "Capital Goods & Infrastructure", "COCHINSHIP": "Capital Goods & Infrastructure",
    "MAZDOCK": "Capital Goods & Infrastructure", "RVNL": "Capital Goods & Infrastructure",
    "IRCON": "Capital Goods & Infrastructure", "ABB": "Capital Goods & Infrastructure",
    "SIEMENS": "Capital Goods & Infrastructure", "CGPOWER": "Capital Goods & Infrastructure",
    "HAVELLS": "Capital Goods & Infrastructure", "PREMIERENE": "Capital Goods & Infrastructure",
    
    "ADANIENT": "Diversified & Infrastructure", "ADANIPORTS": "Diversified & Infrastructure",
    "GMRINFRA": "Diversified & Infrastructure", "IRCTC": "Diversified & Infrastructure",
    "INDIGO": "Diversified & Infrastructure", "CONCOR": "Diversified & Infrastructure",
    
    "BHARTIARTL": "Telecom & Media", "IDEA": "Telecom & Media", "TATACOMM": "Telecom & Media",
    "SUNTV": "Telecom & Media", "ZEEL": "Telecom & Media", "MCX": "Financial Infrastructure",
    "BSE": "Financial Infrastructure", "CDSL": "Financial Infrastructure",
    
    "DLF": "Realty & Construction", "GODREJPROP": "Realty & Construction", "OBEROIRLTY": "Realty & Construction",
    "PHOENIXLTD": "Realty & Construction", "PRESTIGE": "Realty & Construction", "NBCC": "Realty & Construction"
}
...
def assign_financial_quarter(row):
    """
    User Specified Financial Quarter Period Mapping:
    • Q1: April to June period (Announced in July/August/September -> broadcast month 7, 8, 9)
    • Q2: July to September period (Announced in October/November/December -> broadcast month 10, 11, 12)
    • Q3: October to December period (Announced in January/February/March -> broadcast month 1, 2, 3)
    • Q4: January to March period (Announced in April/May/June -> broadcast month 4, 5, 6)
    """
    rel = str(row.get('relatingTo', '')).lower()
    if 'first' in rel or 'q1' in rel:
        return 'Q1'
    elif 'second' in rel or 'q2' in rel:
        return 'Q2'
    elif 'third' in rel or 'q3' in rel:
        return 'Q3'
    elif 'fourth' in rel or 'q4' in rel:
        return 'Q4'
    ...
    # Fallback to broadCastDate month
    ...
    if fr_path.exists() and fr_path.stat().st_size > 10:
        try:
            ...
            if not df_fr.empty and 'broadCastDate' in df_fr.columns:
                ...
                df_fr = df_fr.dropna(subset=['dt']).sort_values('dt', ascending=True) # Ascending for earliest
                
                if not df_fr.empty:
                    earliest_row = df_fr.iloc[0]
                    first_dt = earliest_row['dt']
                ...
                # Now sort descending for analysis
                ...
                # Merge multiple notices for the same financial quarter period (Year + Quarter)
    ...
    for q_t in ["Q1", "Q2", "Q3", "Q4"]:
        ...
        if tot_q_count == 0:
            quarter_data_store[q_t].append({
                ...
                "first_q_name": first_q_name, "first_date": first_date_str,
                "trading_status": trading_status,
                "n_quarters_total": f"{n_quarters_total} Quarters",
                "n_quarters_qtype": "0 Quarters", "window": window_str, "strategy": "N/A",
...
out_path = ROOT / "Nifty211_Quarterly_Performance_Master.xlsx"
...
for q_idx, q_t in enumerate(["Q1", "Q2", "Q3", "Q4"]):
    ...
    df_avail = df_qtype[df_qtype['is_available'] == True].sort_values(['avg_ret', 'win_rate', 'net_pnl'], ascending=[False, False, False])
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {sheet_title.upper()} PERFORMANCE MASTER"
    ...
    top_ret = df_avail.iloc[0]['avg_ret'] if not df_avail.empty else 0.0
    ...
    cards = [
        ...
        ("TOP PERFORMING STOCK", f"{top_stock} (+{top_ret}%)", "D3:F4", "D3"),
        ("AVERAGE QUARTER RETURN %", f"{avg_ret_q}%", "G3:I4", "G3"),
        ("AVERAGE QUARTER WIN RATE %", f"{avg_wr_q}%", "J3:L4", "J3"),
        ("TOTAL QUARTER NET P&L", f"₹{tot_pnl_q:,.2f}", "M3:P4", "M3")
    ]
    ...
    ws.merge_cells(start_row=rank_start_row, start_column=1, end_row=rank_start_row, end_column=16)
    ...
    for r_idx, r in enumerate(df_sorted.to_dict('records'), start=rank_start_row+2):
        ...
        # New Column 4: Listing Start Quarter Name
        ...
        # New Column 5: First Reporting Date / Listing Date
        ws.cell(row=r_idx, column=5, value=r['first_date']).border = border_thin
        ws.cell(row=r_idx, column=5).alignment = Alignment(horizontal="center")

        c_st = ws.cell(row=r_idx, column=6, value=r['strategy'])
        ...
        c_act = ws.cell(row=r_idx, column=7, value=r['trading_status'])
        ...
        ws.cell(row=r_idx, column=8, value=r['n_quarters_total']).border = border_thin
        ws.cell(row=r_idx, column=8).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=9, value=r['n_quarters_qtype']).border = border_thin
        ws.cell(row=r_idx, column=9).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=10, value=r['window']).border = border_thin
        ws.cell(row=r_idx, column=10).alignment = Alignment(horizontal="center")
        
        wr_val_str = f"{r['win_rate']}%" if r['is_available'] else "N/A"
        c_wr = ws.cell(row=r_idx, column=11, value=wr_val_str)
        ...
        c_w = ws.cell(row=r_idx, column=12, value=r['win_ratio_str'])
        ...
        c_l = ws.cell(row=r_idx, column=13, value=r['losses'] if r['is_available'] else "N/A")
        ...
        c_ret = ws.cell(row=r_idx, column=14, value=ret_val_str)
        ...
        ws.cell(row=r_idx, column=15, value=f"₹{r['margin']:,.2f}").border = border_thin
        ws.cell(row=r_idx, column=15).alignment = Alignment(horizontal="right")
        
        pnl_val_str = f"₹{r['net_pnl']:,.2f}" if r['is_available'] else "N/A"
        c_p = ws.cell(row=r_idx, column=16, value=pnl_val_str)
...
try:
    wb.save(out_path)
    print(f"  • Successfully generated: Nifty211_Quarterly_Performance_Master.xlsx")
except PermissionError:
    alt_path = ROOT / "Nifty211_Quarterly_Performance_Master_v3.xlsx"
    wb.save(alt_path)
    print(f"  • WARNING: Nifty211_Quarterly_Performance_Master.xlsx is currently open in Excel!")
    print(f"  • Successfully generated updated file as: Nifty211_Quarterly_Performance_Master_v3.xlsx")
print("==========================================================================================")
print("CLEAN & EASY QUARTERLY PERFORMANCE MASTER V3 CREATED SUCCESSFULLY!")
print("==========================================================================================")
