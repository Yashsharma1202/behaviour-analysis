...
print("==========================================================================================")
print("BUILDING STRICT QUARTER-ISOLATED RANKED PERFORMANCE MASTER WORKBOOK (4 SEPARATE SHEETS)")
print("==========================================================================================")
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    # Process performance for each quarter type independently (strictly isolated)
    for q_t in ["Q1", "Q2", "Q3", "Q4"]:
        dts = q_dates_by_type[q_t]
        tot_q_count = len(dts)
        
        if tot_q_count == 0:
            # Stock has zero history for this quarter type
            quarter_data_store[q_t].append({
                "sym": sym, "trading_status": trading_status, "n_quarters_total": n_quarters_total,
                "n_quarters_qtype": 0, "window": window_str, "strategy": "N/A",
                "win_rate": 0.0, "wins": 0, "losses": 0, "avg_ret": 0.0,
        ...
        # Pick optimal strategy for this stock specifically in this quarter type
        if wr_long >= wr_short:
...
out_path = ROOT / "Nifty211_Four_Quarters_Strict_Ranked_Performance_Master.xlsx"
...
for q_idx, q_t in enumerate(["Q1", "Q2", "Q3", "Q4"]):
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {sheet_title.upper()} STRICT RANKED PERFORMANCE MASTER"
    ...
    # Main Ranking Table
    rank_start_row = 6
    ws.cell(row=rank_start_row, column=1, value=f"ALL {len(df_sorted)} STOCKS RANKED STRICTLY BY {q_t} AVERAGE RETURN %").font = hdr_font
    ...
    for r_idx, r in enumerate(df_sorted.to_dict('records'), start=rank_start_row+2):
        ...
        q_count_str = f"{r['n_quarters_qtype']} Quarters" if r['is_available'] else "N/A"
        ws.cell(row=r_idx, column=5, value=q_count_str).border = border_thin
        ...
        c_wr = ws.cell(row=r_idx, column=7, value=wr_val_str)
        ...
        c_w = ws.cell(row=r_idx, column=8, value=r['wins'] if r['is_available'] else "N/A")
        ...
        c_l = ws.cell(row=r_idx, column=9, value=r['losses'] if r['is_available'] else "N/A")
        ...
        c_ret = ws.cell(row=r_idx, column=10, value=ret_val_str)
        ...
        c_p = ws.cell(row=r_idx, column=12, value=pnl_val_str)
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_Four_Quarters_Strict_Ranked_Performance_Master.xlsx")
print("==========================================================================================")
print("STRICT QUARTER-ISOLATED MASTER WORKBOOK CREATED SUCCESSFULLY!")
print("==========================================================================================")
