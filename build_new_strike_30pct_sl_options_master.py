...
OUTPUT_SL30_MASTER  = BASE_DIR / 'Nifty50_12_Quarters_Options_New_Strike_30PCT_SL_Master.xlsx'
OUTPUT_SL30_SUMMARY = BASE_DIR / 'Nifty50_12_Quarters_Options_New_Strike_30PCT_SL_Summary.xlsx'

# Executive Color Palette (Deep Emerald Green / Slate Navy)
COLOR_TITLE_BG     = '065F46'  # Emerald Deep Title Banner
COLOR_SECTION_BG   = '047857'  # Emerald Section Header
COLOR_HEADER_BG    = '0F172A'  # Deep Slate Table Header
COLOR_SUBHEADER_BG = '10B981'  # Vibrant Emerald Subheader
...
print("Loading Source Data for New Strike Selection -30% Stop Loss Model...", flush=True)
...
for qtr in quarters_order:
    ...
    for idx, row in q_tr.iterrows():
        ...
        # APPLY STRICT -30% PREMIUM STOP LOSS RULE
        if raw_ret < -0.30:
            effective_ret = -0.30
            effective_p_exit = p_entry * 0.70
            sl_triggered = 'YES (-30% SL Hit)'
        else:
            effective_ret = raw_ret
            effective_p_exit = p_exit
            sl_triggered = 'NO (Normal Exit)'
            
        opt_pnl = round(effective_ret * p_entry * lot_size, 2)
        ...
        trade_rec = {
            ...
            'Effective Exit Premium (₹)': round(effective_p_exit, 2),
            'Lot Size (Qty)': lot_size,
            'Capital Outlay per Trade (₹)': capital_outlay,
            'Booked Option P&L (₹)': opt_pnl,
            'Option Return (%)': effective_ret,
            'Stop Loss Triggered': sl_triggered,
        ...
        sl30_trades.append(trade_rec)

df_all_sl30_trades = pd.DataFrame(sl30_trades)

# ------------------- 2. Calculate Day-by-Day Peak Capital & Dual Drawdowns for -30% SL -------------------
sl30_summaries = []

for qtr in quarters_order:
    q_tr = df_all_sl30_trades[df_all_sl30_trades['Quarter'] == qtr].copy()
    q_tr['Entry Date DT'] = pd.to_datetime(q_tr['Entry Date'])
    ...
    sl_hits = len(q_tr[q_tr['Stop Loss Triggered'].str.contains('YES')])
...
df_sl30_summary = pd.DataFrame(sl30_summaries)

# ------------------- 3. Build Master 13-Sheet Excel Workbook for -30% SL -------------------
print("Building Master 13-Sheet Workbook for -30% Stop Loss Model...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 OPTIONS STRATEGY — STRICT -30% STOP-LOSS MODEL EXECUTIVE SCORECARD"
...
global_tot_pnl = round(df_all_sl30_trades['Booked Option P&L (₹)'].sum(), 2)
global_win_rate = len(df_all_sl30_trades[df_all_sl30_trades['Booked Option P&L (₹)'] > 0]) / len(df_all_sl30_trades)
total_sl_hits = len(df_all_sl30_trades[df_all_sl30_trades['Stop Loss Triggered'].str.contains('YES')])

ws_exec.merge_cells('A3:B3')
c_h1 = ws_exec['A3']; c_h1.value = "TOTAL TRADES / SL HITS"; c_h1.font = font_card_lbl; c_h1.fill = fill_subheader; c_h1.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.merge_cells('A4:B4')
c_v1 = ws_exec['A4']; c_v1.value = f"600 Trades ({total_sl_hits} SL Hits)"; c_v1.font = Font(name='Segoe UI', size=13, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')
...
ws_exec.merge_cells('F3:J3')
c_h3 = ws_exec['F3']; c_h3.value = "TOTAL 12-QUARTER NET OPTION PROFIT (-30% SL)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.merge_cells('F4:J4')
...
for r in [3, 4]:
    for c in range(1, 11): ws_exec.cell(row=r, column=c).border = border_thin

ws_exec.merge_cells('A6:J6')
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (-30% STOP LOSS RULE APPLIED)"
...
headers_summary = ['Quarter', 'Peak Concurrent Capital (₹)', 'Peak Date (Open Trades)', 'Net Option Realised P&L (₹)', 'Return on Peak Capital (%)', 'Max Realised Drawdown (₹)', 'DD % on Initial Capital', 'True DD % on Peak Portfolio', 'SL Hits (-30%)', 'Win Rate (%)']
style_table_headers(ws_exec, 7, headers_summary, fill_header)
ws_exec.row_dimensions[7].height = 28

for r_idx, row_data in df_sl30_summary.iterrows():
    row_num = 8 + r_idx
    ...
    ws_exec.cell(row=row_num, column=9, value=int(row_data['SL Hits (-30%)'])).alignment = Alignment(horizontal='center')
    
    c_wr = ws_exec.cell(row=row_num, column=10, value=row_data['Win Rate (%)'])
    ...
    for c in range(1, 11):
        ...
        cell.border = border_thin
        if c not in [4, 6, 7, 8, 10] and row_fill.fill_type: cell.fill = row_fill
...
ws_exec.cell(row=r_tot, column=6, value=-abs(df_sl30_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=6).font = font_loss; ws_exec.cell(row=r_tot, column=6).fill = fill_loss
ws_exec.cell(row=r_tot, column=7, value=df_sl30_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=8, value=df_sl30_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=9, value=total_sl_hits).alignment = Alignment(horizontal='center')
c_tot_wr = ws_exec.cell(row=r_tot, column=10, value=global_win_rate)
c_tot_wr.number_format = '0.00%'; c_tot_wr.font = font_win; c_tot_wr.fill = fill_win; c_tot_wr.alignment = Alignment(horizontal='center')

for c in range(1, 11): ws_exec.cell(row=r_tot, column=c).border = border_thin
...
ws_exec.cell(row=r_avg, column=2, value=df_sl30_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=4, value=df_sl30_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=4).font = font_win; ws_exec.cell(row=r_avg, column=4).fill = fill_win
ws_exec.cell(row=r_avg, column=5, value=df_sl30_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=6, value=-abs(df_sl30_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_loss; ws_exec.cell(row=r_avg, column=6).fill = fill_loss
ws_exec.cell(row=r_avg, column=7, value=df_sl30_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=8, value=df_sl30_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=9, value=round(total_sl_hits/12, 1)).alignment = Alignment(horizontal='center')
c_avg_wr = ws_exec.cell(row=r_avg, column=10, value=global_win_rate)
c_avg_wr.number_format = '0.00%'; c_avg_wr.font = font_win; c_avg_wr.fill = fill_win; c_avg_wr.alignment = Alignment(horizontal='center')

for c in range(1, 11): ws_exec.cell(row=r_avg, column=c).border = border_double_bottom

autofit_columns(ws_exec, max_cols=10)

# Chart
chart_opt = LineChart()
chart_opt.title = "Master 12-Quarter Net Option Buying P&L (-30% Stop Loss Model) (₹)"
chart_opt.style = 13
...
ws_exec.add_chart(chart_opt, "L3")
...
trade_headers = [
    ...
    'Option Entry Premium (₹)', 'Effective Exit Premium (₹)', 'Lot Size (Qty)',
    'Capital Outlay per Trade (₹)', 'Booked Option P&L (₹)', 'Cumulative P&L (₹)', 'Drawdown (₹)',
    'Stop Loss Triggered', 'Assigned Slot', 'Re-entry Type', 'Quarterly Result Date'
]
...
for qtr in quarters_order:
    ...
    q_sum = df_sl30_summary[df_sl30_summary['Quarter'] == qtr].iloc[0]
    ...
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 OPTIONS BUYING MODEL (-30% STOP-LOSS) — {qtr.upper()}"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ws_q.row_dimensions[1].height = 40
    
    card_lbls = ['Peak Concurrent Capital (₹)', 'Peak Date (Open Trades)', 'Net Option P&L (₹)', 'Return on Peak Capital (%)', 'Win Rate (%)', 'Total Trades', 'SL Hits (-30%)', 'Max Drawdown (₹)', 'DD % Initial Capital', 'True DD % Peak Portfolio']
    ws_q.merge_cells('A2:J2')
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS (-30% STOP LOSS RULE APPLIED)"; c_s.font = font_section; c_s.fill = fill_section
    ...
    ws_q.cell(row=4, column=7, value=int(q_sum['SL Hits (-30%)'])).alignment = Alignment(horizontal='center')
    
    c_dd1 = ws_q.cell(row=4, column=8, value=-abs(q_sum['Max Drawdown (₹)']))
    c_dd1.number_format = '-₹#,##0.00'; c_dd1.font = font_loss; c_dd1.fill = fill_loss
    
    c_ddi = ws_q.cell(row=4, column=9, value=q_sum['DD % on Initial Capital'])
    c_ddi.number_format = '-0.00%'; c_ddi.font = font_loss; c_ddi.fill = fill_loss; c_ddi.alignment = Alignment(horizontal='center')
    
    c_dd2 = ws_q.cell(row=4, column=10, value=q_sum['True DD % on Peak Portfolio'])
    ...
    for c in range(1, 11): ws_q.cell(row=4, column=c).border = border_thin
    
    ws_q.merge_cells('A6:W6')
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE OPTIONS JOURNAL (-30% STOP LOSS RULE APPLIED)"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        ws_q.cell(row=row_num, column=14, value=t_row['Effective Exit Premium (₹)']).number_format = '₹#,##0.00'
        ...
        c_sl = ws_q.cell(row=row_num, column=20, value=t_row['Stop Loss Triggered'])
        c_sl.alignment = Alignment(horizontal='center')
        if 'YES' in t_row['Stop Loss Triggered']: c_sl.fill = fill_loss; c_sl.font = font_loss
        
        ws_q.cell(row=row_num, column=21, value=t_row['Assigned Slot']).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=22, value=t_row.get('Re-entry Type', 'First Entry')).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=23, value=t_row.get('Quarterly Result Date', '')).alignment = Alignment(horizontal='center')
        
        for c in range(1, 24):
            cell = ws_q.cell(row=row_num, column=c)
            cell.border = border_thin
            if c not in [5, 17, 19, 20] and row_fill.fill_type: cell.fill = row_fill

    autofit_columns(ws_q, max_cols=23)
    ws_q.freeze_panes = 'A8'

    chart_q = LineChart()
    chart_q.title = f"{qtr} — Option Buying (-30% SL) Cumulative P&L Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
wb.save(OUTPUT_SL30_MASTER)

# ------------------- 4. Build Standalone -30% SL Summary Workbook -------------------
print("Writing Standalone -30% Stop Loss Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Options_30PCT_SL_Sum')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:J1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 OPTIONS STRATEGY — STRICT -30% STOP-LOSS MODEL SUMMARY DASHBOARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
ws_s.row_dimensions[1].height = 40

ws_s.merge_cells('A3:B3')
c_h1 = ws_s['A3']; c_h1.value = "TOTAL TRADES / SL HITS"; c_h1.font = font_card_lbl; c_h1.fill = fill_subheader; c_h1.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('A4:B4')
c_v1 = ws_s['A4']; c_v1.value = f"600 Trades ({total_sl_hits} SL Hits)"; c_v1.font = Font(name='Segoe UI', size=13, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')
...
ws_s.merge_cells('F3:J3')
c_h3 = ws_s['F3']; c_h3.value = "TOTAL 12-QUARTER NET OPTION PROFIT (-30% SL)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('F4:J4')
...
for r in [3, 4]:
    for c in range(1, 11): ws_s.cell(row=r, column=c).border = border_thin

ws_s.merge_cells('A6:J6')
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (-30% STOP LOSS RULE APPLIED)"
...
for r_idx, row_data in df_sl30_summary.iterrows():
    ...
    ws_s.cell(row=row_num, column=9, value=int(row_data['SL Hits (-30%)'])).alignment = Alignment(horizontal='center')
    
    c_wr = ws_s.cell(row=row_num, column=10, value=row_data['Win Rate (%)'])
    c_wr.number_format = '0.00%'; c_wr.alignment = Alignment(horizontal='center')
    if row_data['Win Rate (%)'] >= 0.70: c_wr.fill = fill_win; c_wr.font = font_win
    elif row_data['Win Rate (%)'] >= 0.50: c_wr.fill = fill_gold; c_wr.font = font_gold
    else: c_wr.fill = fill_loss; c_wr.font = font_loss
...
ws_s.cell(row=r_tot, column=6, value=-abs(df_sl30_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=6).font = font_loss; ws_s.cell(row=r_tot, column=6).fill = fill_loss
ws_s.cell(row=r_tot, column=7, value=df_sl30_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=8, value=df_sl30_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=9, value=total_sl_hits).alignment = Alignment(horizontal='center')
c_twr = ws_s.cell(row=r_tot, column=10, value=global_win_rate)
c_twr.number_format = '0.00%'; c_twr.font = font_win; c_twr.fill = fill_win; c_twr.alignment = Alignment(horizontal='center')
for c in range(1, 11): ws_s.cell(row=r_tot, column=c).border = border_thin

# Row 21 Average
r_avg = 21
ws_s.row_dimensions[r_avg].height = 24
ws_s.cell(row=r_avg, column=1, value="QUARTERLY AVERAGE").font = font_data_bold
ws_s.cell(row=r_avg, column=2, value=df_sl30_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=4, value=df_sl30_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=4).font = font_win; ws_s.cell(row=r_avg, column=4).fill = fill_win
ws_s.cell(row=r_avg, column=5, value=df_sl30_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=6, value=-abs(df_sl30_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_loss; ws_s.cell(row=r_avg, column=6).fill = fill_loss
ws_s.cell(row=r_avg, column=7, value=df_sl30_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=8, value=df_sl30_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=9, value=round(total_sl_hits/12, 1)).alignment = Alignment(horizontal='center')
c_awr = ws_s.cell(row=r_avg, column=10, value=global_win_rate)
c_awr.number_format = '0.00%'; c_awr.font = font_win; c_awr.fill = fill_win; c_awr.alignment = Alignment(horizontal='center')
for c in range(1, 11): ws_s.cell(row=r_avg, column=c).border = border_double_bottom
...
# Chart
chart = LineChart()
chart.title = "Consolidated 12-Quarter Net Option Buying P&L (-30% SL Model) Trend (₹)"
chart.style = 13
...
ws_s.add_chart(chart, "L3")

wb_s.save(OUTPUT_SL30_SUMMARY)
print(f"\n🎉 -30% STOP LOSS OPTIONS MASTER & SUMMARY CREATED IN {time.time() - t0:.1f}s!", flush=True)
