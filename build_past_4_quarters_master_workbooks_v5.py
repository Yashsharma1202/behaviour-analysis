...
print("==========================================================================================")
print("BUILDING PAST 4 QUARTERS MASTER WORKBOOKS WITH PREDICTED VS ACTUAL RETURNS & COMPOUNDING FUND FLOW")
print("==========================================================================================")
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    # Compute Historical Mean Return % for Prediction Model
    hist_returns_long = []
    hist_returns_short = []
    for dt in q_dates_history:
        ...
        if pe > 0:
            hist_returns_long.append((px - pe) / pe * 100)
            hist_returns_short.append((pe - px) / pe * 100)
            
    pred_ret_long = round(np.mean(hist_returns_long), 2) if hist_returns_long else 2.50
    pred_ret_short = round(np.mean(hist_returns_short), 2) if hist_returns_short else 2.50
    ...
    for q_idx, (q_name, default_d_str) in enumerate(quarters_list):
        ...
        base_quarter_trades[q_name].append({
            ...
            "window": window_str,
            "pred_ret_long": pred_ret_long,
            "pred_ret_short": pred_ret_short,
...
def generate_past_4_quarters_workbook_v5(mode_name, filename):
    ...
    INITIAL_PORTFOLIO_FUND = 10000000.0  # ₹1 Crore Starting Portfolio Fund
    ...
    for q_name, _ in quarters_list:
        ...
        for r in raw_t:
            ...
            if mode_name == "LONG":
                buy_p = p_e; sell_p = p_x; pred_ret = r['pred_ret_long']; act_ret = round((p_x - p_e)/p_e * 100, 2)
            elif mode_name == "SHORT":
                buy_p = p_x; sell_p = p_e; pred_ret = r['pred_ret_short']; act_ret = round((p_e - p_x)/p_e * 100, 2)
            else:
            ...
            calc_t.append({
                "sym": r['sym'], "trading_status": r['trading_status'], "pred_ret": pred_ret,
                "act_ret": act_ret, "var_ret": var_ret, "n_quarters": r['n_quarters'],
                "window": r['window'], "start_fund": start_fund, "net_pnl": net_pnl,
                "end_fund": portfolio_fund, "comb_to": comb_to, "cost": cost
            })
    ...
    ws_sum["A1"] = f"NIFTY 211 STOCKS - PAST 4 QUARTERS FUTURES {mode_name} COMPOUNDING FUND MODEL"
    ...
    # KPI Cards
    cards = [
        ("STARTING PORTFOLIO FUND", f"₹{INITIAL_PORTFOLIO_FUND:,.2f}", "A3:B4", "A3"),
        ("ENDING REINVESTED FUND", f"₹{portfolio_fund:,.2f}", "C3:D4", "C3"),
        ("TOTAL FUND RETURN %", f"{total_fund_ret_pct}%", "E3:F4", "E3"),
        ("4-QTR NET REALISED P&L", f"₹{cum_all_pnl:,.2f}", "G3:H4", "G3")
    ]
    ...
    # 2. SIDE-BY-SIDE 4-QUARTER PERFORMANCE MATRIX WITH PREDICTED VS ACTUAL
    ws_sum.cell(row=7, column=1, value="PAST 4 QUARTERS SIDE-BY-SIDE MATRIX (PREDICTED VS ACTUAL & FUND FLOW)").font = hdr_font
    ...
    # Matrix Rows Data (Row 10 to 17)
    matrix_rows = [
        ("Average Predicted Return (%)",
         [f"{q_stats['Q2 2026-27']['avg_pred_ret']}%", f"{q_stats['Q2 2026-27']['avg_pred_ret']}%",
          f"{q_stats['Q1 2026-27']['avg_pred_ret']}%", f"{q_stats['Q1 2026-27']['avg_pred_ret']}%",
          f"{q_stats['Q4 2025-26']['avg_pred_ret']}%", f"{q_stats['Q4 2025-26']['avg_pred_ret']}%",
          f"{q_stats['Q3 2025-26']['avg_pred_ret']}%", f"{q_stats['Q3 2025-26']['avg_pred_ret']}%",
          "2.50%", "2.50%"]),
          
        ("Average Actual Realised Return (%)",
         [f"{q_stats['Q2 2026-27']['avg_act_ret']}%", f"{q_stats['Q2 2026-27']['avg_act_ret']}%",
          f"{q_stats['Q1 2026-27']['avg_act_ret']}%", f"{q_stats['Q1 2026-27']['avg_act_ret']}%",
          f"{q_stats['Q4 2025-26']['avg_act_ret']}%", f"{q_stats['Q4 2025-26']['avg_act_ret']}%",
          f"{q_stats['Q3 2025-26']['avg_act_ret']}%", f"{q_stats['Q3 2025-26']['avg_act_ret']}%",
    ...
    # 3. INDIVIDUAL QUARTER SUMMARY TABLES
    ws_sum.cell(row=20, column=1, value="INDIVIDUAL QUARTER EXECUTIVE SUMMARY TABLES").font = hdr_font
    ...
    # 4. ALL 211 STOCKS PREDICTED VS ACTUAL & FUND FLOW TABLE
    rank_start_row = curr_row + 1
    df_rec = q_stats["Q2 2026-27"]["df_calc"]
    df_all_ranked = df_rec.sort_values(['net_pnl'], ascending=[False])

    ws_sum.cell(row=rank_start_row, column=1, value="ALL 211 STOCKS PREDICTED VS ACTUAL RETURNS & COMPOUNDING FUND FLOW").font = hdr_font
    ws_sum.cell(row=rank_start_row, column=1).fill = hdr_fill
    ws_sum.merge_cells(start_row=rank_start_row, start_column=1, end_row=rank_start_row, end_column=9)

    headers_rank = ["Rank", "Stock Symbol", "Action Status", "History", "Window", "Predicted Ret %", "Actual Ret %", "Variance %", "Net P&L (₹)"]
    ...
    for r_idx, r in enumerate(df_all_ranked.to_dict('records'), start=rank_start_row+2):
        ...
        ws_sum.cell(row=r_idx, column=6, value=f"{r['pred_ret']}%").border = border_thin
        ws_sum.cell(row=r_idx, column=6).alignment = Alignment(horizontal="right")
        
        ws_sum.cell(row=r_idx, column=7, value=f"{r['act_ret']}%").border = border_thin
        ws_sum.cell(row=r_idx, column=7).alignment = Alignment(horizontal="right")
        
        ws_sum.cell(row=r_idx, column=8, value=f"{r['var_ret']}%").border = border_thin
        ws_sum.cell(row=r_idx, column=8).alignment = Alignment(horizontal="right")
    ...
    # SHEETS 2 to 5: PAST 4 QUARTER TRADE SHEETS WITH COMPOUNDING FUND FLOW
    for q_name, _ in quarters_list:
        ws_q = wb.create_sheet(title=q_name)
        ws_q.views.sheetView[0].showGridLines = True
        
        ws_q.merge_cells("A1:AC1")
        ws_q["A1"] = f"NIFTY 211 STOCKS - {q_name} FUTURES {mode_name} MASTER (COMPOUNDING FUND MODEL)"
        ...
        for r_idx, r in enumerate(q_trades, start=4):
            ...
            if mode_name == "LONG":
                buy_p = p_e; sell_p = p_x; strat = "FUTURE LONG"; pred_r = r['pred_ret_long']
                act_r = round((p_x - p_e)/p_e * 100, 2)
            elif mode_name == "SHORT":
                buy_p = p_x; sell_p = p_e; strat = "FUTURE SHORT"; pred_r = r['pred_ret_short']
            ...
            ws_q.cell(row=r_idx, column=24, value=f"=K{r_idx}+W{r_idx}") # Ending Fund = Starting Fund + Net PnL
...
generate_past_4_quarters_workbook_v5("LONG", "Nifty211_Past_4_Quarters_Futures_LONG_Master.xlsx")
generate_past_4_quarters_workbook_v5("SHORT", "Nifty211_Past_4_Quarters_Futures_SHORT_Master.xlsx")
generate_past_4_quarters_workbook_v5("COMBINED BEST", "Nifty211_Past_4_Quarters_Combined_Best_Futures_Master_v4.xlsx")

print("==========================================================================================")
print("ALL 3 MASTER WORKBOOKS CREATED SUCCESSFULLY WITH PREDICTED VS ACTUAL & COMPOUNDING FUND FLOW!")
print("==========================================================================================")
