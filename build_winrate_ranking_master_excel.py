...
print("==========================================================================================")
print("BUILDING ALL 211 STOCKS HISTORICAL WIN RATE RANKING MASTER WORKBOOK")
print("==========================================================================================")
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    # Compute Empirical Historical Win Rates for both Long and Short
...
# Sort strictly by Win Rate % descending, then Net P&L descending
df_all = df_all.sort_values(['win_rate', 'pnl_12q'], ascending=[False, False]).reset_index(drop=True)
...
out_path = ROOT / "Nifty211_All_Stocks_Historical_WinRate_Ranking_Master.xlsx"
...
def write_ranking_sheet(ws, sheet_title, df_sheet):
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {sheet_title.upper()} RANKING BY HISTORICAL WIN RATE %"
    ...
    top_wr = df_sheet.iloc[0]['win_rate'] if not df_sheet.empty else 0.0
    avg_wr = round(df_sheet['win_rate'].mean(), 2) if not df_sheet.empty else 0.0
    tot_pnl = round(df_sheet['pnl_12q'].sum(), 2) if not df_sheet.empty else 0.0
    
    cards = [
        ("TOTAL STOCKS RANKED", f"{len(df_sheet)} Stocks", "A3:B4", "A3"),
        ("TOP WIN RATE STOCK", f"{top_stock} ({top_wr}%)", "C3:D4", "C3"),
        ("AVERAGE WIN RATE %", f"{avg_wr}%", "E3:F4", "E3"),
    ...
    ws.cell(row=6, column=1, value="WIN RATE BRACKET DISTRIBUTION SUMMARY").font = hdr_font
    ...
    b_80 = df_sheet[df_sheet['win_rate'] >= 80]
    b_70 = df_sheet[(df_sheet['win_rate'] >= 70) & (df_sheet['win_rate'] < 80)]
    b_60 = df_sheet[(df_sheet['win_rate'] >= 60) & (df_sheet['win_rate'] < 70)]
    b_50 = df_sheet[(df_sheet['win_rate'] >= 50) & (df_sheet['win_rate'] < 60)]
    b_below = df_sheet[df_sheet['win_rate'] < 50]
    ...
    # Main Ranking Table
    rank_start_row = 15
    ws.cell(row=rank_start_row, column=1, value=f"ALL {len(df_sheet)} STOCKS RANKED BY EMPIRICAL WIN RATE %").font = hdr_font
    ...
    headers = [
        ...
        "Position Window", "Empirical Win Rate %", "Winning Quarters", "Losing Quarters",
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_All_Stocks_Historical_WinRate_Ranking_Master.xlsx")
print("==========================================================================================")
print("WIN RATE RANKING MASTER WORKBOOK CREATED SUCCESSFULLY!")
print("==========================================================================================")
