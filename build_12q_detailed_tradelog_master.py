...
print("Target Universe         : 211 Nifty F&O / Broad Market Stocks (Portfolio Master Mapped)")
print("Target Financial Quarters: 12 Quarters (Q3 FY2023-24 to Q2 FY2026-27)")
print("-" * 90)
...
print("\nVERIFICATION OF EXECUTED vs N/A TRADES ACROSS ALL 12 QUARTERS:")
print(f"{'Quarter':<15} | {'Total Stocks':<12} | {'Executed Trades':<16} | {'N/A Trades':<12} | {'QUALIFIED Exec':<15} | {'Net P&L (₹)':<15}")
print("-" * 95)
...
for q in TARGET_QUARTERS:
    ...
    print(f"{q_label:<15} | {n_total:<12} | {n_exec:<16} | {n_na:<12} | {n_qual_exec:<15} | {net_pnl:<15,.2f}")
    quarter_summary_stats.append({
        "info": q, "n_total": n_total, "n_exec": n_exec, "n_na": n_na, "n_qual_exec": n_qual_exec, "trades": trades
    })

print("="*95)

# Build Master Excel Workbook
print("\nGenerating Master Workbook: Nifty211_12Q_Detailed_Trade_Log_Master.xlsx ...")
...
ws_sum["A1"] = "12-QUARTER DETAILED TRADE LOG & STRATEGY PERFORMANCE MASTER"
ws_sum["A1"].font = f_title
ws_sum["A1"].fill = fill_navy
ws_sum["A1"].alignment = Alignment(horizontal="center", vertical="center")

ws_sum.merge_cells("A2:K2")
ws_sum["A2"] = "Comprehensive Audit of Quarterly Result Events Mapped from Portfolio Master Excel (Strict N/A for Unbroadcast Quarters)"
...
tot_na_trades = sum(qs['n_na'] for qs in quarter_summary_stats)
tot_qual_exec_trades = sum(qs['n_qual_exec'] for qs in quarter_summary_stats)

all_qual_exec_trades = [t for qs in quarter_summary_stats for t in qs['trades'] if t['is_valid'] and t['status'] == 'QUALIFIED']
...
all_exec_trades_list = [t for qs in quarter_summary_stats for t in qs['trades'] if t['is_valid']]
...
cards_data = [
    ("TOTAL TRADES EXECUTED (12Q)", f"{tot_exec_trades:,} / {tot_all_stocks_slots:,}\n({tot_na_trades:,} N/A Unbroadcast)", "A4:C5"),
...
ws_sum["A7"] = "📊 12-QUARTER PERFORMANCE BREAKDOWN BY QUARTER (PORTFOLIO MASTER MAPPED)"
...
headers_sum = [
    ...
    "Executed Trades", "N/A Unbroadcast", "Qual. Win Rate %", 
    "Qual. Net P&L (₹)", "Overall Net P&L (₹)", "Total Margin Required (₹)", "Data Coverage Status"
]
...
for qs in quarter_summary_stats:
    ...
    if qs['n_na'] == 0:
        status_str = "100% Broadcast Available"
    else:
...
ws_sum[f"A{row_idx}"] = "💰 DYNAMIC QUARTER-WISE FUND ALLOCATION & PERFORMANCE RETURN %"
...
for qs in quarter_summary_stats:
    ...
    if len(q_all_exec) == 0:
        action_str = "No Broadcast Data (N/A)"
    else:
...
for qs in quarter_summary_stats:
    ...
    ws["A2"] = f"📅 Quarter Duration: {q_info['period']}  |  Results Broadcast Period: {q_info['ann']}  |  Execution Engine: Mapped from Portfolio Master Excel"
    ...
    ws["A3"] = "ℹ️ QUALIFIED STOCKS: ≥12 Quarters History  |  AVOID BUT MONITOR IT: <12 Quarters History  |  N/A: Unbroadcast Quarter (No Result Date in CSV)"
    ...
    kpi_items = [
        ...
        ("N/A UNBROADCAST", f"{n_na} ({n_na/n_tot*100:.1f}%)", "E5:F6"),
        ...
        ("RETURN ON QUARTER FUND %", f"{ret_on_q_fund:.2f}%" if tot_margin > 0 else "N/A", "O5:Q6")
    ]
    ...
    ws["A8"] = f"DETAILED TRADE LOG — {n_exec} EXECUTED TRADES ({n_qual_exec} QUALIFIED) · {n_na} N/A UNBROADCAST STOCKS"
...
saved_path = None
for candidate in [
    ROOT / 'Nifty211_12Q_Detailed_Trade_Log_Master.xlsx',
    ROOT / 'Nifty211_12Q_Detailed_Trade_Log_Master_PortfolioMapped.xlsx',
    ROOT / f'Nifty211_12Q_Detailed_Trade_Log_Master_{pd.Timestamp.now().strftime("%H%M%S")}.xlsx'
]:
    try:
        wb.save(candidate)
...
print(f"\n✅ SUCCESSFULLY BUILT AND SAVED MASTER EXCEL (MAPPED FROM PORTFOLIO MASTER):")
