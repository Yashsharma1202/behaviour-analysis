"""
build_q3_position_windows.py
===============================================================================
Calculates the optimal Q3 (October / November Results Season) position windows,
strategies (LONG vs SHORT), win rates, average returns, lot sizes, and 20% margin
requirements for ALL Nifty 50 stocks.

Outputs:
  - Nifty50_Q3_Position_Execution_Windows_Master.xlsx
  - Nifty50_Q3_Position_Execution_Windows_Master.json
  - Updates simple_execution_portal.html with all 50 stocks
===============================================================================
"""
import pathlib
import os
...
print("==========================================================================================")
print("BUILDING Q3 OPTIMAL POSITION EXECUTION WINDOWS FOR ALL NIFTY 50 STOCKS")
print("==========================================================================================")
...
for idx, sym in enumerate(NIFTY50, 1):
    ...
    # October / November results season
    oct_nov_dts = [d for d in clean_dts if d.month in (10, 11)]
    tot_count = len(oct_nov_dts)
    
    if tot_count == 0:
        # Fallback if no October specific results
        oct_nov_dts = clean_dts[:12]
        tot_count = len(oct_nov_dts)
        
    best_wr = -1.0; best_win_str = ''; best_strat = ''; best_avg_ret = -999.0
    ...
    # Calculate sample trade history for past 4 quarters
    past_trades = []
    ...
    q3_results.append({
        ...
        'pre': best_pre,
        'post': best_post,
        'strat': best_strat if stage == 2 else 'LOCKED (WAITING FOR NSE FILING)',
...
print(f"Successfully processed {len(q3_results)} Nifty 50 stocks for Q3.")

# Export to Excel
excel_file = ROOT / "Nifty50_Q3_Position_Execution_Windows_Master.xlsx"
df_excel = pd.DataFrame([
    {
        'Symbol': r['sym'],
        'Company Name': r['name'],
        'NSE Filing Stage': 'Stage 2 (Confirmed)' if r['stage'] == 2 else 'Stage 1 (Pending)',
        'Result Date': r['dateStr'],
        'Optimal Window': r['winStr'],
        'Recommended Strategy': r['opt_strat'],
        'Q3 Win Rate %': r['wr'],
        'Expected Net Return %': r['avgRet'],
        'Past Wins / Total': f"{r['wins']} / {r['tot_quarters']}",
        'LTP (₹)': r['ltp'],
        'Lot Size': r['lot'],
...
print(f"Saved Excel Master to: {excel_file.name}")

# Export JSON
json_file = ROOT / "Nifty50_Q3_Position_Execution_Windows_Master.json"
with open(json_file, 'w', encoding='utf-8') as f:
    json.dump(q3_results, f, indent=2)
print(f"Saved JSON Master to: {json_file.name}")
