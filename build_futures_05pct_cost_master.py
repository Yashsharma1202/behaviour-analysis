...
OUTPUT_COST_MASTER  = BASE_DIR / 'Nifty50_12_Quarters_Point_In_Time_Futures_0.5PCT_Cost_Master.xlsx'
OUTPUT_COST_SUMMARY = BASE_DIR / 'Nifty50_12_Quarters_Point_In_Time_Futures_0.5PCT_Cost_Summary.xlsx'

# Executive Color Palette (Navy / Slate Header)
...
print("Loading Source Data for Futures 0.5% Transaction Cost Model...", flush=True)
...
for qtr in quarters_order:
    ...
    for idx, row in q_tr.iterrows():
        ...
        contract_val = s_entry * lot_size
        trans_cost   = round(contract_val * 0.005, 2)  # 0.5% transaction cost
        net_pnl      = round(raw_pnl - trans_cost, 2)
        margin_outlay = round(contract_val * 0.20, 2)   # 20% margin outlay
        ...
        trade_rec = {
            ...
            'Contract Value (₹)': round(contract_val, 2),
        ...
        cost_trades.append(trade_rec)

df_all_cost_trades = pd.DataFrame(cost_trades)

# ------------------- 2. Calculate Day-by-Day Peak Capital & Dual Drawdowns (After 0.5% Cost) -------------------
cost_summaries = []

for qtr in quarters_order:
    q_tr = df_all_cost_trades[df_all_cost_trades['Quarter'] == qtr].copy()
    q_tr['Entry Date DT'] = pd.to_datetime(q_tr['Entry Date'])
    ...
    tot_fut_pnl = round(q_tr['Net Futures P&L (After 0.5% Cost) (₹)'].sum(), 2)
    ...
    final_val_quarter = round(peak_margin_capital + tot_fut_pnl, 2)
    fut_booked_return = (tot_fut_pnl / peak_margin_capital) if peak_margin_capital > 0 else 0.0
    ...
    cost_summaries.append({
        ...
        'Net Futures Realised P&L (₹)': tot_fut_pnl,
...
df_cost_summary = pd.DataFrame(cost_summaries)

# ------------------- 3. Build Master 13-Sheet Excel Workbook for 0.5% Cost -------------------
print("Building Master 13-Sheet Workbook for Futures 0.5% Cost Model...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY — POINT-IN-TIME LOTS (WITH 0.5% TRANSACTION COST) EXECUTIVE SCORECARD"
...
global_tot_raw_pnl  = round(df_all_cost_trades['Raw Futures P&L (₹)'].sum(), 2)
global_tot_trans    = round(df_all_cost_trades['Transaction Cost (0.5%) (₹)'].sum(), 2)
global_tot_net_pnl  = round(df_all_cost_trades['Net Futures P&L (After 0.5% Cost) (₹)'].sum(), 2)
global_win_rate     = len(df_all_cost_trades[df_all_cost_trades['Net Futures P&L (After 0.5% Cost) (₹)'] > 0]) / len(df_all_cost_trades)
...
c_v1 = ws_exec['A4']; c_v1.value = len(df_all_cost_trades); c_v1.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')

ws_exec.merge_cells('C3:E3')
c_h2 = ws_exec['C3']; c_h2.value = "GLOBAL WIN RATE (AFTER 0.5% COST)"; c_h2.font = font_card_lbl; c_h2.fill = fill_subheader; c_h2.alignment = Alignment(horizontal='center', vertical='center')
...
ws_exec.merge_cells('F3:H3')
c_h3 = ws_exec['F3']; c_h3.value = "TOTAL TRANSACTION COST (0.5%)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.merge_cells('F4:H4')
c_v3 = ws_exec['F4']; c_v3.value = global_tot_trans; c_v3.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_LOSS_TEXT); c_v3.fill = fill_loss; c_v3.number_format = '₹#,##0.00'; c_v3.alignment = Alignment(horizontal='center', vertical='center')

ws_exec.merge_cells('I3:K3')
c_h4 = ws_exec['I3']; c_h4.value = "NET 12-QUARTER FUTURES PROFIT"; c_h4.font = font_card_lbl; c_h4.fill = fill_subheader; c_h4.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.merge_cells('I4:K4')
c_v4 = ws_exec['I4']; c_v4.value = global_tot_net_pnl; c_v4.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v4.fill = fill_win; c_v4.number_format = '₹#,##0.00'; c_v4.alignment = Alignment(horizontal='center', vertical='center')

ws_exec.row_dimensions[3].height = 22
ws_exec.row_dimensions[4].height = 30

for r in [3, 4]:
    for c in range(1, 12): ws_exec.cell(row=r, column=c).border = border_thin

ws_exec.merge_cells('A6:K6')
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (WITH 0.5% TRANSACTION COST APPLIED)"
...
headers_summary = ['Quarter', 'Peak Margin Capital (₹)', 'Peak Date (Open Trades)', 'Raw Futures P&L (₹)', 'Total Trans Cost (0.5%) (₹)', 'Net Futures P&L (₹)', 'Return on Margin (%)', 'Max Realised Drawdown (₹)', 'DD % on Initial Capital', 'True DD % on Peak Portfolio', 'Win Rate (%)']
style_table_headers(ws_exec, 7, headers_summary, fill_header)
ws_exec.row_dimensions[7].height = 28

for r_idx, row_data in df_cost_summary.iterrows():
    row_num = 8 + r_idx
    ...
    ws_exec.cell(row=row_num, column=2, value=row_data['Peak Concurrent Margin (₹)']).number_format = '₹#,##0.00'
    ...
    ws_exec.cell(row=row_num, column=4, value=row_data['Raw Futures P&L (₹)']).number_format = '₹#,##0.00'
    
    c_tc = ws_exec.cell(row=row_num, column=5, value=row_data['Total Transaction Cost (0.5%) (₹)'])
    c_tc.number_format = '₹#,##0.00'; c_tc.font = font_loss; c_tc.fill = fill_loss
    
    c_pnl = ws_exec.cell(row=row_num, column=6, value=row_data['Net Futures Realised P&L (₹)'])
    ...
    c_ret = ws_exec.cell(row=row_num, column=7, value=row_data['Booked Return on Margin (%)'])
    c_ret.number_format = '+0.00%;-0.00%'
    
    c_dd = ws_exec.cell(row=row_num, column=8, value=-abs(row_data['Max Drawdown (₹)']))
    c_dd.number_format = '-₹#,##0.00'; c_dd.font = font_loss; c_dd.fill = fill_loss
    
    c_ddi = ws_exec.cell(row=row_num, column=9, value=row_data['DD % on Initial Capital'])
    c_ddi.number_format = '-0.00%'; c_ddi.font = font_loss; c_ddi.fill = fill_loss; c_ddi.alignment = Alignment(horizontal='center')
    
    c_ddp = ws_exec.cell(row=row_num, column=10, value=row_data['True DD % on Peak Portfolio'])
    c_ddp.number_format = '-0.00%'; c_ddp.font = font_loss; c_ddp.fill = fill_loss; c_ddp.alignment = Alignment(horizontal='center')
    
    c_wr = ws_exec.cell(row=row_num, column=11, value=row_data['Win Rate (%)'])
    ...
    for c in range(1, 12):
        ...
        cell.border = border_thin
        if c not in [5, 6, 8, 9, 10, 11] and row_fill.fill_type: cell.fill = row_fill
...
ws_exec.cell(row=r_tot, column=4, value=global_tot_raw_pnl).number_format = '₹#,##0.00'
ws_exec.cell(row=r_tot, column=5, value=global_tot_trans).number_format = '₹#,##0.00'
ws_exec.cell(row=r_tot, column=5).font = font_loss; ws_exec.cell(row=r_tot, column=5).fill = fill_loss
ws_exec.cell(row=r_tot, column=6, value=global_tot_net_pnl).number_format = '₹#,##0.00'
ws_exec.cell(row=r_tot, column=6).font = font_win; ws_exec.cell(row=r_tot, column=6).fill = fill_win
ws_exec.cell(row=r_tot, column=7, value="—").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_tot, column=8, value=-abs(df_cost_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=8).font = font_loss; ws_exec.cell(row=r_tot, column=8).fill = fill_loss
ws_exec.cell(row=r_tot, column=9, value=df_cost_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
ws_exec.cell(row=r_tot, column=9).font = font_loss; ws_exec.cell(row=r_tot, column=9).fill = fill_loss; ws_exec.cell(row=r_tot, column=9).alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_tot, column=10, value=df_cost_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
c_tot_wr = ws_exec.cell(row=r_tot, column=11, value=global_win_rate)
c_tot_wr.number_format = '0.00%'; c_tot_wr.font = font_win; c_tot_wr.fill = fill_win; c_tot_wr.alignment = Alignment(horizontal='center')

for c in range(1, 12): ws_exec.cell(row=r_tot, column=c).border = border_thin
...
ws_exec.cell(row=r_avg, column=2, value=df_cost_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=4, value=df_cost_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=5, value=df_cost_summary['Total Transaction Cost (0.5%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=5).font = font_loss; ws_exec.cell(row=r_avg, column=5).fill = fill_loss
ws_exec.cell(row=r_avg, column=6, value=df_cost_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_win; ws_exec.cell(row=r_avg, column=6).fill = fill_win
ws_exec.cell(row=r_avg, column=7, value=df_cost_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=8, value=-abs(df_cost_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=8).font = font_loss; ws_exec.cell(row=r_avg, column=8).fill = fill_loss
ws_exec.cell(row=r_avg, column=9, value=df_cost_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
ws_exec.cell(row=r_avg, column=9).font = font_loss; ws_exec.cell(row=r_avg, column=9).fill = fill_loss; ws_exec.cell(row=r_avg, column=9).alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=10, value=df_cost_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
c_avg_wr = ws_exec.cell(row=r_avg, column=11, value=global_win_rate)
c_avg_wr.number_format = '0.00%'; c_avg_wr.font = font_win; c_avg_wr.fill = fill_win; c_avg_wr.alignment = Alignment(horizontal='center')

for c in range(1, 12): ws_exec.cell(row=r_avg, column=c).border = border_double_bottom

autofit_columns(ws_exec, max_cols=11)

# Chart
chart_f = LineChart()
chart_f.title = "Master 12-Quarter Net Futures Realised P&L (After 0.5% Cost) (₹)"
chart_f.style = 13
...
data_f = Reference(ws_exec, min_col=6, min_row=7, max_row=19)
...
trade_headers = [
    ...
    'Contract Value (₹)', 'Required Margin Outlay (₹)', 'Raw Futures P&L (₹)',
    'Transaction Cost (0.5%) (₹)', 'Net Futures P&L (₹)', 'Cumulative P&L (₹)', 'Drawdown (₹)',
...
for qtr in quarters_order:
    ...
    q_sum = df_cost_summary[df_cost_summary['Quarter'] == qtr].iloc[0]
    
    q_tr['Cumulative P&L (₹)'] = q_tr['Net Futures P&L (After 0.5% Cost) (₹)'].cumsum()
    ...
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 FUTURES STRATEGY (WITH 0.5% TRANSACTION COST) — {qtr.upper()}"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ws_q.row_dimensions[1].height = 40
    
    card_lbls = ['Peak Margin Capital (₹)', 'Peak Date (Open Trades)', 'Raw Futures P&L (₹)', 'Total Trans Cost (0.5%) (₹)', 'Net Futures Realised P&L (₹)', 'Return on Margin (%)', 'Win Rate (%)', 'Total Trades', 'Max Drawdown (₹)', 'DD % Initial Margin', 'True DD % Peak Portfolio']
    ws_q.merge_cells('A2:K2')
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS & CAPITAL OVERVIEW (AFTER 0.5% COST)"; c_s.font = font_section; c_s.fill = fill_section
    ws_q.row_dimensions[2].height = 24
    
    style_table_headers(ws_q, 3, card_lbls, fill_subheader)
    ws_q.row_dimensions[3].height = 22
    
    ws_q.cell(row=4, column=1, value=q_sum['Peak Concurrent Margin (₹)']).number_format = '₹#,##0.00'
    ...
    ws_q.cell(row=4, column=3, value=q_sum['Raw Futures P&L (₹)']).number_format = '₹#,##0.00'
    
    c_tc1 = ws_q.cell(row=4, column=4, value=q_sum['Total Transaction Cost (0.5%) (₹)'])
    c_tc1.number_format = '₹#,##0.00'; c_tc1.font = font_loss; c_tc1.fill = fill_loss
    
    c_pnl = ws_q.cell(row=4, column=5, value=q_sum['Net Futures Realised P&L (₹)'])
    ...
    ws_q.cell(row=4, column=6, value=q_sum['Booked Return on Margin (%)']).number_format = '0.00%'
    
    c_wr = ws_q.cell(row=4, column=7, value=q_sum['Win Rate (%)'])
    c_wr.number_format = '0.00%'; c_wr.font = font_win; c_wr.fill = fill_win; c_wr.alignment = Alignment(horizontal='center')
    
    ws_q.cell(row=4, column=8, value=int(q_sum['Total Trades'])).alignment = Alignment(horizontal='center')
    ...
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE FUTURES JOURNAL (WITH 0.5% TRANSACTION COST)"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        c_st = ws_q.cell(row=row_num, column=5, value=t_row['Strategy'])
        c_st.alignment = Alignment(horizontal='center')
        if 'LONG' in t_row['Strategy']: c_st.fill = fill_win; c_st.font = font_win
        ...
        ws_q.cell(row=row_num, column=12, value=t_row['Contract Value (₹)']).number_format = '₹#,##0.00'
        ws_q.cell(row=row_num, column=13, value=t_row['Required Margin Outlay (₹)']).number_format = '₹#,##0.00'
        
        ws_q.cell(row=row_num, column=14, value=t_row['Raw Futures P&L (₹)']).number_format = '₹#,##0.00'
        
        c_cost = ws_q.cell(row=row_num, column=15, value=t_row['Transaction Cost (0.5%) (₹)'])
        c_cost.number_format = '₹#,##0.00'; c_cost.font = font_loss; c_cost.fill = fill_loss
        
        c_fp = ws_q.cell(row=row_num, column=16, value=t_row['Net Futures P&L (After 0.5% Cost) (₹)'])
        ...
        for c in range(1, 22):
            ...
            cell.border = border_thin
            if c not in [5, 15, 16, 18] and row_fill.fill_type: cell.fill = row_fill

    autofit_columns(ws_q, max_cols=21)
    ws_q.freeze_panes = 'A8'

    chart_q = LineChart()
    chart_q.title = f"{qtr} — Futures (0.5% Cost) Cumulative P&L Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
wb.save(OUTPUT_COST_MASTER)

# ------------------- 4. Build Standalone 0.5% Cost Summary Workbook -------------------
print("Writing Standalone 0.5% Cost Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Futures_0.5PCT_Cost_Summary')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:K1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY (WITH 0.5% TRANSACTION COST) — SUMMARY DASHBOARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
...
c_v1 = ws_s['A4']; c_v1.value = len(df_all_cost_trades); c_v1.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')

ws_s.merge_cells('C3:E3')
c_h2 = ws_s['C3']; c_h2.value = "GLOBAL WIN RATE (AFTER 0.5% COST)"; c_h2.font = font_card_lbl; c_h2.fill = fill_subheader; c_h2.alignment = Alignment(horizontal='center', vertical='center')
...
ws_s.merge_cells('F3:H3')
c_h3 = ws_s['F3']; c_h3.value = "TOTAL TRANSACTION COST (0.5%)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('F4:H4')
c_v3 = ws_s['F4']; c_v3.value = global_tot_trans; c_v3.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_LOSS_TEXT); c_v3.fill = fill_loss; c_v3.number_format = '₹#,##0.00'; c_v3.alignment = Alignment(horizontal='center', vertical='center')

ws_s.merge_cells('I3:K3')
c_h4 = ws_s['I3']; c_h4.value = "NET 12-QUARTER FUTURES PROFIT"; c_h4.font = font_card_lbl; c_h4.fill = fill_subheader; c_h4.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('I4:K4')
c_v4 = ws_s['I4']; c_v4.value = global_tot_net_pnl; c_v4.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v4.fill = fill_win; c_v4.number_format = '₹#,##0.00'; c_v4.alignment = Alignment(horizontal='center', vertical='center')

ws_s.row_dimensions[3].height = 20
ws_s.row_dimensions[4].height = 28
for r in [3, 4]:
    for c in range(1, 12): ws_s.cell(row=r, column=c).border = border_thin

ws_s.merge_cells('A6:K6')
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (WITH 0.5% TRANSACTION COST APPLIED)"
...
for r_idx, row_data in df_cost_summary.iterrows():
    ...
    ws_s.cell(row=row_num, column=2, value=row_data['Peak Concurrent Margin (₹)']).number_format = '₹#,##0.00'
    ...
    ws_s.cell(row=row_num, column=4, value=row_data['Raw Futures P&L (₹)']).number_format = '₹#,##0.00'
    
    c_tc = ws_s.cell(row=row_num, column=5, value=row_data['Total Transaction Cost (0.5%) (₹)'])
    c_tc.number_format = '₹#,##0.00'; c_tc.font = font_loss; c_tc.fill = fill_loss
    
    c_pnl = ws_s.cell(row=row_num, column=6, value=row_data['Net Futures Realised P&L (₹)'])
    c_pnl.number_format = '₹#,##0.00'; c_pnl.fill = fill_win if row_data['Net Futures Realised P&L (₹)'] >= 0 else fill_loss
    c_pnl.font = font_win if row_data['Net Futures Realised P&L (₹)'] >= 0 else font_loss
    
    ws_s.cell(row=row_num, column=7, value=row_data['Booked Return on Margin (%)']).number_format = '+0.00%;-0.00%'
    
    c_dd = ws_s.cell(row=row_num, column=8, value=-abs(row_data['Max Drawdown (₹)']))
    c_dd.number_format = '-₹#,##0.00'; c_dd.font = font_loss; c_dd.fill = fill_loss
    
    c_ddi = ws_s.cell(row=row_num, column=9, value=row_data['DD % on Initial Capital'])
    c_ddi.number_format = '-0.00%'; c_ddi.font = font_loss; c_ddi.fill = fill_loss; c_ddi.alignment = Alignment(horizontal='center')
    
    c_ddp = ws_s.cell(row=row_num, column=10, value=row_data['True DD % on Peak Portfolio'])
    c_ddp.number_format = '-0.00%'; c_ddp.font = font_loss; c_ddp.fill = fill_loss; c_ddp.alignment = Alignment(horizontal='center')
    
    c_wr = ws_s.cell(row=row_num, column=11, value=row_data['Win Rate (%)'])
    c_wr.number_format = '0.00%'; c_wr.alignment = Alignment(horizontal='center')
    if row_data['Win Rate (%)'] >= 0.70: c_wr.fill = fill_win; c_wr.font = font_win
    elif row_data['Win Rate (%)'] >= 0.50: c_wr.fill = fill_gold; c_wr.font = font_gold
    else: c_wr.fill = fill_loss; c_wr.font = font_loss
...
ws_s.cell(row=r_tot, column=4, value=global_tot_raw_pnl).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=5, value=global_tot_trans).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=5).font = font_loss; ws_s.cell(row=r_tot, column=5).fill = fill_loss
ws_s.cell(row=r_tot, column=6, value=global_tot_net_pnl).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=6).font = font_win; ws_s.cell(row=r_tot, column=6).fill = fill_win
ws_s.cell(row=r_tot, column=7, value="—").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_tot, column=8, value=-abs(df_cost_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=8).font = font_loss; ws_s.cell(row=r_tot, column=8).fill = fill_loss
ws_s.cell(row=r_tot, column=9, value=df_cost_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
ws_s.cell(row=r_tot, column=9).font = font_loss; ws_s.cell(row=r_tot, column=9).fill = fill_loss; ws_s.cell(row=r_tot, column=9).alignment = Alignment(horizontal='center')
ws_s.cell(row=r_tot, column=10, value=df_cost_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
ws_s.cell(row=r_tot, column=10).font = font_loss; ws_s.cell(row=r_tot, column=10).fill = fill_loss; ws_s.cell(row=r_tot, column=10).alignment = Alignment(horizontal='center')
c_twr = ws_s.cell(row=r_tot, column=11, value=global_win_rate)
c_twr.number_format = '0.00%'; c_twr.font = font_win; c_twr.fill = fill_win; c_twr.alignment = Alignment(horizontal='center')
for c in range(1, 12): ws_s.cell(row=r_tot, column=c).border = border_thin

# Row 21 Average
r_avg = 21
ws_s.row_dimensions[r_avg].height = 24
ws_s.cell(row=r_avg, column=1, value="QUARTERLY AVERAGE").font = font_data_bold
ws_s.cell(row=r_avg, column=2, value=df_cost_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=4, value=df_cost_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=5, value=df_cost_summary['Total Transaction Cost (0.5%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=5).font = font_loss; ws_s.cell(row=r_avg, column=5).fill = fill_loss
ws_s.cell(row=r_avg, column=6, value=df_cost_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_win; ws_s.cell(row=r_avg, column=6).fill = fill_win
ws_s.cell(row=r_avg, column=7, value=df_cost_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=8, value=-abs(df_cost_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=8).font = font_loss; ws_s.cell(row=r_avg, column=8).fill = fill_loss
ws_s.cell(row=r_avg, column=9, value=df_cost_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
ws_s.cell(row=r_avg, column=9).font = font_loss; ws_s.cell(row=r_avg, column=9).fill = fill_loss; ws_s.cell(row=r_avg, column=9).alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=10, value=df_cost_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
ws_s.cell(row=r_avg, column=10).font = font_loss; ws_s.cell(row=r_avg, column=10).fill = fill_loss; ws_s.cell(row=r_avg, column=10).alignment = Alignment(horizontal='center')
c_awr = ws_s.cell(row=r_avg, column=11, value=global_win_rate)
c_awr.number_format = '0.00%'; c_awr.font = font_win; c_awr.fill = fill_win; c_awr.alignment = Alignment(horizontal='center')
for c in range(1, 12): ws_s.cell(row=r_avg, column=c).border = border_double_bottom
...
# Chart
chart = LineChart()
chart.title = "Consolidated 12-Quarter Net Futures Realised P&L (After 0.5% Cost) Trend (₹)"
chart.style = 13
chart.y_axis.title = "Net Realised P&L (₹)"
chart.x_axis.title = "Quarter"
chart.width = 16
chart.height = 11

data = Reference(ws_s, min_col=6, min_row=7, max_row=19)
cats = Reference(ws_s, min_col=1, min_row=8, max_row=19)
chart.add_data(data, titles_from_data=True)
chart.set_categories(cats)
ws_s.add_chart(chart, "M3")

wb_s.save(OUTPUT_COST_SUMMARY)
print(f"\n🎉 FUTURES 0.5% COST MASTER & SUMMARY CREATED IN {time.time() - t0:.1f}s!", flush=True)
