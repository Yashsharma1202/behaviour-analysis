...
async function renderBehaviour(){
  ...
  for(const et of keys){
    ...
    html+=`<div class="bcard" onclick="openBeh('${et}')">
      ...
      <div class="brec">Buy <b>${dbf} candle${dbf>1?'s':''} before</b> the event, then sell <b>${daf} candle${daf>1?'s':''} after</b>.</div>
      <div class="bstats">
        <div class="bstat">
...
function openRecent(et){
  ...
  bv.innerHTML=`
    ...
    <div class="bverdict v-${m3>=0?'strong':'no'}" style="display:inline-block">
      ${outcome}: buying 6 candles before and selling 3 candles after would have returned ${s(m3)}</div>
...
function openHistory(et){
  ...
  bv.innerHTML=`
    ...
    <div class="enote">Each row: buy 6 candles before the event, sell 3 candles after — the same
    ...
    <div class="tblwrap" style="margin-top:12px"><table><thead><tr>
      ...
      <th style="text-align:right">Event candle</th><th style="text-align:right">Sell (T+3)</th>
...
function openBeh(et){
  ...
  bv.innerHTML=`
    ...
    <div class="brec" style="margin-top:12px">Best rule: buy <b>${b.days_before} candle${b.days_before>1?'s':''} before</b> the event, sell <b>${b.days_after} candle${b.days_after>1?'s':''} after</b>.</div>
    ...
    <div class="statrow" style="grid-template-columns:repeat(3,1fr)">
      <div class="sbox"><div class="l">6 candles before</div><div class="v" style="color:${COL.amber}">₹${rup(t.last.prices[0])}</div></div>
      <div class="sbox"><div class="l">On event candle</div><div class="v">₹${rup(t.last.prices[t.last.days.indexOf(0)])}</div></div>
      <div class="sbox"><div class="l">3 candles after</div><div class="v" style="color:${COL.pos}">₹${rup(t.last.prices[t.last.prices.length-1])}</div></div>
    </div>`:''}
