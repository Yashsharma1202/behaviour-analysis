...
function showTab(name){
  ...
  const isFund=name==="fundamentals", isBeh=name==="behaviour", isRank=name==="rankings",
        isScr=name==="screener", isCmp=name==="compare";
  ...
  $("eventsview").style.display=(isFund||isBeh||isRank||isScr||isCmp)?"none":"";
...
function drawRankings(){
  ...
  rv.innerHTML=`
    ...
    <div class="enote">${list.length} of ${d.n_stocks} stocks have ${esc(pretty(rankEvent))} data.
      Win rate = share of past events where the best buy/sell rule made money.
      <b>Note:</b> this is in-sample — see the concept note on why high past win-rates don't guarantee future profit.</div>`;
}
...
function drawScreener(){
  const sv=$("screenview");
  if(!screenData||!screenData.stocks||!screenData.stocks.length){
    sv.innerHTML='<div class="banner">No fundamentals loaded yet. If this persists, hard-refresh the page (Ctrl+Shift+R).</div>'; return; }
  ...
  // column stats for good/bad coloring
  ...
  sv.innerHTML=`
    <div class="bintro">Filter the tracked universe (<b>${all.length}</b> stocks — the Nifty 50 plus a couple
