...
OUTPUT_FUTURES_MASTER = BASE_DIR / 'Nifty50_12_Quarters_Futures_OI_Master_v7.xlsx'
OUTPUT_FUTURES_ONLY = BASE_DIR / 'Futures_Master_Only_v6.xlsx'
...
# ------------------- 8. Build Stunning 17-Sheet Excel Workbook -------------------
print("Building Stunning 17-Sheet Master Workbook...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
# Apply thin borders to hero cards
for r in [3, 4]:
    for c in range(1, 11):
...
print(f"\n🎉 STUNNING EXECUTIVE FUTURES MASTER CREATED IN {time.time() - t0:.1f}s!", flush=True)
