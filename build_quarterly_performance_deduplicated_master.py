...
print("==========================================================================================")
print("BUILDING DEDUPLICATED QUARTERLY PERFORMANCE WITH SECTORS MASTER WORKBOOK")
print("==========================================================================================")
    ...
    if fr_path.exists() and fr_path.stat().st_size > 10:
        try:
            ...
            if not df_fr.empty and 'broadCastDate' in df_fr.columns:
                ...
                # DEDUPLICATE MULTIPLE NOTICES FOR THE SAME FINANCIAL QUARTER (Year + Quarter)
    ...
    for q_t in ["Q1", "Q2", "Q3", "Q4"]:
        ...
        if tot_q_count == 0:
            quarter_data_store[q_t].append({
                "sym": sym, "sector": sector_name, "trading_status": trading_status,
        ...
        win_ratio_str = f"{opt_wins} Wins / {tot_q_count} {q_t} Qtrs"
...
out_path = ROOT / "Nifty211_Quarterly_Performance_Deduplicated_Sectors_Master.xlsx"
...
quarter_names = {
    "Q1": "Q1 Performance with Sectors",
    "Q2": "Q2 Performance with Sectors",
    "Q3": "Q3 Performance with Sectors",
    "Q4": "Q4 Performance with Sectors"
}
...
for q_idx, q_t in enumerate(["Q1", "Q2", "Q3", "Q4"]):
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {q_t} DEDUPLICATED PERFORMANCE WITH SECTORS & QUARTER COUNTS"
    ...
    cards = [
        ("TOTAL STOCKS RANKED", f"{len(df_sorted)} Stocks ({len(df_avail)} Avail)", "A3:B4", "A3"),
    ...
    # Main Ranking Table
    rank_start_row = 6
    ws.cell(row=rank_start_row, column=1, value=f"ALL {len(df_sorted)} STOCKS RANKED BY {q_t} AVERAGE RETURN % (DEDUPLICATED UNIQUE FINANCIAL QUARTERS)").font = hdr_font
    ws.cell(row=rank_start_row, column=1).fill = hdr_fill
    ws.merge_cells(start_row=rank_start_row, start_column=1, end_row=rank_start_row, end_column=14)

    headers = [
        "Rank", "Stock Symbol", "Industry Sector", "Optimal Quarter Strategy", "Action Status",
        "Total History Unique Quarters", "Specific Quarter Unique Data Count", "Position Window", "Specific Quarter Win Rate %",
        "Specific Quarter Win Ratio", "Losing Quarters", "Quarter Average Return %",
        "Margin Capital (₹)", "Quarter Cumulative Net P&L (₹)"
    ]
    ...
    for r_idx, r in enumerate(df_sorted.to_dict('records'), start=rank_start_row+2):
        ...
        c_st = ws.cell(row=r_idx, column=4, value=r['strategy'])
        ...
        ws.cell(row=r_idx, column=6, value=r['n_quarters_total']).border = border_thin
        ws.cell(row=r_idx, column=6).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=7, value=r['n_quarters_qtype']).border = border_thin
        ws.cell(row=r_idx, column=7).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=8, value=r['window']).border = border_thin
        ws.cell(row=r_idx, column=8).alignment = Alignment(horizontal="center")
        
        wr_val_str = f"{r['win_rate']}%" if r['is_available'] else "N/A"
        c_wr = ws.cell(row=r_idx, column=9, value=wr_val_str)
        ...
        c_w = ws.cell(row=r_idx, column=10, value=r['win_ratio_str'])
        ...
        c_l = ws.cell(row=r_idx, column=11, value=r['losses'] if r['is_available'] else "N/A")
        ...
        c_ret = ws.cell(row=r_idx, column=12, value=ret_val_str)
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_Quarterly_Performance_With_Sectors_Master.xlsx")
print("==========================================================================================")
print("DEDUPLICATED QUARTERLY PERFORMANCE WITH SECTORS MASTER WORKBOOK CREATED SUCCESSFULLY!")
print("==========================================================================================")
