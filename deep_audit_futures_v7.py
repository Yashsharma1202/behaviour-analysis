...
print("==========================================")
print("DEEP AUDIT OF Nifty50_12_Quarters_Futures_OI_Master_v7.xlsx")
print("==========================================")
...
print(f"  Entry Col : '{en_col}'")
print(f"  Exit Col  : '{ex_col}'")
print(f"  Lot Col   : '{lot_col}'")

# Check 1: Audit trade row calculations
pnl_errors = 0
ret_errors = 0
...
for q in quarters:
    ...
    # Summary card values are in row index 2 (excel row 4)
    card_vals = ws_df.iloc[2].values
    
    # Trade rows start at index 6 (excel row 8)
    ...
    sum_pnl = trade_rows.iloc[:, 13].astype(float).sum()
    ...
    if abs(card_pnl - sum_pnl) > 1.0 or card_wins != sum_wins or card_losses != sum_losses or card_tot != sum_tot:
        banner_mismatches += 1
...
print(f"  Banner Mismatches across 12 Quarters: {banner_mismatches}")
