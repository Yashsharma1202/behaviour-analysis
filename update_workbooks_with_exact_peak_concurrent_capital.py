...
# ------------------- 2. Calculate Exact Day-by-Day Peak Concurrent Capital & Drawdown -------------------
opt_summaries = []
...
print("Writing Exact Day-by-Day Peak Concurrent Capital to Options Master Excel...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS BUYING STRATEGY — DAY-BY-DAY PEAK CONCURRENT CAPITAL SCORECARD"
...
for r in [3, 4]:
    for c in range(1, 9): ws_exec.cell(row=r, column=c).border = border_thin

ws_exec.merge_cells('A6:H6')
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PEAK CONCURRENT CAPITAL & TRUE PEAK DRAWDOWN SCORECARD"
...
headers_summary = ['Quarter', 'Peak Concurrent Capital at Single Time (₹)', 'Peak Date (Max Open Trades)', 'Net Option Realised P&L (₹)', 'Return on Peak Capital (%)', 'Max Realised Drawdown (₹)', 'True Peak DD (%)', 'Win Rate (%)']
...
for r_idx, row_data in df_opt_summary.iterrows():
    ...
    c_ddp = ws_exec.cell(row=row_num, column=7, value=row_data['True Peak DD (%)'])
    c_ddp.number_format = '-0.00%'; c_ddp.font = font_loss; c_ddp.fill = fill_loss; c_ddp.alignment = Alignment(horizontal='center')
    
    c_wr = ws_exec.cell(row=row_num, column=8, value=row_data['Win Rate (%)'])
    ...
    for c in range(1, 9):
        ...
        cell.border = border_thin
        if c not in [4, 6, 7, 8] and row_fill.fill_type: cell.fill = row_fill
...
ws_exec.cell(row=r_tot, column=6, value=-abs(df_opt_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=6).font = font_loss; ws_exec.cell(row=r_tot, column=6).fill = fill_loss
ws_exec.cell(row=r_tot, column=7, value=df_opt_summary['True Peak DD (%)'].min()).number_format = '-0.00%'
...
c_tot_wr = ws_exec.cell(row=r_tot, column=8, value=global_win_rate)
c_tot_wr.number_format = '0.00%'; c_tot_wr.font = font_win; c_tot_wr.fill = fill_win; c_tot_wr.alignment = Alignment(horizontal='center')

for c in range(1, 9): ws_exec.cell(row=r_tot, column=c).border = border_thin
...
ws_exec.cell(row=r_avg, column=2, value=df_opt_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=4, value=df_opt_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=4).font = font_win; ws_exec.cell(row=r_avg, column=4).fill = fill_win
ws_exec.cell(row=r_avg, column=5, value=df_opt_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=6, value=-abs(df_opt_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_loss; ws_exec.cell(row=r_avg, column=6).fill = fill_loss
ws_exec.cell(row=r_avg, column=7, value=df_opt_summary['True Peak DD (%)'].mean()).number_format = '-0.00%'
...
c_avg_wr = ws_exec.cell(row=r_avg, column=8, value=global_win_rate)
c_avg_wr.number_format = '0.00%'; c_avg_wr.font = font_win; c_avg_wr.fill = fill_win; c_avg_wr.alignment = Alignment(horizontal='center')

for c in range(1, 9): ws_exec.cell(row=r_avg, column=c).border = border_double_bottom

autofit_columns(ws_exec, max_cols=8)
...
ws_exec.add_chart(chart_opt, "J3")
...
for qtr in quarters_order:
    ...
    card_lbls = ['Peak Concurrent Capital at Single Time (₹)', 'Peak Date (Open Trades)', 'Net Option P&L (₹)', 'Return on Peak Capital (%)', 'Win Rate (%)', 'Total Trades', 'Winning Trades', 'Losing Trades', 'Max Drawdown (₹)', 'True Peak DD (%)', 'Slots Deployed']
    ws_q.merge_cells('A2:K2')
    c_s = ws_q['A2']; c_s.value = "1% ITM OPTIONS PERFORMANCE & PEAK CONCURRENT CAPITAL OVERVIEW"; c_s.font = font_section; c_s.fill = fill_section
    ...
    ws_q.cell(row=4, column=11, value=int(q_sum['Peak Open Trades'])).alignment = Alignment(horizontal='center')
...
# ------------------- 4. Build Standalone Summary Workbook -------------------
print("Writing Standalone Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Options_1PCT_ITM_Summary')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:H1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — DAY-BY-DAY PEAK CONCURRENT CAPITAL SUMMARY SCORECARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
...
for r in [3, 4]:
    for c in range(1, 9): ws_s.cell(row=r, column=c).border = border_thin

ws_s.merge_cells('A6:H6')
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PEAK CONCURRENT CAPITAL & TRUE PEAK DRAWDOWN SCORECARD"
...
for r_idx, row_data in df_opt_summary.iterrows():
    ...
    c_ddp = ws_s.cell(row=row_num, column=7, value=row_data['True Peak DD (%)'])
    c_ddp.number_format = '-0.00%'; c_ddp.font = font_loss; c_ddp.fill = fill_loss; c_ddp.alignment = Alignment(horizontal='center')
    
    c_wr = ws_s.cell(row=row_num, column=8, value=row_data['Win Rate (%)'])
    c_wr.number_format = '0.00%'; c_wr.alignment = Alignment(horizontal='center')
    if row_data['Win Rate (%)'] >= 0.70: c_wr.fill = fill_win; c_wr.font = font_win
    elif row_data['Win Rate (%)'] >= 0.50: c_wr.fill = fill_gold; c_wr.font = font_gold
    else: c_wr.fill = fill_loss; c_wr.font = font_loss
...
ws_s.cell(row=r_tot, column=6, value=-abs(df_opt_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=6).font = font_loss; ws_s.cell(row=r_tot, column=6).fill = fill_loss
ws_s.cell(row=r_tot, column=7, value=df_opt_summary['True Peak DD (%)'].min()).number_format = '-0.00%'
...
c_twr = ws_s.cell(row=r_tot, column=8, value=global_win_rate)
c_twr.number_format = '0.00%'; c_twr.font = font_win; c_twr.fill = fill_win; c_twr.alignment = Alignment(horizontal='center')
for c in range(1, 9): ws_s.cell(row=r_tot, column=c).border = border_thin

# Row 21 Average
r_avg = 21
ws_s.row_dimensions[r_avg].height = 24
ws_s.cell(row=r_avg, column=1, value="QUARTERLY AVERAGE").font = font_data_bold
ws_s.cell(row=r_avg, column=2, value=df_opt_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=4, value=df_opt_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=4).font = font_win; ws_s.cell(row=r_avg, column=4).fill = fill_win
ws_s.cell(row=r_avg, column=5, value=df_opt_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=6, value=-abs(df_opt_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_loss; ws_s.cell(row=r_avg, column=6).fill = fill_loss
ws_s.cell(row=r_avg, column=7, value=df_opt_summary['True Peak DD (%)'].mean()).number_format = '-0.00%'
...
c_awr = ws_s.cell(row=r_avg, column=8, value=global_win_rate)
c_awr.number_format = '0.00%'; c_awr.font = font_win; c_awr.fill = fill_win; c_awr.alignment = Alignment(horizontal='center')
for c in range(1, 9): ws_s.cell(row=r_avg, column=c).border = border_double_bottom
...
ws_s.add_chart(chart, "J3")

wb_s.save(OUTPUT_SUMMARY)
print(f"\n🎉 ALL WORKBOOKS UPDATED WITH EXACT PEAK CONCURRENT CAPITAL IN {time.time() - t0:.1f}s!", flush=True)
