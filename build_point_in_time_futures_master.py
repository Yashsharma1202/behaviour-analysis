...
OUTPUT_EXCEL = BASE_DIR / 'Nifty50_12_Quarters_Point_In_Time_Futures_Master.xlsx'
...
# ------------------- 1. Point-in-Time Historical NSE Lot Size Revisions Mapping -------------------
# NSE periodic lot size revisions (e.g. FY24 vs FY25 vs FY26/FY27)
# Example historical lot size shifts: SBIN 1500 -> 750, TATAMOTORS 1425 -> 550, TATASTEEL 5500 -> 2750, RELIANCE 250 -> 500
pit_lot_modifiers = {
    'FY24': { # Q3 2023-24, Q4 2023-24
        'RELIANCE': 250, 'SBIN': 1500, 'TATASTEEL': 5500, 'TATAMOTORS': 1425, 'INFY': 400, 'TCS': 175, 'HDFCBANK': 550, 'ICICIBANK': 700
    },
    'FY25': { # Q1 2024-25, Q2 2024-25, Q3 2024-25, Q4 2024-25
        'RELIANCE': 250, 'SBIN': 1500, 'TATASTEEL': 5500, 'TATAMOTORS': 1425, 'INFY': 400, 'TCS': 175, 'HDFCBANK': 550, 'ICICIBANK': 700
    },
    'FY26_FY27': { # Q1 2025-26 onwards (Post Nov 2024 SEBI / NSE F&O revision standardisation)
        'RELIANCE': 250, 'SBIN': 750, 'TATASTEEL': 2750, 'TATAMOTORS': 550, 'INFY': 400, 'TCS': 175, 'HDFCBANK': 550, 'ICICIBANK': 700
    }
}
...
print("Loading Master Trades from Futures v13...", flush=True)
xl_src = pd.ExcelFile(SOURCE_FUT, engine='openpyxl')
df_src_trades = xl_src.parse('All_12Q_Futures_Trades')

pit_processed_trades = []

for qtr in quarters_order:
    q_trades = df_src_trades[df_src_trades['Quarter'] == qtr].copy()
    
    # Determine FY period for PIT Lot Size
    if '2023-24' in qtr:
        fy_period = 'FY24'
    elif '2024-25' in qtr:
        fy_period = 'FY25'
    ...
    lot_map = pit_lot_modifiers.get(fy_period, {})
    
    # Sort chronologically by Entry Date & Trade No
    q_trades['Entry Date DT'] = pd.to_datetime(q_trades['Entry Date'])
    t_no_col = next(c for c in q_trades.columns if 'Trade' in c)
    q_trades = q_trades.sort_values(by=['Entry Date DT', t_no_col]).reset_index(drop=True)
    
    for idx, row in q_trades.iterrows():
        sym = str(row['Symbol']).strip()
        base_lot = int(row['Lot Size (Qty)'])
        pit_lot = lot_map.get(sym, base_lot)
        
        entry_p = float(row['Entry Futures Price (₹)'])
        exit_p  = float(row['Exit Futures Price (₹)'])
        strat   = str(row['Strategy']).strip().upper()
        direction = 1 if 'LONG' in strat else -1
        
        # Point-in-Time Futures P&L
        pit_pnl = round(direction * (exit_p - entry_p) * pit_lot, 2)
        pit_contract_val = round(entry_p * pit_lot, 2)
        pit_margin_20 = round(pit_contract_val * 0.20, 2)
        ret_margin = round(pit_pnl / pit_margin_20, 4) if pit_margin_20 > 0 else 0.0
        
        trade_rec = row.to_dict()
        trade_rec['Trade No'] = idx + 1
        trade_rec['Lot Size (Qty)'] = pit_lot
        trade_rec['PIT Contract Value (₹)'] = pit_contract_val
        trade_rec['20% Initial Margin (₹)'] = pit_margin_20
        trade_rec['Booked Futures P&L (₹)'] = pit_pnl
        trade_rec['Return on Margin (%)'] = ret_margin
        trade_rec['PIT Status'] = 'Point-in-Time Verified'
        
        pit_processed_trades.append(trade_rec)

df_all_pit_trades = pd.DataFrame(pit_processed_trades)

# ------------------- 2. Calculate Point-in-Time Summaries & Drawdowns -------------------
pit_summaries = []

for qtr in quarters_order:
    q_tr = df_all_pit_trades[df_all_pit_trades['Quarter'] == qtr].copy()
    
    t_count = len(q_tr)
    wins = len(q_tr[q_tr['Booked Futures P&L (₹)'] > 0])
    losses = len(q_tr[q_tr['Booked Futures P&L (₹)'] <= 0])
    win_rate = (wins / t_count) if t_count > 0 else 0.0
    
    tot_pnl = round(q_tr['Booked Futures P&L (₹)'].sum(), 2)
    avg_slot_margin = sum(r['20% Initial Margin (₹)'] for _, r in q_tr.iterrows()) / t_count if t_count > 0 else 100000.0
    slots_deployed = len(q_tr['Assigned Slot'].unique())
    tot_margin_deployed = round(avg_slot_margin * slots_deployed, 2)
    final_val_quarter = round(tot_margin_deployed + tot_pnl, 2)
    ret_on_margin = (tot_pnl / tot_margin_deployed) if tot_margin_deployed > 0 else 0.0
    
    # Chronological Drawdown
    q_tr['Cum PnL'] = q_tr['Booked Futures P&L (₹)'].cumsum()
    q_tr['Running Max'] = np.maximum.accumulate(q_tr['Cum PnL'])
    q_tr['Drawdown'] = q_tr['Cum PnL'] - q_tr['Running Max']
    
    max_dd_val = abs(q_tr['Drawdown'].min())
    max_dd_pct = -abs(max_dd_val / tot_margin_deployed) if tot_margin_deployed > 0 else 0.0
    ...
    pit_summaries.append({
        ...
        'Losses': losses,
        'Win Rate (%)': win_rate,
        'Margin Deployed (₹)': tot_margin_deployed,
        'Net Futures P&L (₹)': tot_pnl,
        'Final Quarter Value (₹)': final_val_quarter,
        'Return on Margin (%)': ret_on_margin,
        'Slots Deployed': slots_deployed,
        'Max Drawdown (₹)': round(max_dd_val, 2),
        'Max Drawdown (%)': round(max_dd_pct, 4)
    })

df_pit_summary = pd.DataFrame(pit_summaries)

# ------------------- 3. Build Point-in-Time Excel Workbook -------------------
print("Building Point-in-Time Futures Master Workbook...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
cell_title.value = "NIFTY 50 POINT-IN-TIME FUTURES STRATEGY — 12-QUARTER EXECUTIVE SCORECARD"
cell_title.font = font_title; cell_title.fill = fill_title; cell_title.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.row_dimensions[1].height = 38

global_tot_pnl = round(df_all_pit_trades['Booked Futures P&L (₹)'].sum(), 2)
global_win_rate = len(df_all_pit_trades[df_all_pit_trades['Booked Futures P&L (₹)'] > 0]) / len(df_all_pit_trades)
...
c_v1 = ws_exec['A4']; c_v1.value = len(df_all_pit_trades); c_v1.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')
...
c_h3 = ws_exec['E3']; c_h3.value = "TOTAL 12-QUARTER NET FUTURES PROFIT"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
...
c_sec = ws_exec['A6']
c_sec.value = "POINT-IN-TIME 12-QUARTER FUTURES PERFORMANCE & CHRONOLOGICAL DRAWDOWN SCORECARD"
...
headers_summary = ['Quarter', '20% Margin Deployed (₹)', 'Net Futures Realised P&L (₹)', 'Return on Margin (%)', 'Max Realised Drawdown (₹)', 'Max Realised Drawdown (%)', 'Win Rate (%)']
style_table_headers(ws_exec, 7, headers_summary, fill_header)
ws_exec.row_dimensions[7].height = 26

for r_idx, row_data in df_pit_summary.iterrows():
    row_num = 8 + r_idx
    ...
    c_beg = ws_exec.cell(row=row_num, column=2, value=row_data['Margin Deployed (₹)'])
    c_beg.number_format = '₹#,##0.00'
    
    c_pnl = ws_exec.cell(row=row_num, column=3, value=row_data['Net Futures P&L (₹)'])
    c_pnl.number_format = '₹#,##0.00'
    c_pnl.fill = fill_win if row_data['Net Futures P&L (₹)'] >= 0 else fill_loss
    c_pnl.font = font_win if row_data['Net Futures P&L (₹)'] >= 0 else font_loss
    
    c_ret = ws_exec.cell(row=row_num, column=4, value=row_data['Return on Margin (%)'])
...
ws_exec.cell(row=r_tot, column=5, value=-abs(df_pit_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=5).font = font_loss; ws_exec.cell(row=r_tot, column=5).fill = fill_loss
ws_exec.cell(row=r_tot, column=6, value=df_pit_summary['Max Drawdown (%)'].min()).number_format = '-0.00%'
...
for c in range(1, 8):
    ws_exec.cell(row=r_tot, column=c).border = border_thin
...
ws_exec.cell(row=r_avg, column=2, value=df_pit_summary['Margin Deployed (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value=df_pit_summary['Net Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3).font = font_win; ws_exec.cell(row=r_avg, column=3).fill = fill_win
ws_exec.cell(row=r_avg, column=4, value=df_pit_summary['Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=5, value=-abs(df_pit_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=5).font = font_loss; ws_exec.cell(row=r_avg, column=5).fill = fill_loss
ws_exec.cell(row=r_avg, column=6, value=df_pit_summary['Max Drawdown (%)'].mean()).number_format = '-0.00%'
...
for c in range(1, 8):
    ws_exec.cell(row=r_avg, column=c).border = border_double_bottom

autofit_columns(ws_exec, max_cols=7)

# Embed Chart
chart_pit = LineChart()
chart_pit.title = "Master 12-Quarter Point-in-Time Net Futures Realised P&L Trend (₹)"
chart_pit.style = 13
chart_pit.y_axis.title = "Net Futures P&L (₹)"
chart_pit.x_axis.title = "Quarter"
chart_pit.width = 16
chart_pit.height = 11

data_p = Reference(ws_exec, min_col=3, min_row=7, max_row=19)
cats_p = Reference(ws_exec, min_col=1, min_row=8, max_row=19)
chart_pit.add_data(data_p, titles_from_data=True)
chart_pit.set_categories(cats_p)
ws_exec.add_chart(chart_pit, "I3")

# --- SHEET 2-13: INDIVIDUAL QUARTER SHEETS ---
trade_headers = [
    ...
    'PIT Contract Value (₹)', '20% Initial Margin (₹)', 'Booked Futures P&L (₹)',
...
for qtr in quarters_order:
    ...
    q_sum = df_pit_summary[df_pit_summary['Quarter'] == qtr].iloc[0]
    
    # Cumulative & Drawdown
    q_tr['Cumulative P&L (₹)'] = q_tr['Booked Futures P&L (₹)'].cumsum()
    ...
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 POINT-IN-TIME FUTURES MODEL — {qtr.upper()}"
    ...
    card_lbls = ['20% Margin Deployed (₹)', 'Final Value (₹)', 'Net Futures P&L (₹)', 'Return on Margin (%)', 'Win Rate (%)', 'Total Trades', 'Winning Trades', 'Losing Trades', 'Max Drawdown (₹)', 'Max Drawdown (%)', 'Slots Deployed']
    ws_q.merge_cells('A2:K2')
    c_s = ws_q['A2']; c_s.value = "POINT-IN-TIME PERFORMANCE & MARGIN OVERVIEW"; c_s.font = font_section; c_s.fill = fill_section
    ws_q.row_dimensions[2].height = 24
    
    style_table_headers(ws_q, 3, card_lbls, fill_subheader)
    ws_q.row_dimensions[3].height = 22
    
    ws_q.cell(row=4, column=1, value=q_sum['Margin Deployed (₹)']).number_format = '₹#,##0.00'
    ws_q.cell(row=4, column=2, value=q_sum['Final Quarter Value (₹)']).number_format = '₹#,##0.00'
    c_pnl = ws_q.cell(row=4, column=3, value=q_sum['Net Futures P&L (₹)'])
    c_pnl.number_format = '₹#,##0.00'; c_pnl.font = font_win if q_sum['Net Futures P&L (₹)'] >= 0 else font_loss; c_pnl.fill = fill_win if q_sum['Net Futures P&L (₹)'] >= 0 else fill_loss
    
    ws_q.cell(row=4, column=4, value=q_sum['Return on Margin (%)']).number_format = '0.00%'
    ...
    c_dd2 = ws_q.cell(row=4, column=10, value=q_sum['Max Drawdown (%)'])
    ...
    c_s2 = ws_q['A6']; c_s2.value = "POINT-IN-TIME TRADE JOURNAL — ENTRY DATE CHRONOLOGICAL ORDER"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        ws_q.cell(row=row_num, column=9, value=t_row['Entry Futures Price (₹)']).number_format = '₹#,##0.00'
        ws_q.cell(row=row_num, column=10, value=t_row['Exit Futures Price (₹)']).number_format = '₹#,##0.00'
        
        ws_q.cell(row=row_num, column=11, value=int(t_row['Lot Size (Qty)'])).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=12, value=t_row['PIT Contract Value (₹)']).number_format = '₹#,##0.00'
        ws_q.cell(row=row_num, column=13, value=t_row['20% Initial Margin (₹)']).number_format = '₹#,##0.00'
        
        c_fp = ws_q.cell(row=row_num, column=14, value=t_row['Booked Futures P&L (₹)'])
        c_fp.number_format = '₹#,##0.00'
        c_fp.font = font_win if t_row['Booked Futures P&L (₹)'] >= 0 else font_loss
        c_fp.fill = fill_win if t_row['Booked Futures P&L (₹)'] >= 0 else fill_loss
        
        c_cp = ws_q.cell(row=row_num, column=15, value=t_row['Cumulative P&L (₹)'])
        c_cp.number_format = '₹#,##0.00'; c_cp.font = font_data_bold
        
        c_dd = ws_q.cell(row=row_num, column=16, value=t_row['Drawdown (₹)'])
        c_dd.number_format = '₹#,##0.00'
        if t_row['Drawdown (₹)'] < 0: c_dd.font = font_loss; c_dd.fill = fill_loss
        
        ws_q.cell(row=row_num, column=17, value=t_row['Assigned Slot']).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=18, value=t_row.get('Re-entry Type', 'First Entry')).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=19, value=t_row.get('Quarterly Result Date', '')).alignment = Alignment(horizontal='center')
    ...
    # Native Excel LineChart
    chart_q = LineChart()
    chart_q.title = f"{qtr} — Point-in-Time Futures Cumulative P&L & Drawdown Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
# Save Workbook
print(f"Saving Point-in-Time Futures Master Workbook to {OUTPUT_EXCEL.name}...", flush=True)
wb.save(OUTPUT_EXCEL)

print(f"\n🎉 POINT-IN-TIME FUTURES MASTER WORKBOOK CREATED IN {time.time() - t0:.1f}s!", flush=True)
