...
OUTPUT_SL40_MASTER  = BASE_DIR / 'Nifty50_12_Quarters_Options_1PCT_ITM_40PCT_SL_Master.xlsx'
OUTPUT_SL40_SUMMARY = BASE_DIR / 'Nifty50_12_Quarters_Options_1PCT_ITM_40PCT_SL_Summary.xlsx'

# Executive Color Palette (Navy / Emerald Green Accent)
COLOR_TITLE_BG     = '065F46'  # Emerald Deep Title Banner
COLOR_SECTION_BG   = '047857'  # Emerald Section Header
COLOR_HEADER_BG    = '1E293B'  # Slate Navy Table Header
COLOR_SUBHEADER_BG = '10B981'  # Vibrant Emerald Subheader
COLOR_ZEBRA_BG     = 'F8FAFC'  # Subtle Light Zebra
...
print("Loading Source Data for -40% Stop Loss Model...", flush=True)
...
for qtr in quarters_order:
    ...
    for idx, row in q_tr.iterrows():
        ...
        # APPLY STRICT -40% PREMIUM STOP LOSS RULE
        if raw_ret < -0.40:
            effective_ret = -0.40
            effective_p_exit = p_entry * 0.60
            sl_triggered = 'YES (-40% SL Hit)'
        else:
        ...
        sl40_trades.append(trade_rec)

df_all_sl40_trades = pd.DataFrame(sl40_trades)

# ------------------- 2. Calculate Day-by-Day Peak Capital & Dual Drawdowns for -40% SL -------------------
sl40_summaries = []

for qtr in quarters_order:
    q_tr = df_all_sl40_trades[df_all_sl40_trades['Quarter'] == qtr].copy()
    q_tr['Entry Date DT'] = pd.to_datetime(q_tr['Entry Date'])
...
df_sl40_summary = pd.DataFrame(sl40_summaries)

# ------------------- 3. Build Master 13-Sheet Excel Workbook for -40% SL -------------------
print("Building Master 13-Sheet Workbook for -40% Stop Loss Model...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — STRICT -40% STOP-LOSS MODEL EXECUTIVE SCORECARD"
...
global_tot_pnl = round(df_all_sl40_trades['Booked Option P&L (₹)'].sum(), 2)
global_win_rate = len(df_all_sl40_trades[df_all_sl40_trades['Booked Option P&L (₹)'] > 0]) / len(df_all_sl40_trades)
total_sl_hits = len(df_all_sl40_trades[df_all_sl40_trades['Stop Loss Triggered'].str.contains('YES')])
...
c_h3 = ws_exec['F3']; c_h3.value = "TOTAL 12-QUARTER NET OPTION PROFIT (-40% SL)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
...
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (-40% STOP LOSS RULE APPLIED)"
...
headers_summary = ['Quarter', 'Peak Concurrent Capital (₹)', 'Peak Date (Open Trades)', 'Net Option Realised P&L (₹)', 'Return on Peak Capital (%)', 'Max Realised Drawdown (₹)', 'DD % on Initial Capital', 'True DD % on Peak Portfolio', 'SL Hits (-40%)', 'Win Rate (%)']
style_table_headers(ws_exec, 7, headers_summary, fill_header)
ws_exec.row_dimensions[7].height = 28

for r_idx, row_data in df_sl40_summary.iterrows():
    row_num = 8 + r_idx
    ...
    ws_exec.cell(row=row_num, column=9, value=int(row_data['SL Hits (-40%)'])).alignment = Alignment(horizontal='center')
...
ws_exec.cell(row=r_tot, column=6, value=-abs(df_sl40_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=6).font = font_loss; ws_exec.cell(row=r_tot, column=6).fill = fill_loss
ws_exec.cell(row=r_tot, column=7, value=df_sl40_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=8, value=df_sl40_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=2, value=df_sl40_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=4, value=df_sl40_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=4).font = font_win; ws_exec.cell(row=r_avg, column=4).fill = fill_win
ws_exec.cell(row=r_avg, column=5, value=df_sl40_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=6, value=-abs(df_sl40_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_loss; ws_exec.cell(row=r_avg, column=6).fill = fill_loss
ws_exec.cell(row=r_avg, column=7, value=df_sl40_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=8, value=df_sl40_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
# Chart
chart_opt = LineChart()
chart_opt.title = "Master 12-Quarter Net Option Buying P&L (-40% Stop Loss Model) (₹)"
chart_opt.style = 13
...
for qtr in quarters_order:
    ...
    q_sum = df_sl40_summary[df_sl40_summary['Quarter'] == qtr].iloc[0]
    ...
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 1% ITM OPTIONS BUYING MODEL (-40% STOP-LOSS) — {qtr.upper()}"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ws_q.row_dimensions[1].height = 40
    
    card_lbls = ['Peak Concurrent Capital (₹)', 'Peak Date (Open Trades)', 'Net Option P&L (₹)', 'Return on Peak Capital (%)', 'Win Rate (%)', 'Total Trades', 'SL Hits (-40%)', 'Max Drawdown (₹)', 'DD % Initial Capital', 'True DD % Peak Portfolio']
    ws_q.merge_cells('A2:J2')
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS (-40% STOP LOSS RULE APPLIED)"; c_s.font = font_section; c_s.fill = fill_section
    ...
    ws_q.cell(row=4, column=7, value=int(q_sum['SL Hits (-40%)'])).alignment = Alignment(horizontal='center')
    ...
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE OPTIONS JOURNAL (-40% STOP LOSS RULE APPLIED)"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        ws_q.cell(row=row_num, column=13, value=t_row['Effective Exit Premium (₹)']).number_format = '₹#,##0.00'
        ...
        c_sl = ws_q.cell(row=row_num, column=19, value=t_row['Stop Loss Triggered'])
        ...
        for c in range(1, 23):
            ...
            cell.border = border_thin
            if c not in [5, 16, 18, 19] and row_fill.fill_type: cell.fill = row_fill

    autofit_columns(ws_q, max_cols=22)
    ws_q.freeze_panes = 'A8'

    chart_q = LineChart()
    chart_q.title = f"{qtr} — 1% ITM Option Buying (-40% SL) Cumulative P&L Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
wb.save(OUTPUT_SL40_MASTER)

# ------------------- 4. Build Standalone -40% SL Summary Workbook -------------------
print("Writing Standalone -40% Stop Loss Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Options_40PCT_SL_Summary')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:J1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — STRICT -40% STOP-LOSS MODEL SUMMARY DASHBOARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
...
c_h3 = ws_s['F3']; c_h3.value = "TOTAL 12-QUARTER NET OPTION PROFIT (-40% SL)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
...
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (-40% STOP LOSS RULE APPLIED)"
...
for r_idx, row_data in df_sl40_summary.iterrows():
    ...
    ws_s.cell(row=row_num, column=9, value=int(row_data['SL Hits (-40%)'])).alignment = Alignment(horizontal='center')
...
ws_s.cell(row=r_tot, column=6, value=-abs(df_sl40_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=6).font = font_loss; ws_s.cell(row=r_tot, column=6).fill = fill_loss
ws_s.cell(row=r_tot, column=7, value=df_sl40_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=8, value=df_sl40_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=2, value=df_sl40_summary['Peak Concurrent Capital (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=4, value=df_sl40_summary['Net Option P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=4).font = font_win; ws_s.cell(row=r_avg, column=4).fill = fill_win
ws_s.cell(row=r_avg, column=5, value=df_sl40_summary['Booked Return (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=6, value=-abs(df_sl40_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_loss; ws_s.cell(row=r_avg, column=6).fill = fill_loss
ws_s.cell(row=r_avg, column=7, value=df_sl40_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=8, value=df_sl40_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
# Chart
chart = LineChart()
chart.title = "Consolidated 12-Quarter Net 1% ITM Option Buying (-40% SL Model) P&L Trend (₹)"
chart.style = 13
...
wb_s.save(OUTPUT_SL40_SUMMARY)
print(f"\n🎉 -40% STOP LOSS OPTIONS MASTER & SUMMARY CREATED IN {time.time() - t0:.1f}s!", flush=True)
