...
print("==========================================================================================")
print("BUILDING ALL STOCKS PORTFOLIO CAPITAL ALLOCATOR MASTER (FULL 211 STOCKS SIMULATION)")
print("==========================================================================================")
...
# FULL STOCKS UNIVERSE SIMULATION
all_symbols_with_data = [s for s, d in stock_quarterly_returns.items() if d.get("is_available") and len(d.get("q_map", {})) > 0]
print(f"Total Active Stocks in Full Universe Portfolio Simulation: {len(all_symbols_with_data)} Stocks")

all_q_keys = set()
for sym in all_symbols_with_data:
...
for q_k in sorted_q_keys:
    ...
    # Active stocks in this specific quarter
    active_stocks_in_q = [s for s in all_symbols_with_data if q_k in stock_quarterly_returns[s]["q_map"]]
    n_active = len(active_stocks_in_q)
    
    if n_active == 0:
        continue

    # Equal Capital Allocation across ALL active stocks in that quarter
    ...
    portfolio_sim_25l.append({
        ...
        "trades_summary": f"All {n_active} Active Stocks Traded Equal Weight"
    })
...
out_path_all_port = ROOT / "Nifty211_All_Stocks_Portfolio_Master.xlsx"
...
# SHEET 1: ALL STOCKS PORTFOLIO ALLOCATOR
ws_p = wb.active
ws_p.title = "ALL STOCKS PORTFOLIO (25L & 50L)"
ws_p.views.sheetView[0].showGridLines = True

ws_p.merge_cells("A1:N1")
ws_p["A1"] = "FULL 211 STOCKS PORTFOLIO CAPITAL ALLOCATOR SIMULATION (EARNINGS ANNOUNCEMENTS)"
...
ws_p.cell(row=6, column=1, value="FULL 211 STOCKS PORTFOLIO EQUITY GROWTH TIMELINE (EARNINGS ANNOUNCEMENTS)").font = hdr_font
...
p_headers = [
    ...
    "Full Universe Execution Notes"
]

ws_p.row_dimensions[7].height = 28
c_i = 1
for h in p_headers:
    if h == "Full Universe Execution Notes":
...
saved_paths = []
target_list = [
    out_path_all_port, out_path_q_master, out_path_q_clean
]
...
print("==========================================================================================")
print(f"ALL STOCKS PORTFOLIO CAPITAL ALLOCATOR MASTER CREATED SUCCESSFULLY! ({len(saved_paths)} files saved)")
print("==========================================================================================")
