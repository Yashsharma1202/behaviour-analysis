...
print("==========================================================================================")
print("BUILDING PAST 12 QUARTERS COMBINED BEST FUTURES MASTER WORKBOOK")
print("==========================================================================================")
...
out_path = ROOT / "Nifty211_Past_12_Quarters_Combined_Best_Futures_Master.xlsx"
...
print("\nINDIVIDUAL QUARTER WIN RATES BREAKDOWN ACROSS ALL 12 QUARTERS:")
print("-" * 80)
for q_name, _ in quarters_list:
    st = q_stats[q_name]
...
print("-" * 80)
print(f"  12-QUARTER CUMULATIVE: All Stocks Win Rate = {cum_wr_all}% | Qualified Win Rate = {cum_wr_q}% | Total P&L = ₹{cum_all_pnl:,.2f}")

# Title
ws_sum.merge_cells("A1:Y1")
ws_sum["A1"] = "NIFTY 211 STOCKS - PAST 12 QUARTERS COMBINED BEST FUTURES STRATEGY SUMMARY"
...
cards = [
    ...
    ("12-QUARTER CUMULATIVE RETURN %", f"{total_fund_ret_pct}%", "G3:I4", "G3"),
    ("12-QUARTER NET REALISED P&L", f"₹{cum_all_pnl:,.2f}", "J3:L4", "J3"),
    ("12-QUARTER GLOBAL WIN RATE %", f"{cum_wr_all}%", "M3:O4", "M3")
]
...
ws_sum.cell(row=7, column=1, value="PAST 12 QUARTERS COMBINED BEST SIDE-BY-SIDE MATRIX (ALL VS QUALIFIED)").font = hdr_font
...
matrix_rows = [
    ("Average Predicted Return (%)",
     [f"{q_stats[q]['avg_pred_ret']}%" for q, _ in quarters_list for _ in (0, 1)] + ["2.50%", "2.50%"]),
      
    ("Average Actual Realised Return (%)",
     [f"{q_stats[q]['avg_act_ret']}%" for q, _ in quarters_list for _ in (0, 1)] +
     [f"{round((cum_all_pnl/cum_all_to)*100, 2)}%", f"{round((cum_q_pnl/cum_q_to)*100, 2)}%"]),

    ("Total Portfolio Trades Analyzed",
     [val for q, _ in quarters_list for val in (q_stats[q]["n_all"], q_stats[q]["n_q"])] + [cum_all_trades, cum_q_trades]),
    
    ("Winning Trades (Net P&L > 0)",
     [val for q, _ in quarters_list for val in (q_stats[q]["w_all"], q_stats[q]["w_q"])] + [cum_all_wins, cum_q_wins]),

    ("Losing Trades (Net P&L <= 0)",
     [val for q, _ in quarters_list for val in (q_stats[q]["l_all"], q_stats[q]["l_q"])] + [cum_all_losses, cum_q_losses]),

    ("Combined Win Rate (%)",
     [val for q, _ in quarters_list for val in (f"{q_stats[q]['wr_all']}%", f"{q_stats[q]['wr_q']}%")] + [f"{cum_wr_all}%", f"{cum_wr_q}%"]),

    ("Total Net Realised P&L (₹)",
     [val for q, _ in quarters_list for val in (f"₹{q_stats[q]['pnl_all']:,.2f}", f"₹{q_stats[q]['pnl_q']:,.2f}")] + [f"₹{cum_all_pnl:,.2f}", f"₹{cum_q_pnl:,.2f}"]),

    ("Total Combined Turnover (₹)",
     [val for q, _ in quarters_list for val in (f"₹{q_stats[q]['to_all']:,.2f}", f"₹{q_stats[q]['to_q']:,.2f}")] + [f"₹{cum_all_to:,.2f}", f"₹{cum_q_to:,.2f}"])
]
...
for q_name, _ in quarters_list:
    ...
    ws_q["A1"] = f"NIFTY 211 STOCKS - {q_name} COMBINED BEST FUTURES MASTER"
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_Past_12_Quarters_Combined_Best_Futures_Master.xlsx")
print("==========================================================================================")
print("PAST 12 QUARTERS MASTER WORKBOOK CREATED SUCCESSFULLY!")
print("==========================================================================================")
