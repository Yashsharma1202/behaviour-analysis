...
print("==========================================================================================")
print("FAST EMPIRICAL 211 STOCKS MASTER EXCEL GENERATOR WITH DYNAMIC WINDOWS")
print("==========================================================================================")
...
stock_windows = {r[0]: (r[1], r[2], r[3], r[4]) for r in results}
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    w_str, pre_days, post_days, win_rate = stock_windows.get(sym, ("T-8 to T+1", 8, 1, 60.0))
...
out_path = ROOT / 'Nifty211_Recent_Quarter_Combined_Best_Futures_Empirical_Master.xlsx'
...
ws_sum["A1"] = "NIFTY 211 STOCKS - Q2 2026-27 EMPIRICAL BEST FUTURES PERFORMANCE SUMMARY"
...
df_70 = df_trades[df_trades['win_rate'] >= 65.0].sort_values('win_rate', ascending=False)
ws_sum.cell(row=7, column=4, value="TOP HIGH WIN-RATE STOCKS (WIN RATE >= 65%)").font = hdr_font
ws_sum.cell(row=7, column=4).fill = hdr_fill
ws_sum.merge_cells("D7:H7")

headers_70 = ["Stock Symbol", "Best Strategy", "Position Taking Window", "Historical Win Rate %", "Net Realised P&L (₹)"]
...
for r_idx, r in enumerate(df_70.head(12).to_dict('records'), start=9):
    ...
    ws_sum.cell(row=r_idx, column=5, value=r['strategy']).border = border_thin
    ws_sum.cell(row=r_idx, column=6, value=r['window']).border = border_thin
    
    c_wr = ws_sum.cell(row=r_idx, column=7, value=f"{r['win_rate']}%")
    c_wr.border = border_thin; c_wr.alignment = Alignment(horizontal="right")
    
    c_pnl = ws_sum.cell(row=r_idx, column=8, value=r['net_pnl'])
    c_pnl.border = border_thin; c_pnl.number_format = "₹#,##0.00"
    if r['net_pnl'] > 0: c_pnl.fill = pos_fill

for col in ws_sum.columns:
    col_letter = get_column_letter(col[0].column)
    ws_sum.column_dimensions[col_letter].width = 24

# SHEET 2: MAIN COMBINED BEST TRADE SHEET (Q2 2026-27)
ws = wb.create_sheet(title="Q2 2026-27")
ws.views.sheetView[0].showGridLines = True

ws.merge_cells("A1:X1")
ws["A1"] = "NIFTY 211 STOCKS - RECENT QUARTER (Q2 2026-27) COMBINED BEST FUTURES MASTER"
...
headers = [
    "Sr. No.", "Stock Symbol", "Quarter", "Best Strategy Selected", "Position Taking Window",
...
for r_idx, r in enumerate(df_trades.to_dict('records'), start=4):
    ...
    ws.cell(row=r_idx, column=6, value=f"{r['win_rate']}%")
...
try:
    import shutil
    shutil.copy(out_path, ROOT / 'Nifty211_Recent_Quarter_Combined_Best_Futures_Master.xlsx')
except Exception:
    pass

print("==========================================================================================")
print(f"✅ SUCCESSFULLY SAVED FAST EMPIRICAL MASTER EXCEL: {out_path.name}")
print("==========================================================================================")
