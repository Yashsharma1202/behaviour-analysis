...
body{{background:#0f0505;color:#e6edf3;margin:0;padding:34px;
...
.sub{{color:#8b98ad;font-family:ui-monospace,Consolas,monospace;font-size:.85rem;
 margin-bottom:22px}}
table{{width:100%;border-collapse:collapse;background:#150909;border:1px solid #3f1010;
 border-radius:10px;overflow:hidden}}
td{{padding:11px 16px;border-bottom:1px solid #3f1010;vertical-align:top}}
tr:last-child td{{border-bottom:0}} tr:hover{{background:#2b0e0e}}
.k{{color:#8b98ad;width:44%;font-size:.9rem}}
.v{{color:#e6edf3;font-weight:600}}
a{{color:#ef4444;font-family:ui-monospace,Consolas,monospace;font-size:.82rem;
 text-decoration:none}}
a:hover{{text-decoration:underline}}
.btn{{display:inline-block;background:#ef4444;color:#ffffff;font-weight:700;
 padding:8px 15px;border-radius:8px;margin-right:10px}}
.btn:hover{{background:#ff6b6b;text-decoration:none}}
.ghost{{display:inline-block;background:#150909;border:1px solid #3f1010;color:#e6edf3;
 padding:8px 15px;border-radius:8px}}
.ghost:hover{{background:#2b0e0e;text-decoration:none}}
.foot{{margin-top:18px}}
details{{margin-top:20px;background:#150909;border:1px solid #3f1010;border-radius:10px}}
...
.eyebrow{{color:#ef4444;font-family:ui-monospace,Consolas,monospace;font-size:.72rem;
 letter-spacing:.14em;text-transform:uppercase;margin-bottom:10px}}
h1{{margin:0 0 6px;font-size:1.5rem;font-weight:700}}
.sub{{color:#8b98ad;font-family:ui-monospace,Consolas,monospace;font-size:.85rem}}
.tag{{display:inline-block;margin-top:10px;background:#150909;border:1px solid #3f1010;
...
.plabel{{color:#ef4444;font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;
 margin-bottom:6px}}
...
def _avg_path(px, dates, N, M) -> list:
    ...
    for ev in dates:
        ...
        rows.append([(1 - float(v) / base) * 100 for v in w])
    if not rows:
        return []
...
def _all_event_windows(px, dates, before=6, after=3) -> list:
    ...
    for d in sorted(dates):
        ...
        out.append({
            ...
            "before_pct": round((1 - pe / p0) * 100, 2),
            "after_pct": round((1 - pa / pe) * 100, 2),
            "total_pct": round((1 - pa / p0) * 100, 2),
            "days": list(range(-before, after + 1)),
            "prices": [round(float(v), 2) for v in w.values],
        })
...
def analyse_event_short(px, event_date, symbol, event_type) -> list:
    ...
    for db in range(1, N_BEFORE + 1):
        ...
        for da in range(1, N_AFTER + 1):
            ...
            ret_pct = ((entry - exit_px) / entry - COST) * 100
            rows.append({
                "symbol": symbol,
                ...
                "return_rs": round(entry - exit_px, 2),
                "win_loss": "WIN" if ret_pct > 0 else ("LOSS" if ret_pct < 0 else "FLAT"),
            })
    return rows

def aggregate_short(detail: pd.DataFrame) -> pd.DataFrame:
...
def build_behaviour(sym: str) -> dict:
    ...
    for _, e in events.iterrows():
        rows += analyse_event_short(px, e["event_date"], sym, e["event_type"])
    if not rows:
        return {"symbol": sym, "available": False, "reason": "events outside price range"}

    detail = pd.DataFrame(rows)
    agg = aggregate_short(detail)
...
<title>NSE Short Stock Browser</title>
<style>
  :root{
  ...
  *{box-sizing:border-box}
  ...
  /* Detail Modal */
  .dm-overlay{position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;
    background:rgba(0,0,0,0.78);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
    display:flex;align-items:center;justify-content:center;padding:16px}
  .dm-box{background:#151c2a;border:1px solid var(--grid);border-radius:14px;
    width:100%;max-width:760px;max-height:88vh;display:flex;flex-direction:column;
    box-shadow:0 24px 60px rgba(0,0,0,0.7);overflow:hidden;animation:dmPop .18s ease-out}
  @keyframes dmPop{from{transform:scale(.95);opacity:0}to{transform:scale(1);opacity:1}}
  .dm-hdr{display:flex;justify-content:space-between;align-items:center;
    padding:14px 20px;background:#1b2436;border-bottom:1px solid var(--grid)}
  .dm-hdr-left{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
  .dm-badge{background:var(--blue);color:#0b1220;font-weight:700;font-family:var(--mono);
    font-size:.82rem;padding:3px 10px;border-radius:6px}
  .dm-feed-tag{color:var(--amber);font-family:var(--mono);font-size:.82rem;font-weight:600}
  .dm-date-tag{color:var(--mute);font-family:var(--mono);font-size:.82rem}
  .dm-close{background:transparent;border:0;color:var(--mute);font-size:1.2rem;
    cursor:pointer;padding:2px 8px;border-radius:6px}
  .dm-close:hover{color:var(--ink);background:#243044}
  .dm-body{padding:20px;overflow-y:auto;line-height:1.5}
  .dm-title{margin:0 0 16px;font-size:1.25rem;font-weight:700;color:var(--ink)}
  .dm-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;margin-bottom:16px}
  .dm-kv{background:#0e1420;border:1px solid var(--grid);padding:8px 12px;border-radius:8px}
  .dm-k{display:block;font-size:.7rem;color:var(--mute);font-family:var(--mono);text-transform:uppercase}
  .dm-v{display:block;font-size:.9rem;font-weight:600;color:var(--ink);margin-top:2px}
  .dm-desc-wrap{margin-bottom:18px}
  .dm-lbl{font-size:.75rem;font-family:var(--mono);color:var(--mute);text-transform:uppercase;margin-bottom:6px;letter-spacing:.03em}
  .dm-desc{background:#0e1420;border:1px solid var(--grid);padding:14px 16px;border-radius:10px;
    color:var(--ink);font-size:.92rem;white-space:pre-wrap;word-break:break-word}
  .dm-file-sec{margin-top:16px}
  .dm-file-box{background:#0e1420;border:1px solid #303d55;border-radius:10px;padding:14px}
  .dm-file-hdr{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;font-weight:600;font-size:.9rem}
  .dm-btn{display:inline-block;background:var(--blue);color:#0b1220;font-weight:700;
    padding:8px 16px;border-radius:8px;text-decoration:none;font-size:.88rem}
  .dm-btn:hover{background:#7cb8ff;text-decoration:none}
  .dm-btn-sm{display:inline-block;background:#243044;border:1px solid var(--grid);color:var(--ink);
    font-weight:600;padding:5px 12px;border-radius:6px;text-decoration:none;font-size:.78rem;font-family:var(--mono)}
  .dm-btn-sm:hover{background:#303e58;text-decoration:none}
  .dm-iframe{width:100%;height:320px;border:1px solid var(--grid);border-radius:8px;background:#0f131b;margin-top:8px}
  .dm-note{background:rgba(210,153,34,0.12);border:1px solid rgba(210,153,34,0.3);color:var(--amber);
    padding:10px 12px;border-radius:8px;font-size:.82rem;line-height:1.45;margin-top:8px}
  .ev-row{cursor:pointer;transition:background .1s}
  .ev-row:hover{background:#1e2738 !important}
  .filelink{color:var(--blue);font-weight:600;text-decoration:none;font-family:var(--mono);font-size:.82rem}
  .filelink:hover{text-decoration:underline}
</style>
</head>
<body>
  <div class="bar">
    <h1>NSE Short Stock Browser</h1>
  ...
  <main>
    ...
    <!-- Detail Modal for XML/PDF/Event Filings -->
    <div id="detailModal" class="dm-overlay" style="display:none" onclick="if(event.target===this)closeDetailModal()">
      <div class="dm-box">
        <div class="dm-hdr">
          <div class="dm-hdr-left">
            <span class="dm-badge" id="dm-sym">SYM</span>
            <span class="dm-feed-tag" id="dm-feed">Feed</span>
            <span class="dm-date-tag" id="dm-date">Date</span>
          </div>
          <button class="dm-close" onclick="closeDetailModal()">✕</button>
        </div>
        <div class="dm-body">
          <h2 class="dm-title" id="dm-title">Filing Details</h2>
          <div id="dm-meta"></div>
          <div id="dm-desc-wrap" class="dm-desc-wrap" style="display:none">
            <div class="dm-lbl">Full Announcement / Intimation Details</div>
            <div class="dm-desc" id="dm-desc"></div>
          </div>
          <div id="dm-file-sec" class="dm-file-sec" style="display:none"></div>
        </div>
      </div>
    </div>
...
<script>
const COL={ink:"#e6edf3",mute:"#8b98ad",grid:"#3f1010",panel:"#150909",
           blue:"#ef4444",amber:"#d29922",pos:"#3fb950",neg:"#f85149"};
...
function drawLineBase(cv,labels,seriesList){
  const {ctx,w,h}=setup(cv); ctx.clearRect(0,0,w,h);
  const all=seriesList.flatMap(s=>s.data);
  const b=niceBounds(all,false); if(!b||!labels.length){empty(ctx,w,h,"no annual P&L");return null;}
  ...
  return {ctx,w,h,X,Y,n,base};
}
function lineChart(cv,labels,seriesList){
  drawLineBase(cv,labels,seriesList);
  cv._model={kind:"line",labels,seriesList,draw:()=>drawLineBase(cv,labels,seriesList)};
  bindHover(cv);
}
...
function bindHover(cv){
  ...
  cv.onmousemove=function(e){
    ...
    const g=m.draw(); if(!g)return;
    const r=cv.getBoundingClientRect();
    const mx=e.clientX-r.left;               // CSS px; ctx is dpr-scaled so this maps directly
    if(m.kind==="line") hoverLine(g,m,mx); else hoverBar(g,m,mx);
  };
  cv.onmouseleave=function(){ const m=cv._model; if(m&&m.draw)m.draw(); };
}
function hoverLine(g,m,mx){
  const ctx=g.ctx, n=g.n; if(n<1)return;
  let idx=n<2?0:Math.round((mx-M.l)/((g.w-M.l-M.r)/(n-1)));
  idx=Math.max(0,Math.min(n-1,idx));
  const x=g.X(idx);
  ctx.save(); ctx.strokeStyle="rgba(139,152,173,.55)"; ctx.setLineDash([4,4]); ctx.lineWidth=1;
  ctx.beginPath(); ctx.moveTo(x,M.t); ctx.lineTo(x,g.base); ctx.stroke(); ctx.setLineDash([]); ctx.restore();
  const lines=[];
  m.seriesList.forEach(s=>{ const v=s.data[idx]; if(v==null||isNaN(v))return;
    const y=g.Y(v);
    ctx.fillStyle=COL.panel; ctx.beginPath();ctx.arc(x,y,6,0,7);ctx.fill();
    ctx.strokeStyle=s.color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,5,0,7);ctx.stroke();
    ctx.fillStyle=s.color;ctx.beginPath();ctx.arc(x,y,2.5,0,7);ctx.fill();
    lines.push([s.color,s.name,"₹"+Math.round(v).toLocaleString("en-IN")+" cr"]);
  });
  chartTip(g,x,m.labels[idx],lines);
}
function hoverBar(g,m,mx){
  const ctx=g.ctx, n=m.vals.length; if(!n)return;
...
function chartTip(g,x,title,lines){
  ...
  let tw=title?ctx.measureText(title).width:0;
  lines.forEach(l=>{ tw=Math.max(tw,16+ctx.measureText(l[1]+": "+l[2]).width); });
  const pad=9,lh=17, boxW=tw+pad*2, boxH=pad*2+lh*(lines.length+(title?1:0));
  let bx=x+14; if(bx+boxW>g.w-4) bx=x-14-boxW; if(bx<4)bx=4;
  const by=M.t+6;
  ctx.fillStyle="rgba(18,24,36,.97)"; ctx.strokeStyle="#2a3346"; ctx.lineWidth=1;
  rrect(ctx,bx,by,boxW,boxH,9); ctx.fill(); ctx.stroke();
  let yy=by+pad;
  if(title){ ctx.fillStyle=COL.mute; ctx.fillText(title,bx+pad,yy); yy+=lh; }
  lines.forEach(l=>{
    ctx.fillStyle=l[0]; ctx.beginPath(); ctx.arc(bx+pad+4,yy+6,4,0,7); ctx.fill();
    ctx.fillStyle=COL.ink; ctx.fillText(l[1]+": "+l[2], bx+pad+15, yy); yy+=lh;
  });
}
...
async function renderEvents(feed){
  ...
  F.rows.forEach((r, rowIdx)=>{
    html+='<tr class="ev-row" onclick="openDetailModal(\''+esc(SYMS[idx])+'\', \''+esc(feed)+'\', '+rowIdx+')">'+F.columns.map(c=>{
      const v=r[c.key]||'';
      if(c.type==='link'){
        if(!v.startsWith('http') && !v.startsWith('./')) return `<td>${esc(v)}</td>`;
        if(c.key==='xbrl'){        // Download Excel -> built from the LOCAL folders
          if(STATIC) return `<td><a href="javascript:void(0)" onclick="event.stopPropagation();openDetailModal('${esc(SYMS[idx])}','${esc(feed)}',${rowIdx})" class="filelink" title="${esc(v)}">📄 Detail ↗</a></td>`;
          const u="/api/statements_excel?sym="+encodeURIComponent(SYMS[idx]);
          return `<td><a href="${u}" onclick="event.stopPropagation()" title="Download financial statements from local data">📊 Excel ↓</a></td>`;
        }
        const isPdf=v.toLowerCase().endsWith(".pdf");
        const isBm=v.startsWith("./bm/");
        let label = isBm ? "📑 Summary ↗" : (isPdf ? "📄 PDF ↗" : "🔍 Detail ↗");
        return `<td><a href="javascript:void(0)" onclick="event.stopPropagation();openDetailModal('${esc(SYMS[idx])}','${esc(feed)}',${rowIdx})" class="filelink" title="Click to view detailed summary">${label}</a></td>`;
      }
  ...
  if(F.total>F.shown) html+=`<div class="enote">Showing latest ${F.shown} of ${F.total.toLocaleString()} rows. (Click any row for details)</div>`;
  else html+=`<div class="enote">${F.total.toLocaleString()} row(s). (Click any row for details)</div>`;
  ev.innerHTML=html;
}

function openDetailModal(sym, feedKey, rowIdx){
  const pay=eventsCache[sym];
  if(!pay || !pay.feeds || !pay.feeds[feedKey]) return;
  const F=pay.feeds[feedKey];
  const r=F.rows[rowIdx];
  if(!r) return;

  const dateStr = r.sort_date || r.bm_date || r.broadCastDate || r.exDate || "";
  const title = r.desc || r.bm_purpose || r.subject || r.relatingTo || F.label || "Filing Details";
  const descText = r.attchmntText || r.bm_desc || r.subject || "";
  let fileUrl = "";
  F.columns.forEach(c => { if(c.type === 'link' && r[c.key]) fileUrl = r[c.key]; });

  $("dm-sym").textContent = sym;
  $("dm-feed").textContent = F.label;
  $("dm-date").textContent = dateStr ? ("📅 " + dateStr) : "";
  $("dm-title").textContent = title;

  let metaHtml = '<div class="dm-grid">';
  F.columns.forEach(c => {
    if(c.type !== 'link' && r[c.key]){
      metaHtml += `<div class="dm-kv"><span class="dm-k">${esc(c.label)}</span><span class="dm-v">${esc(r[c.key])}</span></div>`;
    }
  });
  metaHtml += '</div>';
  $("dm-meta").innerHTML = metaHtml;

  if(descText && descText !== title){
    $("dm-desc-wrap").style.display = "block";
    $("dm-desc").textContent = descText;
  } else {
    $("dm-desc-wrap").style.display = "none";
  }

  const fileSec = $("dm-file-sec");
  if(fileUrl && (fileUrl.startsWith("http") || fileUrl.startsWith("./bm/"))){
    fileSec.style.display = "block";
    const isBaked = fileUrl.startsWith("./bm/") || fileUrl.includes("/bm/");
    const isPdf = fileUrl.toLowerCase().endsWith(".pdf");
    const isXml = fileUrl.toLowerCase().endsWith(".xml");

    let fhtml = '';
    if(isBaked){
      fhtml = `<div class="dm-file-box">`+
        `<div class="dm-file-hdr"><span>📑 Parsed XBRL Filing Summary</span>`+
        `<a href="${esc(fileUrl)}" target="_blank" class="dm-btn-sm">Open Full Page ↗</a></div>`+
        `<iframe src="${esc(fileUrl)}" class="dm-iframe"></iframe></div>`;
    } else if(isXml){
      const rawUrl = STATIC ? fileUrl : ("/api/file?url=" + encodeURIComponent(fileUrl));
      fhtml = `<div class="dm-file-box">`+
        `<div class="dm-file-hdr"><span>📄 Source XBRL File (.xml)</span>`+
        `<a href="${esc(rawUrl)}" target="_blank" class="dm-btn-sm">Open Source XML ↗</a></div>`+
        `<div class="dm-note">💡 <b>Note on XBRL files:</b> Stock exchange XML files are technical filings. `+
        `The full announcement text &amp; meeting purpose extracted from the filing are shown in the summary above.</div></div>`;
    } else if(isPdf){
      const pdfUrl = STATIC ? fileUrl : ("/api/file?url=" + encodeURIComponent(fileUrl));
      fhtml = `<div class="dm-file-box">`+
        `<div class="dm-file-hdr"><span>📄 Official PDF Attachment</span>`+
        `<a href="${esc(pdfUrl)}" target="_blank" class="dm-btn">Open PDF Document ↗</a></div></div>`;
    } else {
      fhtml = `<div class="dm-file-box">`+
        `<a href="${esc(fileUrl)}" target="_blank" class="dm-btn">Open Attachment ↗</a></div>`;
    }
    fileSec.innerHTML = fhtml;
  } else {
...
async function renderBehaviour(){
  ...
  behaviourData=d;
  let html='<div class="bintro">The best time to SHORT <b>'+esc(sym)+'</b> around each kind of event — '+
    'based on what actually happened in the past. <b>Click any card</b> for its graph &amp; full summary. '+
    '<span class="enote">(1 candle = 1 trading day.)</span></div><div class="bcards">';
  for(const et of keys){
    ...
    html+=`<div class="bcard" onclick="openBeh('${et}')">
      ...
      <div class="brec">📉 <b>Sell (Short) ${dbf} candle${dbf>1?'s':''} before</b> the event, then <b>buy (cover) ${daf} candle${daf>1?'s':''} after</b>.</div>
      <div class="bstats">
        <div class="bstat">
...
function openRecent(et){
  ...
  const m1=(1-pe/p0)*100, m2=(1-pa/pe)*100, m3=(1-pa/p0)*100;
  const c=v=>v>=0?COL.pos:COL.neg, s=v=>(v>=0?'+':'')+v.toFixed(2)+'%';
  const rows=L.days.map((dd,i)=>{
    const pct=(1-L.prices[i]/pe)*100;
    const lab=dd===0?'EVENT CANDLE':(dd>0?'T+'+dd:'T'+dd);
  ...
  bv.innerHTML=`
    ...
    <div class="bverdict v-${m3>=0?'strong':'no'}" style="display:inline-block">
      ${outcome}: shorting 6 candles before and covering 3 candles after would have returned ${s(m3)}</div>
    <div class="statrow" style="grid-template-columns:repeat(3,1fr);margin-top:14px">
      <div class="sbox"><div class="l">Price 6 candles before</div><div class="v" style="color:${COL.amber}">₹${rup(p0)}</div></div>
      <div class="sbox"><div class="l">Price on event candle</div><div class="v">₹${rup(pe)}</div></div>
      <div class="sbox"><div class="l">Price 3 candles after</div><div class="v" style="color:${COL.pos}">₹${rup(pa)}</div></div>
    </div>
    ...
    <div class="bhead" style="margin-top:18px">Candle-by-candle prices</div>
    <div class="tblwrap"><table><thead><tr><th>Candle</th>
      <th style="text-align:right">Price</th><th style="text-align:right">vs event candle</th>
...
function openHistory(et){
  ...
  bv.innerHTML=`
    ...
    <div class="enote">Each row: short 6 candles before the event, cover 3 candles after — the same
      window as the “recent event report”, shown for every past event (newest first). Click a row for its price chart. (1 candle = 1 trading day.)</div>
    <div id="histchart"></div>
    <div class="tblwrap" style="margin-top:12px"><table><thead><tr>
      <th>Event date</th><th style="text-align:right">Short (T-6)</th>
      <th style="text-align:right">Event candle</th><th style="text-align:right">Cover (T+3)</th>
...
function openEventGraph(i){
  ...
  const P=v=>((1-v/px[ei])*100), sp=v=>(v>=0?'+':'')+v.toFixed(2)+'%';
  ...
  function move(ev){
    ...
    const d=days[k], lab=d===0?'EVENT CANDLE':('T'+(d>0?'+':'')+d);
    tip.style.opacity=1;
...
function openBeh(et){
  ...
  bv.innerHTML=`
    ...
    <div class="brec" style="margin-top:12px">📉 Best rule: <b>Sell (Short) ${b.days_before} candle${b.days_before>1?'s':''} before</b> the event, then <b>Buy (Cover) ${b.days_after} candle${b.days_after>1?'s':''} after</b>.</div>
    <div class="chart" style="margin-top:8px">
      <span class="cs">🔴 SHORT/SELL · 🟢 COVER/BUY · move cursor 👆</span>
    ...
    <div class="enote">The line shows how the stock moved on average, lined up so candle 0 is the event.
      A rise before candle 0 = it drifts up INTO the event; drift after 0 = it keeps re-pricing on the news.</div>
    ${t.last?`
    ...
    <div class="statrow" style="grid-template-columns:repeat(3,1fr)">
      <div class="sbox"><div class="l">🔴 Short Entry (T−6)</div><div class="v" style="color:${COL.neg}">₹${rup(t.last.prices[0])}</div></div>
      <div class="sbox"><div class="l">On Event Candle (T0)</div><div class="v">₹${rup(t.last.prices[t.last.days.indexOf(0)])}</div></div>
      <div class="sbox"><div class="l">🟢 Cover Exit (T+3)</div><div class="v" style="color:${COL.pos}">₹${rup(t.last.prices[t.last.prices.length-1])}</div></div>
    </div>`:''}
    <details class="bdetails"><summary>▸ Show the full candle-by-candle grid (advanced)</summary>
...
function avgMoveChart(cv,days,vals,best){
  ...
  function draw(hi){
    ...
    if(hi!=null){const x=X(hi),y=Y(vals[hi]);
      ...
      const dd=days[hi], lab=dd===0?"EVENT CANDLE":(dd>0?"T+"+dd:"T"+dd);
...
function verdict(win,avg){
  if(avg<=0)return['no','✕ Avoid — shorting this historically ended in a loss'];
  if(win>=60&&avg>=0.5)return['strong','✓ Strong short pattern — price fell after event'];
  if(win>=55)return['ok','◐ Some short edge — decent, not a sure thing'];
  return['weak','⚠ Weak short signal — barely better than a coin flip'];
}
...
function heatmap(grid,best,NB,NA){
  ...
  let h='<div class="bwrap"><table class="hm"><thead><tr><th>short ↓ / cover →</th>';
...
function drawUpcoming(){
  ...
  const body=rows.map(r=>{
    ...
    return `<tr class="rrow" onclick="goSym('${r.symbol}')" title="open ${esc(r.symbol)}">
      ...
      <td class="rnum">${r.db!=null?`buy ${r.db}c / sell ${r.da}c`:'—'}</td>
  ...
  uv.innerHTML=`
    ...
    <div class="tblwrap" style="margin-top:12px"><table>
      <thead><tr><th>Date</th><th>Stock</th><th>Event</th><th style="text-align:right">Best rule (candles)</th>
...
function drawRankings(){
  ...
  const rows=list.map((r,i)=>{
    ...
    return `<tr class="rrow" onclick="goSym('${r.symbol}')" title="open ${esc(r.symbol)}">
      ...
      <td class="rnum">buy ${r.db}c / sell ${r.da}c</td>
  ...
  rv.innerHTML=`
    ...
    <div class="tblwrap" style="margin-top:12px"><table>
      ...
      <th style="text-align:right">Avg gain</th><th style="text-align:right">Best rule (candles)</th>
      <th style="text-align:right">Events</th></tr></thead>
...
async function drawCompare(){
  ...
  const brows=EVT.map(([et,lab])=>`<tr><td class="cmplab">${esc(lab)} — rule · win</td>`+
    cmpSel.map(s=>{ const t=(cmpBeh[s]&&cmpBeh[s].types||{})[et];
      ...
      return `<td class="rnum"><span class="mono">buy${b.days_before}c/sell${b.days_after}c</span><br><b style="color:${wc}">${b.win_rate_pct}%</b> · ${b.avg_return_pct>=0?'+':''}${b.avg_return_pct}%</td>`;
    }).join('')+'</tr>').join('');
...
def main():
    ...
    print("=" * 62)
    print(f"  NSE Short Stock Browser  —  serving {len(SYMBOLS):,} stocks")
