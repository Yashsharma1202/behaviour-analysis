...
OUTPUT_DETAILED_MASTER = BASE_DIR / 'Nifty50_12_Quarters_Options_1PCT_ITM_Detailed_Master.xlsx'
OUTPUT_PIT_MASTER = BASE_DIR / 'Nifty50_12_Quarters_Options_1PCT_ITM_Point_In_Time_Master.xlsx'

# Premium Executive Color Palette
COLOR_NAVY_DARK    = '0F172A'  # Deep Navy Title Banner
COLOR_NAVY_HEADER  = '1E293B'  # Slate Navy Table Headers
COLOR_BLUE_SECTION = '1E3A8A'  # Royal Blue Section Dividers
COLOR_BLUE_SUB     = '3B82F6'  # Accent Blue Card Headers
COLOR_ZEBRA_BG     = 'F8FAFC'  # Subtle Light Gray Zebra
...
COLOR_WHITE        = 'FFFFFF'
COLOR_TEXT_MAIN    = '334155'  # Dark Slate Main Text
...
fill_section   = PatternFill(start_color=COLOR_BLUE_SECTION, end_color=COLOR_BLUE_SECTION, fill_type='solid')
fill_header    = PatternFill(start_color=COLOR_NAVY_HEADER, end_color=COLOR_NAVY_HEADER, fill_type='solid')
fill_subheader = PatternFill(start_color=COLOR_BLUE_SUB, end_color=COLOR_BLUE_SUB, fill_type='solid')
...
print("Loading Source Data from Point-in-Time Futures Master...", flush=True)
...
# ------------------- 3. Build Ultra-Attractive 13-Sheet Excel Workbook -------------------
print("Building Ultra-Attractive 13-Sheet Master Options Excel Workbook...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — MASTER 12-QUARTER PERFORMANCE SCORECARD"
...
c_v1 = ws_exec['A4']; c_v1.value = len(df_all_pit_opt_trades); c_v1.font = Font(name='Segoe UI', size=16, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.alignment = Alignment(horizontal='center', vertical='center')
...
for qtr in quarters_order:
    ...
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS & CAPITAL OVERVIEW"; c_s.font = font_section; c_s.fill = fill_section
    ...
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE OPTIONS JOURNAL (ENTRY DATE CHRONOLOGICAL ORDER)"; c_s2.font = font_section; c_s2.fill = fill_section
...
# Save Workbooks
wb.save(OUTPUT_DETAILED_MASTER)
wb.save(OUTPUT_PIT_MASTER)

print(f"\n🎉 ULTRA-ATTRACTIVE DETAILED MASTER EXCEL CREATED IN {time.time() - t0:.1f}s!", flush=True)
