...
# ------------------- 4. Load Regularized Trades -------------------
print("Loading Regularized Master Trades...", flush=True)
try:
    df_reg_trades = pd.read_excel(REGULARIZED_PATH, sheet_name='All_12_Quarters_Master_Trades', engine='openpyxl')
except Exception as e:
    print(f"Error reading regularized trades: {e}", flush=True)
    sys.exit(1)

processed_trades = []

for idx, row in df_reg_trades.iterrows():
...
print(f"Processed {len(df_all_trades)} Regularized Futures trades!", flush=True)
...
cell_title.value = "NIFTY 50 REGULARIZED FUTURES 12-QUARTERS EXECUTIVE DASHBOARD & FUND COMPOUNDING"
...
qtr_card_labels = [
    'Beginning Capital (₹)', 'Ending Capital (₹)', 'Net Booked Futures P&L (₹)',
...
trade_table_headers = [
    ...
    'Slot ID', 'Re-entry Type', 'Quarterly Result Date'
]
...
for qtr in quarters_order:
    ...
    # Row 4: Card Values
    ws_q.cell(row=4, column=1, value=q_summary['Beginning Capital (₹)']).number_format = '₹#,##0.00'
    
    c_end = ws_q.cell(row=4, column=2, value=q_summary['Ending Capital (₹)'])
    ...
    ws_q.cell(row=4, column=4, value=q_summary['Quarter Return (%)']).number_format = '0.00%'
...
print(f"\n🎉 PERFECT REGULARIZED FUTURES MASTER CREATED IN {time.time() - t0:.1f}s!", flush=True)
