...
OUTPUT_EXCEL = BASE_DIR / 'Nifty50_All_12_Quarters_And_YearWise_Best_Stock_Rankings.xlsx'
...
# Helper to compute stock rankings for a subset of trades
def compute_stock_ranking(df_eq_sub, df_fut_sub):
...
print("Building All 12-Quarter & Year-Wise Stock Rankings Excel...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
headers_detail = ['Rank', 'Symbol', 'Company Name', 'Sector', 'Trades', 'Equity P&L (₹)', 'Equity Avg Return (%)', 'Equity Win Rate (%)', 'Futures Net P&L (₹)', 'Futures Return on Margin (%)', 'Futures Win Rate (%)']

# --- SHEET 1: Master Executive Leaderboard ---
ws_exec = wb.create_sheet(title='Master_12Q_Best_Stock_Summary')
ws_exec.merge_cells('A1:K1')
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 STRATEGY — ALL 12-QUARTER MASTER STOCK LEADERBOARD"
...
df_global_rank = compute_stock_ranking(df_eq, df_fut)
style_table_headers(ws_exec, 3, headers_detail, fill_header)
ws_exec.row_dimensions[3].height = 26

for r_idx, r_data in df_global_rank.iterrows():
    row_num = 4 + r_idx
    row_fill = fill_zebra if r_idx % 2 == 1 else PatternFill(fill_type=None)
    ws_exec.row_dimensions[row_num].height = 21
    
    ws_exec.cell(row=row_num, column=1, value=r_idx + 1).alignment = Alignment(horizontal='center')
    ws_exec.cell(row=row_num, column=2, value=r_data['Symbol']).alignment = Alignment(horizontal='center')
    ws_exec.cell(row=row_num, column=3, value=r_data['Company Name'])
    ws_exec.cell(row=row_num, column=4, value=r_data['Sector'])
    ws_exec.cell(row=row_num, column=5, value=int(r_data['Eq_Trades'])).alignment = Alignment(horizontal='center')
    
    c_eqp = ws_exec.cell(row=row_num, column=6, value=r_data['Eq_Total_PnL'])
    c_eqp.number_format = '₹#,##0.00'; c_eqp.font = font_win if r_data['Eq_Total_PnL'] >= 0 else font_loss
    
    c_eqr = ws_exec.cell(row=row_num, column=7, value=r_data['Eq_Avg_Return'])
    c_eqr.number_format = '0.00%'
    
    c_eqw = ws_exec.cell(row=row_num, column=8, value=r_data['Eq_Win_Rate'])
    c_eqw.number_format = '0.00%'; c_eqw.alignment = Alignment(horizontal='center')
    
    c_futp = ws_exec.cell(row=row_num, column=9, value=r_data['Fut_Total_PnL'])
    ...
    c_futr = ws_exec.cell(row=row_num, column=10, value=r_data['Fut_Avg_Return'])
    c_futr.number_format = '+0.00%'
    
    c_futw = ws_exec.cell(row=row_num, column=11, value=r_data['Fut_Win_Rate'])
...
# --- SHEETS 2-13: ALL 12 SEPARATE INDIVIDUAL QUARTER SHEETS ---
for qtr in quarters_order:
    ws_q = wb.create_sheet(title=qtr)
    
    df_eq_q  = df_eq[df_eq['Quarter'] == qtr]
    df_fut_q = df_fut[df_fut['Quarter'] == qtr]
    
    df_q_rank = compute_stock_ranking(df_eq_q, df_fut_q)
    
    ws_q.merge_cells('A1:K1')
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 STOCKS PERFORMANCE RANKING — {qtr.upper()} (EQUITY vs FUTURES)"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ws_q.row_dimensions[1].height = 36
    
    style_table_headers(ws_q, 3, headers_detail, fill_header)
    ws_q.row_dimensions[3].height = 26
    
    for r_idx, r_data in df_q_rank.iterrows():
        row_num = 4 + r_idx
        row_fill = fill_zebra if r_idx % 2 == 1 else PatternFill(fill_type=None)
        ws_q.row_dimensions[row_num].height = 21
        
        ws_q.cell(row=row_num, column=1, value=r_idx + 1).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=2, value=r_data['Symbol']).alignment = Alignment(horizontal='center')
        ws_q.cell(row=row_num, column=3, value=r_data['Company Name'])
        ws_q.cell(row=row_num, column=4, value=r_data['Sector'])
        ws_q.cell(row=row_num, column=5, value=int(r_data['Eq_Trades'])).alignment = Alignment(horizontal='center')
        
        c_eqp = ws_q.cell(row=row_num, column=6, value=r_data['Eq_Total_PnL'])
        c_eqp.number_format = '₹#,##0.00'; c_eqp.font = font_win if r_data['Eq_Total_PnL'] >= 0 else font_loss
        
        c_eqr = ws_q.cell(row=row_num, column=7, value=r_data['Eq_Avg_Return'])
        c_eqr.number_format = '0.00%'
        
        c_eqw = ws_q.cell(row=row_num, column=8, value=r_data['Eq_Win_Rate'])
        c_eqw.number_format = '0.00%'; c_eqw.alignment = Alignment(horizontal='center')
        
        c_futp = ws_q.cell(row=row_num, column=9, value=r_data['Fut_Total_PnL'])
        c_futp.number_format = '₹#,##0.00'; c_futp.font = font_win if r_data['Fut_Total_PnL'] >= 0 else font_loss
        if r_data['Fut_Total_PnL'] >= 0: c_futp.fill = fill_win
        
        c_futr = ws_q.cell(row=row_num, column=10, value=r_data['Fut_Avg_Return'])
        c_futr.number_format = '+0.00%'
        
        c_futw = ws_q.cell(row=row_num, column=11, value=r_data['Fut_Win_Rate'])
        c_futw.number_format = '0.00%'; c_futw.alignment = Alignment(horizontal='center')
    ...
    autofit_columns(ws_q, max_cols=11)
    ws_q.freeze_panes = 'A4'

# --- SHEETS 14-17: 4 YEAR-WISE SEPARATE SHEETS (FY24, FY25, FY26, FY27) ---
fy_list = ['2023-24', '2024-25', '2025-26', '2026-27']
for fy in fy_list:
    sheet_title = f"FY_{fy}_All_Stocks_Ranking"
    ws_fy = wb.create_sheet(title=sheet_title)
    
    df_eq_fy  = df_eq[df_eq['Quarter'].str.contains(fy)]
    df_fut_fy = df_fut[df_fut['Quarter'].str.contains(fy)]
    
    df_fy_rank = compute_stock_ranking(df_eq_fy, df_fut_fy)
    
    ws_fy.merge_cells('A1:K1')
    c_t = ws_fy['A1']
    c_t.value = f"NIFTY 50 STOCKS PERFORMANCE RANKING — FINANCIAL YEAR {fy} (EQUITY vs FUTURES)"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ...
    for r_idx, r_data in df_fy_rank.iterrows():
        row_num = 4 + r_idx
        row_fill = fill_zebra if r_idx % 2 == 1 else PatternFill(fill_type=None)
...
print(f"\n🎉 ALL 12-QUARTER & YEAR-WISE STOCK RANKING WORKBOOK CREATED IN {time.time() - t0:.1f}s!", flush=True)
