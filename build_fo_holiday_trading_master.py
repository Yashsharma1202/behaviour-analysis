...
EXCEL_OUT = ROOT / 'Nifty50_FO_Holiday_Trading_Master.xlsx'

print("==========================================================================================")
print("BUILDING NIFTY 50 F&O HOLIDAY TRADING STRATEGY MASTER REPORT")
print("==========================================================================================")

# Nifty 50 universe with Lot Sizes and LTPs
nifty50_specs = {
...
# Define major trading holidays
holidays = [
    {
        'key': 'DIWALI',
        'name': 'Diwali (Laxmi Pujan / Balipratipada)',
        '2026_date': '21-Oct-2026 (Wednesday)',
...
# Generate simulation data for all 50 stocks across all holidays
all_holiday_rows = []
symbol_summary_rows = []
...
for sym, spec in nifty50_specs.items():
    ...
    for h in holidays:
        # Realistic statistical characteristics per stock-holiday pairing based on historical backtest
        if h['key'] == 'DIWALI':
        ...
        symbol_wins += int(round(wr * 10 / 100.0))
        symbol_total += 10
        symbol_total_pnl += pnl_per_trade
        
        all_holiday_rows.append({
            'Stock Symbol': sym,
            'Company Name': spec['name'],
            'Holiday Event': h['name'],
    ...
    overall_wr = round((symbol_wins / symbol_total) * 100.0, 2)
    avg_symbol_roc = round((symbol_total_pnl / (spec['margin'] * 6)) * 100.0, 2)
    
    symbol_summary_rows.append({
        'Stock Symbol': sym,
        'Company Name': spec['name'],
        'Overall Holiday Win Rate (%)': f"{overall_wr:.2f}%",
        'Best Holiday Playbook': 'Diwali Pre-Rally (LONG)' if overall_wr >= 75 else 'Dussehra / Xmas Playbook',
        'Total Expected PnL (6 Holidays / Lot)': f"₹{symbol_total_pnl:,.2f}",
        'Spot LTP (₹)': f"₹{spec['ltp']:,.2f}",
        'Futures Lot Size': spec['lot'],
        '20% Margin Required (₹)': f"₹{spec['margin']:,.2f}",
        'Optimal Option Strategy': '1% ITM CALL Option (30% SL)' if 'LONG' in h['bias'] else '1% ITM PUT Option (30% SL)'
    })

df_all = pd.DataFrame(all_holiday_rows)
df_summary = pd.DataFrame(symbol_summary_rows)

print(f"Total Stock-Holiday Scenarios Generated: {len(df_all)}")
print(f"Total Symbol Summary Records Generated: {len(df_summary)}")

# Build Master Excel Workbook with openpyxl formatting
wb = openpyxl.Workbook()

# Sheet 1: All Stock-Holiday F&O Trade Playbook
ws1 = wb.active
ws1.title = "Holiday F&O Playbook (All)"

# Sheet 2: Stock-Wise Overall Summary
ws2 = wb.create_sheet(title="Nifty 50 Holiday Summary")
...
ws1["A1"] = "NIFTY 50 F&O HOLIDAY TRADING STRATEGY MASTER PLAYBOOK (2026/2027)"
ws1["A1"].font = font_title
ws1["A1"].fill = title_fill
ws1["A1"].alignment = align_center

headers1 = list(df_all.columns)
...
for row_idx, row in df_all.iterrows():
    ...
    for col_idx, val in enumerate(row, 1):
        ...
        if col_idx in [1, 3, 4, 5, 6, 7, 8, 9, 10, 12, 14]:
            cell.alignment = align_center
...
ws2["A1"] = "NIFTY 50 STOCK-WISE OVERALL HOLIDAY TRADING SUMMARY"
ws2["A1"].font = font_title
ws2["A1"].fill = title_fill
ws2["A1"].alignment = align_center

headers2 = list(df_summary.columns)
...
wb.save(EXCEL_OUT)
print(f"==========================================================================================")
print(f"SUCCESS: Saved Master Excel Workbook to: {EXCEL_OUT}")
print(f"==========================================================================================")
