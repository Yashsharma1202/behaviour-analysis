...
OUT_SYN_LONG = ROOT / "Nifty50_12_Quarters_Synthetic_Long_Master.xlsx"
OUT_SYN_SHORT = ROOT / "Nifty50_12_Quarters_Synthetic_Short_Master.xlsx"

OUT_SYN_LONG_REP = REPORTS_DIR / "Nifty50_12_Quarters_Synthetic_Long_Master.xlsx"
OUT_SYN_SHORT_REP = REPORTS_DIR / "Nifty50_12_Quarters_Synthetic_Short_Master.xlsx"
...
def schedule_synthetic_slots(trades):
    ...
    total_margin = sum(max(tr["Synthetic Margin (₹)"] for tr in s["trades"]) for s in slots) if slots else 0.0
    total_profit = sum(t["Total Lot Booked P&L (₹)"] for t in trades)
...
def build_synthetic_master_workbook(mode, out_path, alt_path):
    """
    mode: "LONG" or "SHORT"
    """
    title_mode = "SYNTHETIC LONG (BUY ATM CE + SELL ATM PE)" if mode == "LONG" else "SYNTHETIC SHORT (BUY ATM PE + SELL ATM CE)"
    print(f"\n=======================================================")
    print(f"Building Synthetic Options Master Workbook: {title_mode}")
    ...
    for fn, q_name, period_desc, fy_tag in QUARTERS:
        ...
        for _, row in df_src.iterrows():
            ...
            if ex_dt <= curr_expiry:
                ...
                # Target exit extends past current monthly expiry -> Select Next Month Expiry contract
                ...
                expiry_type = "Next Month Expiry (Rollover / Multi-Month Window)"
                expiry_month_str = next_expiry.strftime("%b %Y").upper()
                
            atm_strike = get_atm_strike(p_en)
            
            # Synthetic P&L Calculation
            # SPAN + Exposure Margin in F&O = ~20% of contract value
            contract_value = p_en * lot_size
            syn_margin = contract_value * 0.20
            
            if mode == "LONG":
                # Buy ATM Call + Sell ATM Put (Delta = +1.0)
                pnl_per_share = p_ex - p_en
                underlying_ret = (p_ex - p_en) / p_en if p_en > 0 else 0.0
                instr_desc = f"{atm_strike} CE (Buy) + {atm_strike} PE (Sell)"
            else:
                # Buy ATM Put + Sell ATM Call (Delta = -1.0)
                pnl_per_share = p_en - p_ex
                underlying_ret = (p_en - p_ex) / p_en if p_en > 0 else 0.0
                instr_desc = f"{atm_strike} PE (Buy) + {atm_strike} CE (Sell)"
                
            total_lot_pnl = pnl_per_share * lot_size
            booked_ret_on_margin = (total_lot_pnl / syn_margin) if syn_margin > 0 else 0.0
            ...
            trades_for_q.append({
                ...
                "Synthetic Options Pair": instr_desc,
                ...
                "Lot Size": lot_size,
                "Synthetic Margin (₹)": syn_margin,
                "Expected Return (%)": exp_ret_val * 5.0, # Leveraged Option Expected Return
                "Underlying Drift (%)": underlying_ret,
                "P&L Per Share (₹)": pnl_per_share,
                "Total Lot Booked P&L (₹)": total_lot_pnl,
                "Booked Return on Margin (%)": booked_ret_on_margin
            })
            
        # Re-schedule slots based on Synthetic Margin
        slots, tot_margin, final_val, net_prof, booked_ret, total_reentries = schedule_synthetic_slots(trades_for_q)
        
        tot_cnt = len(trades_for_q)
        win_cnt = sum(1 for t in trades_for_q if t["Total Lot Booked P&L (₹)"] > 0)
        lose_cnt = sum(1 for t in trades_for_q if t["Total Lot Booked P&L (₹)"] <= 0)
        ...
        q_sum = {
            ...
            "Period": period_desc,
            "FY": fy_tag,
            "Synthetic Margin Deployed (₹)": tot_margin,
        ...
        for t_idx, t in enumerate(trades_for_q):
            ...
            if t["Total Lot Booked P&L (₹)"] <= 0:
                all_losers_master.append({
                    ...
                    "Lot Size": t["Lot Size"],
                    "Synthetic Margin (₹)": t["Synthetic Margin (₹)"],
                    "Total Lot Loss (₹)": t["Total Lot Booked P&L (₹)"],
                    "Booked Loss on Margin (%)": t["Booked Return on Margin (%)"]
                })
        ...
        # Title Banner
        ws_q.merge_cells("A1:R1")
        t_c = ws_q.cell(1, 1, f"NIFTY 50 {title_mode} — {q_name} ({period_desc})")
        style_cell(t_c, size=13, bold=True, color=COLOR_WHITE, bg=FILL_NAVY, align="center")
        ws_q.row_dimensions[1].height = 36
        
        # KPI Header
        ws_q.merge_cells("A3:R3")
        kpi_title = ws_q.cell(3, 1, f"QUARTER OPTIONS PERFORMANCE & SYNTHETIC MARGIN UTILISATION ({fy_tag})")
        style_cell(kpi_title, size=10, bold=True, color=COLOR_WHITE, bg=FILL_STEEL, align="left")
        ws_q.row_dimensions[3].height = 24
        
        kpi_headers = [
            "Synthetic Margin Deployed", "Final Value", "Net Booked Profit (₹)", "Booked Return on Margin", 
        ...
        ws_q.cell(7, 1, f"SYNTHETIC OPTIONS TRADE EXECUTION & MONTHLY EXPIRY LOG ({title_mode})")
        ws_q.merge_cells("A7:R7")
        style_cell(ws_q.cell(7, 1), size=10.5, bold=True, color=COLOR_WHITE, bg=FILL_NAVY, align="left")
        ws_q.row_dimensions[7].height = 26
        
        trade_headers = [
            "Trade #", "Symbol", "Company Name", "Sector", "Synthetic Pair", "ATM Strike (₹)",
            "Contract Expiry", "Expiry Decision", "Position Taking Window", 
            "Entry Date", "Exit Date", "Entry Spot (₹)", "Exit Spot (₹)", "Lot Size", 
            "Synthetic Margin (₹)", "Total Lot Booked P&L (₹)", "Booked Return on Margin (%)", 
        ...
        for r_idx, t in enumerate(trades_for_q):
            ...
            lot_pnl_val = t["Total Lot Booked P&L (₹)"]
            ret_on_mgn = t["Booked Return on Margin (%)"]
            ...
            for c_idx, val in enumerate(row_vals, 1):
                ...
                txt_c = "1A202C"
                if c_idx in [16, 17]:
                    bg = FILL_WIN if lot_pnl_val > 0 else FILL_LOSS
                    txt_c = COLOR_WIN_TXT if lot_pnl_val > 0 else COLOR_LOSS_TXT
                elif c_idx in [5, 8]:
                    bg = FILL_BADGE
                ...
                style_cell(c, size=9.5, bold=(c_idx in [2, 16, 17]), color=txt_c, bg=bg, align=align, num_fmt=fmt)
        ...
        ws_q.column_dimensions["E"].width = 30
        ws_q.column_dimensions["H"].width = 32
    ...
    avg_cap_all = float(df_qall["Synthetic Margin Deployed (₹)"].mean())
    avg_booked_ret = float(df_qall["Booked Return on Margin (%)"].mean())
    
    # Title Banner
    ws_exec.merge_cells("A1:M1")
    t_exec = ws_exec.cell(1, 1, f"NIFTY 50 {title_mode} — 12-QUARTERS EXECUTIVE OPTIONS DASHBOARD")
    ...
    sub_exec = ws_exec.cell(2, 1, f"Indian Financial Year Standard (Q1 Starts in April) | ATM Options (Current vs Next Month Expiry Rollover) | 5x Leverage Margin Efficiency")
    ...
    kpi_bar = ws_exec.cell(4, 1, "12-QUARTER GLOBAL SYNTHETIC OPTIONS PERFORMANCE SCORECARDS")
    style_cell(kpi_bar, size=11, bold=True, color=COLOR_WHITE, bg=FILL_STEEL, align="center")
    ws_exec.row_dimensions[4].height = 24
    
    scorecards = [
        ("Total Options Trades", f"{tot_trades_all:,}", "Across 12 Quarters"),
        ("Global Win Rate", f"{tot_win_rate_all:.1%}", f"{tot_win_all} Wins / {tot_lose_all} Losses"),
        ("Total Net Booked Profit", f"₹{tot_net_pnl_all:,.2f}", "Cumulative Options P&L"),
        ("Avg Booked Return / Qtr", f"{avg_booked_ret:.2%}", "On Synthetic Margin Deployed"),
        ("Avg Synthetic Margin", f"₹{avg_cap_all:,.2f}", "Per Quarter (Peak Slot Margin)")
    ]
    ...
    ws_exec.cell(9, 1, "QUARTER-BY-QUARTER SYNTHETIC OPTIONS PERFORMANCE (ALL 12 QUARTERS)")
    ...
    master_table_cols = [
        "Quarter", "Business Quarter (FY)", "Financial Year", "Synthetic Margin Deployed (₹)", 
    ...
    for r_idx, qrow in df_qall.iterrows():
        ...
        row_vals = [
            ...
            qrow["Period"],
            qrow["FY"],
            qrow["Synthetic Margin Deployed (₹)"],
            qrow["Final Value (₹)"],
            pnl_q,
    ...
    tot_row_vals = [
        ...
        "3 Financial Years",
        float(df_qall["Synthetic Margin Deployed (₹)"].sum()),
    ...
    tr_cols = [
        "Quarter", "FY", "Trade No", "Symbol", "Company Name", "Sector", "Synthetic Pair", "ATM Strike (₹)",
        "Contract Expiry", "Expiry Decision", "Position Taking Window", 
        "Entry Date", "Exit Date", "Entry Spot (₹)", "Exit Spot (₹)", "Lot Size", 
        "Synthetic Margin (₹)", "Total Lot Booked P&L (₹)", "Booked Return on Margin (%)", 
        "Assigned Slot", "Re-entry Type"
    ...
    for r_idx, trow in df_m.iterrows():
        ...
        lot_pnl_v = trow["Total Lot Booked P&L (₹)"]
        ret_mgn_v = trow["Booked Return on Margin (%)"]
        ...
        for c_idx, val in enumerate(row_vals, 1):
            ...
            style_cell(c, size=9.5, bold=(c_idx in [4, 18, 19]), color=txt_c, bg=bg, align=align, num_fmt=fmt)
    ...
    ws_all.column_dimensions["J"].width = 32
...
def main():
    print("Building 12-Quarters Synthetic Options Master Workbooks...")
    
    # 1. Build Synthetic Long Master
    build_synthetic_master_workbook("LONG", OUT_SYN_LONG, OUT_SYN_LONG_REP)
    
    # 2. Build Synthetic Short Master
    build_synthetic_master_workbook("SHORT", OUT_SYN_SHORT, OUT_SYN_SHORT_REP)
    
    print("\nBoth Synthetic Options Master Workbooks generated successfully!")
