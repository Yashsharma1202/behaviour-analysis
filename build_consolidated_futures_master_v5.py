...
OUTPUT_FUTURES_MASTER = BASE_DIR / 'Nifty50_12_Quarters_Futures_OI_Master_v5.xlsx'
OUTPUT_FUTURES_ONLY = BASE_DIR / 'Futures_Master_Only_v4.xlsx'

QUARTER_FUND_ALLOCATION = 10000000.0  # 1 Crore per quarter (Fresh fund per quarter)
...
lot_size_map = {}
try:
    df_opt_trades = pd.read_excel(OPTIONS_MASTER_PATH, sheet_name='All_12Q_Options_Trades', engine='openpyxl')
    for _, r in df_opt_trades.iterrows():
        sym = str(r['Symbol']).strip()
        lot = r.get('Lot Size (Qty)', 500)
        if pd.notnull(lot) and float(lot) > 0:
            lot_size_map[sym] = int(lot)
except Exception:
    pass

def get_lot_size(sym):
    if sym in lot_size_map: return lot_size_map[sym]
    if sym in ['RELIANCE', 'TCS', 'INFY', 'HDFCBANK', 'ICICIBANK', 'AXISBANK', 'SBIN']: return 550
    if sym in ['MARUTI', 'ULTRACEMCO', 'BAJAJ-AUTO']: return 100
    if sym in ['COALINDIA', 'ITC', 'BEL']: return 1500
    if sym in ['JIOFIN', 'TATASTEEL', 'POWERGRID']: return 2000
    return 500
...
print(f"Loaded {len(df_cons_trades)} consolidated trade records.", flush=True)
...
for idx, row in df_cons_trades.iterrows():
    ...
    eq_entry = row.get('Entry Price (₹)', 0.0) or row.get('Entry Price (?)', 0.0) or 1000.0
    eq_exit  = row.get('Exit Price (₹)', 0.0) or row.get('Exit Price (?)', 0.0) or 1000.0
    try: eq_entry = float(eq_entry)
    except: eq_entry = 1000.0
    try: eq_exit = float(eq_exit)
    except: eq_exit = 1000.0
    ...
    ret_we_get = (direction * (exit_price - entry_price) / entry_price) if entry_price > 0 else 0.0
...
print(f"Processed {len(df_all_trades)} Consolidated Futures trades with ZERO missing prices!", flush=True)
...
# ------------------- 5. Per-Quarter Fund Allocation & Compounding -------------------
quarter_summaries = []
cumulative_capital = QUARTER_FUND_ALLOCATION
...
for qtr in quarters_order:
    ...
    # Standalone Fund Utilised for Quarter
    fund_utilised = QUARTER_FUND_ALLOCATION
    final_val_quarter = fund_utilised + q_pnl
    q_return = (q_pnl / fund_utilised) if fund_utilised > 0 else 0.0
    
    # Cumulative Fund Compounding
    beg_cum_cap = cumulative_capital
    end_cum_cap = beg_cum_cap + q_pnl
    cumulative_capital = end_cum_cap
    ...
    allocated_per_trade = (fund_utilised * 0.90) / t_count if t_count > 0 else 0.0
    ...
    quarter_summaries.append({
        ...
        'Cumulative Beg Capital (₹)': beg_cum_cap,
        'Cumulative End Capital (₹)': end_cum_cap
    })
...
cell_title.value = "NIFTY 50 CONSOLIDATED FUTURES 12-QUARTERS EXECUTIVE DASHBOARD & CAPITAL UTILISATION"
...
ws_exec.cell(row=3, column=1, value="Per-Quarter Deployed Capital:").font = font_bold
ws_exec.cell(row=3, column=2, value=QUARTER_FUND_ALLOCATION).number_format = '₹#,##0.00'

ws_exec.cell(row=3, column=4, value="Cumulative Compounded Capital:").font = font_bold
ws_exec.cell(row=3, column=5, value=cumulative_capital).number_format = '₹#,##0.00'
...
headers_summary = [
    ...
    'Quarter Fund Utilised (₹)', 'Net Quarter P&L (₹)', 'Final Quarter Value (₹)',
    'Booked Return (%)', 'Allocated Capital per Trade (₹)'
]
...
print(f"\n🎉 CONSOLIDATED FUTURES MASTER CREATED IN {time.time() - t0:.1f}s!", flush=True)
