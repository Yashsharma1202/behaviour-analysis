...
for sym in SYMBOLS:
    ...
    stock_records.append({
        "sym": sym,
        "status": status,
        "hist_wr": stock_hist_wr,
        "pred_strat": pred_strat,
        "valid_dates": valid_dates,
        "pre_days": pre_days,
        "post_days": post_days,
        "df_fr_clean": df_fr_clean,
        "df_prices": df_prices,
        "hist_q_dates": hist_q_dates
    })

# Evaluate filtering thresholds
for wr_thresh in [50, 60, 65, 70, 75]:
    filtered_wins = 0
    filtered_total = 0
    
    for s in stock_records:
        if s['hist_wr'] < wr_thresh:
            continue
        ...
        for q_info in TARGET_QUARTERS:
            ...
            b_dt = None
            if not s['df_fr_clean'].empty:
                for _, row in s['df_fr_clean'].iterrows():
                    r_bdt = row['bDate']
                    r_tdt = row['tDate']
            ...
            if b_dt is None:
                q_past = s['hist_q_dates'].get(q_type, [])
                if q_past:
            ...
            res_idx = min(range(len(s['valid_dates'])), key=lambda i: abs((s['valid_dates'][i] - b_dt).days))
            entry_idx = max(0, res_idx - s['pre_days'])
            exit_idx = min(len(s['valid_dates']) - 1, res_idx + s['post_days'])
            
            p_entry = float(s['df_prices'].loc[entry_idx, 'adj'])
            p_exit = float(s['df_prices'].loc[exit_idx, 'adj'])
            
            if s['pred_strat'] == "LONG":
            ...
            filtered_total += 1
            filtered_wins += is_win
            
    wr_real = (filtered_wins / filtered_total * 100) if filtered_total > 0 else 0.0
    print(f"Filter Threshold: Hist Win Rate >= {wr_thresh}% | Stocks Included: {sum(1 for s in stock_records if s['hist_wr']>=wr_thresh)} | Trades: {filtered_total} | Predictive Win Rate: {wr_real:.2f}%")
