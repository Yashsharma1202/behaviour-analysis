...
# ------------------- 2. Calculate BOTH Drawdown % (Initial Capital vs Peak Portfolio) -------------------
opt_summaries = []
...
for qtr in quarters_order:
    ...
    # BOTH DRAWDOWN CALCULATIONS:
    # 1. DD % on Initial Capital
    dd_pct_initial = -abs(max_dd_val / peak_opt_capital) if peak_opt_capital > 0 else 0.0
    
    # 2. DD % on Peak Portfolio Value
...
print("Writing Both Drawdowns to Options Master Excel...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — EXECUTIVE SCORECARD (INITIAL CAPITAL vs PEAK PORTFOLIO DRAWDOWN)"
...
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER OPTIONS PERFORMANCE & DUAL DRAWDOWN SCORECARD"
...
ws_exec.cell(row=r_tot, column=7, value=df_opt_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=8, value=df_opt_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=7, value=df_opt_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=8, value=df_opt_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
for qtr in quarters_order:
    ...
    c_s = ws_q['A2']; c_s.value = "1% ITM OPTIONS DUAL DRAWDOWN & CAPITAL OVERVIEW"; c_s.font = font_section; c_s.fill = fill_section
...
print("Writing Standalone Options Dual Drawdown Summary Excel...", flush=True)
...
c_t = ws_s['A1']
c_t.value = "NIFTY 50 1% ITM OPTIONS STRATEGY — DUAL DRAWDOWN (INITIAL CAPITAL vs PEAK PORTFOLIO) SUMMARY"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
...
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER OPTIONS PERFORMANCE & DUAL DRAWDOWN SCORECARD"
...
ws_s.cell(row=r_tot, column=7, value=df_opt_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=8, value=df_opt_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=7, value=df_opt_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=8, value=df_opt_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
print(f"\n🎉 DUAL DRAWDOWN OPTIONS WORKBOOKS CREATED IN {time.time() - t0:.1f}s!", flush=True)
