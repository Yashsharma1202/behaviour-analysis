...
OUTPUT_FUTURES_MASTER = BASE_DIR / 'Nifty50_12_Quarters_Futures_OI_Master_v11.xlsx'
OUTPUT_FUTURES_ONLY = BASE_DIR / 'Futures_Master_Only_v10.xlsx'
...
# ------------------- 5. Per-Quarter Fund & Drawdown Summaries -------------------
quarter_summaries = []

for qtr in quarters_order:
    q_trades = df_all_trades[df_all_trades['Quarter'] == qtr].copy().sort_values(by='Exit Date')
    ...
    # Calculate Cumulative PnL & Max Drawdown
    ...
    # Calculate Concurrent Trades per Day
    min_date = pd.to_datetime(q_trades['Entry Date']).min()
    max_date = pd.to_datetime(q_trades['Exit Date']).max()
    all_b_dates = pd.date_range(min_date, max_date, freq='B')
    
    daily_conc = []
    for dt in all_b_dates:
        open_c = q_trades[(pd.to_datetime(q_trades['Entry Date']) <= dt) & (pd.to_datetime(q_trades['Exit Date']) >= dt)]
...
# ------------------- 8. Build Perfect v11 Excel Workbook with Embedded Charts -------------------
print("Building Master Workbook with 12 Quarter Drawdown Charts (v11)...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_sec = ws_exec['A6']
c_sec.value = "QUARTER-BY-QUARTER PERFORMANCE COMPARISON & MAX DRAWDOWN (ALL 12 QUARTERS)"
...
headers_conc = [
    ...
    'Max Drawdown (₹)', 'Max Drawdown (%)', 'Capital Reuse Efficiency (%)'
]
...
for r_idx, r_data in df_qtr_summary.iterrows():
    ...
    reuse_eff = (1.0 - (r_data['Slots Deployed'] / r_data['Total Trades']))
    ...
    c_eff = ws_conc.cell(row=row_num, column=9, value=reuse_eff)
    c_eff.number_format = '0.00%'; c_eff.font = font_win; c_eff.alignment = Alignment(horizontal='center')
...
autofit_columns(ws_conc, max_cols=9)

# --- SHEET 3-14: 12 SEPARATE QUARTER SHEETS WITH EMBEDDED DRAWDOWN CHARTS ---
qtr_card_labels = [
...
for qtr in quarters_order:
    ...
    q_trades = df_all_trades[df_all_trades['Quarter'] == qtr].copy().sort_values(by='Exit Date').reset_index(drop=True)
    q_summary = df_qtr_summary[df_qtr_summary['Quarter'] == qtr].iloc[0]
    dt_range = qtr_date_ranges.get(qtr, '')
    fy_str = q_summary['FY']
    
    # Calculate Cumulative PnL & Drawdown columns for trade journal
    ...
    # EMBED NATIVE EXCEL DRAWDOWN & CUMULATIVE PNL CHART ON EVERY QUARTER SHEET
    chart_q = LineChart()
...
print(f"\n🎉 MASTER WORKBOOK V11 WITH 12 EMBEDDED DRAWDOWN CHARTS CREATED IN {time.time() - t0:.1f}s!", flush=True)
