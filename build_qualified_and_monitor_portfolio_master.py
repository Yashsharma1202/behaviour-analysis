...
print("==========================================================================================")
print("BUILDING QUALIFIED & MONITOR SEPARATE PORTFOLIO CAPITAL ALLOCATOR MASTER WORKBOOK")
print("==========================================================================================")
...
def run_portfolio_simulation(target_symbols, label_name):
    ...
    for q_k in sorted_q_keys:
        ...
        portfolio_sim_25l.append({
            ...
            "trades_summary": f"All {n_active} Active {label_name} Stocks Traded Equal Weight"
        })
...
qualified_symbols = [s for s, d in stock_quarterly_returns.items() if d.get("is_available") and d.get("trading_status") == 'QUALIFIED']
monitor_symbols = [s for s, d in stock_quarterly_returns.items() if d.get("is_available") and d.get("trading_status") == 'AVOID BUT MONITOR IT']

print(f"Total QUALIFIED Stocks: {len(qualified_symbols)}")
print(f"Total MONITOR Stocks: {len(monitor_symbols)}")
...
def populate_portfolio_sheet(ws_p, res_data, sheet_title_name, sub_header_name):
    ...
    port_cards = [
        ("₹25 LAKHS PORTFOLIO NET P&L", f"₹{res_data['tot_pnl_25l']:,.2f}\n(+{res_data['tot_ret_25l']}%)", "A3:C4", "A3"),
        ("₹25L MAX DRAWDOWN %", f"-{res_data['max_dd_25l']}%", "D3:F4", "D3"),
        ("₹50 LAKHS PORTFOLIO NET P&L", f"₹{res_data['tot_pnl_50l']:,.2f}\n(+{res_data['tot_ret_50l']}%)", "G3:I4", "G3"),
        ("₹50L MAX DRAWDOWN %", f"-{res_data['max_dd_50l']}%", "J3:L4", "J3"),
        (f"TOTAL {sub_header_name.upper()} TRADED", f"{len(res_data['symbols'])} Stocks\nEqual Weight Allocation", "M3:N4", "M3")
    ]
    ...
    ws_p.cell(row=6, column=1, value=f"{sub_header_name.upper()} EQUITY GROWTH TIMELINE (₹25 LAKHS & ₹50 LAKHS PORTFOLIOS)").font = hdr_font
    ...
    for r_idx, (r25, r50) in enumerate(zip(res_data['sim_25l'], res_data['sim_50l']), start=8):
...
# SHEETS 3-7: ALL QUARTERS + Q1, Q2, Q3, Q4
quarter_names = {
...
print("==========================================================================================")
print(f"QUALIFIED & MONITOR SEPARATE PORTFOLIO MASTER CREATED SUCCESSFULLY! ({len(saved_paths)} files saved)")
print("==========================================================================================")
