...
OUTPUT_005_MASTER  = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.05PCT_Cost_Master.xlsx'
OUTPUT_005_SUMMARY = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.05PCT_Cost_Summary.xlsx'
...
print("Loading Source Data for Futures 0.05% Transaction Cost Model...", flush=True)
...
for qtr in quarters_order:
    ...
    for idx, row in q_tr.iterrows():
        ...
        # 100% EXACT MATH FORMULA: 0.05% ON COMBINED TRADE TURNOVER (0.0005 * Turnover)
        trans_cost   = round(combined_turnover * 0.0005, 2)
        ...
        # Position Taking Window & Metadata Clean Defaults
        pos_window  = clean_str(row.get('Position Taking Window'), 'T-8 to T+1')
        ...
        trade_rec = {
            ...
            'Strategy': strat,
            'Position Taking Window': pos_window,
            ...
            'Transaction Cost (0.05%) (₹)': trans_cost,
            'Net Futures P&L (After 0.05% Cost) (₹)': net_pnl,
            'Net Return on Margin (%)': net_ret_pct,
            'Assigned Slot': slot_val,
            'Re-entry Type': reentry_val,
            'Quarterly Result Date': q_result_date
        }
        cost_005_trades.append(trade_rec)

df_all_005_trades = pd.DataFrame(cost_005_trades)

# ------------------- 2. Calculate Day-by-Day Peak Capital & Dual Drawdowns (After 0.05% Cost) -------------------
summaries_005 = []

for qtr in quarters_order:
    q_tr = df_all_005_trades[df_all_005_trades['Quarter'] == qtr].copy()
    q_tr['Entry Date DT'] = pd.to_datetime(q_tr['Entry Date'])
    q_tr['Exit Date DT']  = pd.to_datetime(q_tr['Exit Date'])
    q_tr = q_tr.sort_values(by=['Entry Date DT', 'Trade No']).reset_index(drop=True)
    
    t_count = len(q_tr)
    wins = len(q_tr[q_tr['Net Futures P&L (After 0.05% Cost) (₹)'] > 0])
    losses = len(q_tr[q_tr['Net Futures P&L (After 0.05% Cost) (₹)'] <= 0])
    ...
    tot_trans_cost    = round(q_tr['Transaction Cost (0.05%) (₹)'].sum(), 2)
    tot_raw_pnl       = round(q_tr['Raw Futures P&L (₹)'].sum(), 2)
    tot_net_pnl       = round(q_tr['Net Futures P&L (After 0.05% Cost) (₹)'].sum(), 2)
    ...
    q_tr['Cum PnL'] = q_tr['Net Futures P&L (After 0.05% Cost) (₹)'].cumsum()
    ...
    summaries_005.append({
        ...
        'Total Transaction Cost (0.05%) (₹)': tot_trans_cost,
...
df_005_summary = pd.DataFrame(summaries_005)

# ------------------- 3. Build Master 13-Sheet Excel Workbook for 0.05% Cost -------------------
print("Building Master 13-Sheet Workbook for Futures 0.05% Cost Model...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY — COMBINED BUY+SELL TURNOVER (0.05% COST) EXECUTIVE SCORECARD"
...
global_tot_comb_turnover = round(df_all_005_trades['Combined Trade Turnover (₹)'].sum(), 2)
global_tot_raw_pnl       = round(df_all_005_trades['Raw Futures P&L (₹)'].sum(), 2)
global_tot_trans         = round(df_all_005_trades['Transaction Cost (0.05%) (₹)'].sum(), 2)
global_tot_net_pnl       = round(df_all_005_trades['Net Futures P&L (After 0.05% Cost) (₹)'].sum(), 2)
global_win_rate          = len(df_all_005_trades[df_all_005_trades['Net Futures P&L (After 0.05% Cost) (₹)'] > 0]) / len(df_all_005_trades)
...
c_h2 = ws_exec['D3']; c_h2.value = "GLOBAL WIN RATE (AFTER 0.05% COST)"; c_h2.font = font_card_lbl; c_h2.fill = fill_subheader; c_h2.alignment = Alignment(horizontal='center', vertical='center')
ws_exec.merge_cells('D4:F4')
c_v2 = ws_exec['D4']; c_v2.value = global_win_rate; c_v2.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v2.fill = fill_win; c_v2.number_format = '0.0%'; c_v2.alignment = Alignment(horizontal='center', vertical='center')

ws_exec.merge_cells('G3:I3')
c_h3 = ws_exec['G3']; c_h3.value = "TOTAL TRANSACTION COST (0.05%)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
...
c_v4 = ws_exec['J4']; c_v4.value = global_tot_net_pnl; c_v4.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v4.fill = fill_win; c_v4.number_format = '₹#,##0.00'; c_v4.alignment = Alignment(horizontal='center', vertical='center')
...
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (REAL-WORLD 0.05% TRANSACTION COST MODEL)"
...
headers_summary = ['Quarter', 'Combined Quarterly Turnover (₹)', 'Peak Margin Capital (₹)', 'Peak Date (Open Trades)', 'Raw Futures P&L (₹)', 'Total Trans Cost (0.05%) (₹)', 'Net Futures P&L (₹)', 'Return on Margin (%)', 'Max Realised Drawdown (₹)', 'DD % on Initial Capital', 'True DD % on Peak Portfolio', 'Win Rate (%)']
style_table_headers(ws_exec, 7, headers_summary, fill_header)
ws_exec.row_dimensions[7].height = 28

for r_idx, row_data in df_005_summary.iterrows():
    row_num = 8 + r_idx
    ...
    c_tc = ws_exec.cell(row=row_num, column=6, value=row_data['Total Transaction Cost (0.05%) (₹)'])
...
ws_exec.cell(row=r_tot, column=7).font = font_win; ws_exec.cell(row=r_tot, column=7).fill = fill_win
ws_exec.cell(row=r_tot, column=8, value="—").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_tot, column=9, value=-abs(df_005_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=9).font = font_loss; ws_exec.cell(row=r_tot, column=9).fill = fill_loss
ws_exec.cell(row=r_tot, column=10, value=df_005_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=11, value=df_005_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=2, value=df_005_summary['Combined Quarterly Turnover (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value=df_005_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=4, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=5, value=df_005_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=6, value=df_005_summary['Total Transaction Cost (0.05%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_loss; ws_exec.cell(row=r_avg, column=6).fill = fill_loss
ws_exec.cell(row=r_avg, column=7, value=df_005_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=7).font = font_win; ws_exec.cell(row=r_avg, column=7).fill = fill_win
ws_exec.cell(row=r_avg, column=8, value=df_005_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=9, value=-abs(df_005_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=9).font = font_loss; ws_exec.cell(row=r_avg, column=9).fill = fill_loss
ws_exec.cell(row=r_avg, column=10, value=df_005_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=11, value=df_005_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
# Chart
chart_f = LineChart()
chart_f.title = "Master 12-Quarter Net Futures Realised P&L (After 0.05% Cost) (₹)"
chart_f.style = 13
...
trade_headers = [
    ...
    'Raw Futures P&L (₹)', 'Transaction Cost (0.05%) (₹)', 'Net Futures P&L (₹)', 'Cumulative P&L (₹)', 'Drawdown (₹)',
...
for qtr in quarters_order:
    ...
    cum_pnl = q_tr['Net Futures P&L (After 0.05% Cost) (₹)'].cumsum()
    running_max = np.maximum(0.0, np.maximum.accumulate(cum_pnl))
    q_tr['Cumulative P&L (₹)'] = cum_pnl
    q_tr['Drawdown (₹)'] = cum_pnl - running_max
    
    q_sum = df_005_summary[df_005_summary['Quarter'] == qtr].iloc[0]
    
    ws_q.merge_cells('A1:W1')
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 FUTURES STRATEGY (EXACT 0.05% ON COMBINED BUY+SELL TURNOVER) — {qtr.upper()}"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ws_q.row_dimensions[1].height = 40
    
    card_lbls = ['Combined Quarterly Turnover (₹)', 'Peak Margin Capital (₹)', 'Peak Date (Open Trades)', 'Raw Futures P&L (₹)', 'Total Trans Cost (0.05%) (₹)', 'Net Futures Realised P&L (₹)', 'Return on Margin (%)', 'Win Rate (%)', 'Total Trades', 'Max Drawdown (₹)', 'DD % Initial Margin', 'True DD % Peak Portfolio']
    ws_q.merge_cells('A2:L2')
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS & TURNOVER OVERVIEW (AFTER 0.05% COMBINED COST)"; c_s.font = font_section; c_s.fill = fill_section
    ...
    c_tc1 = ws_q.cell(row=4, column=5, value=q_sum['Total Transaction Cost (0.05%) (₹)'])
    ...
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE FUTURES JOURNAL (EXACT 0.05% ON COMBINED BUY+SELL TURNOVER)"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        # Position Taking Window Included
        ws_q.cell(row=row_num, column=6, value=clean_str(t_row['Position Taking Window'], 'T-8 to T+1')).alignment = Alignment(horizontal='center')
        ...
        # 0.05% Transaction Cost Cell
        c_cost = ws_q.cell(row=row_num, column=17, value=t_row['Transaction Cost (0.05%) (₹)'])
        c_cost.number_format = '₹#,##0.00'; c_cost.font = font_loss; c_cost.fill = fill_loss
        
        c_fp = ws_q.cell(row=row_num, column=18, value=t_row['Net Futures P&L (After 0.05% Cost) (₹)'])
        c_fp.number_format = '₹#,##0.00'
        c_fp.font = font_win if t_row['Net Futures P&L (After 0.05% Cost) (₹)'] >= 0 else font_loss
        c_fp.fill = fill_win if t_row['Net Futures P&L (After 0.05% Cost) (₹)'] >= 0 else fill_loss
    ...
    chart_q = LineChart()
    chart_q.title = f"{qtr} — Futures (0.05% Cost) Cumulative P&L Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
wb.save(OUTPUT_005_MASTER)
print(f"  ✓ Saved Master Workbook: {OUTPUT_005_MASTER.name}")

# --- STANDALONE SUMMARY WORKBOOK ---
print("Writing Standalone 0.05% Cost Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Futures_0.05PCT_Cost_Summary')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:L1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY (REAL-WORLD 0.05% TRANSACTION COST) — SUMMARY DASHBOARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
ws_s.row_dimensions[1].height = 40

ws_s.merge_cells('A3:C3')
c_h1 = ws_s['A3']; c_h1.value = "TOTAL 12-QUARTER COMBINED TURNOVER"; c_h1.font = font_card_lbl; c_h1.fill = fill_subheader; c_h1.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('A4:C4')
c_v1 = ws_s['A4']; c_v1.value = global_tot_comb_turnover; c_v1.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_TEXT_MAIN); c_v1.fill = fill_card; c_v1.number_format = '₹#,##0.00'; c_v1.alignment = Alignment(horizontal='center', vertical='center')

ws_s.merge_cells('D3:F3')
c_h2 = ws_s['D3']; c_h2.value = "GLOBAL WIN RATE (AFTER 0.05% COST)"; c_h2.font = font_card_lbl; c_h2.fill = fill_subheader; c_h2.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('D4:F4')
c_v2 = ws_s['D4']; c_v2.value = global_win_rate; c_v2.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v2.fill = fill_win; c_v2.number_format = '0.0%'; c_v2.alignment = Alignment(horizontal='center', vertical='center')

ws_s.merge_cells('G3:I3')
c_h3 = ws_s['G3']; c_h3.value = "TOTAL TRANSACTION COST (0.05%)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('G4:I4')
c_v3 = ws_s['G4']; c_v3.value = global_tot_trans; c_v3.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_LOSS_TEXT); c_v3.fill = fill_loss; c_v3.number_format = '₹#,##0.00'; c_v3.alignment = Alignment(horizontal='center', vertical='center')

ws_s.merge_cells('J3:L3')
c_h4 = ws_s['J3']; c_h4.value = "NET 12-QUARTER FUTURES PROFIT"; c_h4.font = font_card_lbl; c_h4.fill = fill_subheader; c_h4.alignment = Alignment(horizontal='center', vertical='center')
ws_s.merge_cells('J4:L4')
c_v4 = ws_s['J4']; c_v4.value = global_tot_net_pnl; c_v4.font = Font(name='Segoe UI', size=15, bold=True, color=COLOR_WIN_TEXT); c_v4.fill = fill_win; c_v4.number_format = '₹#,##0.00'; c_v4.alignment = Alignment(horizontal='center', vertical='center')

ws_s.row_dimensions[3].height = 20
ws_s.row_dimensions[4].height = 28
for r in [3, 4]:
    for c in range(1, 13): ws_s.cell(row=r, column=c).border = border_thin

ws_s.merge_cells('A6:L6')
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (REAL-WORLD 0.05% TRANSACTION COST MODEL)"
...
for r_idx, row_data in df_005_summary.iterrows():
    ...
    ws_s.cell(row=row_num, column=2, value=row_data['Combined Quarterly Turnover (₹)']).number_format = '₹#,##0.00'
    ws_s.cell(row=row_num, column=3, value=row_data['Peak Concurrent Margin (₹)']).number_format = '₹#,##0.00'
    ws_s.cell(row=row_num, column=4, value=f"{row_data['Peak Capital Date']} ({row_data['Peak Open Trades']} Trades)").alignment = Alignment(horizontal='center')
    
    ws_s.cell(row=row_num, column=5, value=row_data['Raw Futures P&L (₹)']).number_format = '₹#,##0.00'
    
    c_tc = ws_s.cell(row=row_num, column=6, value=row_data['Total Transaction Cost (0.05%) (₹)'])
    c_tc.number_format = '₹#,##0.00'; c_tc.font = font_loss; c_tc.fill = fill_loss
    
    c_pnl = ws_s.cell(row=row_num, column=7, value=row_data['Net Futures Realised P&L (₹)'])
    c_pnl.number_format = '₹#,##0.00'; c_pnl.fill = fill_win if row_data['Net Futures Realised P&L (₹)'] >= 0 else fill_loss
    c_pnl.font = font_win if row_data['Net Futures Realised P&L (₹)'] >= 0 else font_loss
    
    ws_s.cell(row=row_num, column=8, value=row_data['Booked Return on Margin (%)']).number_format = '+0.00%;-0.00%'
    
    c_dd = ws_s.cell(row=row_num, column=9, value=-abs(row_data['Max Drawdown (₹)']))
    c_dd.number_format = '-₹#,##0.00'; c_dd.font = font_loss; c_dd.fill = fill_loss
    
    c_ddi = ws_s.cell(row=row_num, column=10, value=row_data['DD % on Initial Capital'])
    c_ddi.number_format = '-0.00%'; c_ddi.font = font_loss; c_ddi.fill = fill_loss; c_ddi.alignment = Alignment(horizontal='center')
    
    c_ddp = ws_s.cell(row=row_num, column=11, value=row_data['True DD % on Peak Portfolio'])
    c_ddp.number_format = '-0.00%'; c_ddp.font = font_loss; c_ddp.fill = fill_loss; c_ddp.alignment = Alignment(horizontal='center')
    
    c_wr = ws_s.cell(row=row_num, column=12, value=row_data['Win Rate (%)'])
    c_wr.number_format = '0.00%'; c_wr.alignment = Alignment(horizontal='center')
    if row_data['Win Rate (%)'] >= 0.70: c_wr.fill = fill_win; c_wr.font = font_win
    elif row_data['Win Rate (%)'] >= 0.50: c_wr.fill = fill_gold; c_wr.font = font_gold
    else: c_wr.fill = fill_loss; c_wr.font = font_loss
...
ws_s.cell(row=r_tot, column=2, value=global_tot_comb_turnover).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=3, value="—").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_tot, column=4, value="—").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_tot, column=5, value=global_tot_raw_pnl).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=6, value=global_tot_trans).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=6).font = font_loss; ws_s.cell(row=r_tot, column=6).fill = fill_loss
ws_s.cell(row=r_tot, column=7, value=global_tot_net_pnl).number_format = '₹#,##0.00'
ws_s.cell(row=r_tot, column=7).font = font_win; ws_s.cell(row=r_tot, column=7).fill = fill_win
ws_s.cell(row=r_tot, column=8, value="—").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_tot, column=9, value=-abs(df_005_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=9).font = font_loss; ws_s.cell(row=r_tot, column=9).fill = fill_loss
ws_s.cell(row=r_tot, column=10, value=df_005_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=11, value=df_005_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
ws_s.cell(row=r_tot, column=11).font = font_loss; ws_s.cell(row=r_tot, column=11).fill = fill_loss; ws_s.cell(row=r_tot, column=11).alignment = Alignment(horizontal='center')
c_twr = ws_s.cell(row=r_tot, column=12, value=global_win_rate)
c_twr.number_format = '0.00%'; c_twr.font = font_win; c_twr.fill = fill_win; c_twr.alignment = Alignment(horizontal='center')
for c in range(1, 13): ws_s.cell(row=r_tot, column=c).border = border_thin

# Row 21 Average
r_avg = 21
ws_s.row_dimensions[r_avg].height = 24
ws_s.cell(row=r_avg, column=1, value="QUARTERLY AVERAGE").font = font_data_bold
ws_s.cell(row=r_avg, column=2, value=df_005_summary['Combined Quarterly Turnover (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value=df_005_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=4, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=5, value=df_005_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=6, value=df_005_summary['Total Transaction Cost (0.05%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_loss; ws_s.cell(row=r_avg, column=6).fill = fill_loss
ws_s.cell(row=r_avg, column=7, value=df_005_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=7).font = font_win; ws_s.cell(row=r_avg, column=7).fill = fill_win
ws_s.cell(row=r_avg, column=8, value=df_005_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=9, value=-abs(df_005_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=9).font = font_loss; ws_s.cell(row=r_avg, column=9).fill = fill_loss
ws_s.cell(row=r_avg, column=10, value=df_005_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=11, value=df_005_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
ws_s.cell(row=r_avg, column=11).font = font_loss; ws_s.cell(row=r_avg, column=11).fill = fill_loss; ws_s.cell(row=r_avg, column=11).alignment = Alignment(horizontal='center')
c_awr = ws_s.cell(row=r_avg, column=12, value=global_win_rate)
c_awr.number_format = '0.00%'; c_awr.font = font_win; c_awr.fill = fill_win; c_awr.alignment = Alignment(horizontal='center')
for c in range(1, 13): ws_s.cell(row=r_avg, column=c).border = border_double_bottom
...
# Chart
chart = LineChart()
chart.title = "Consolidated 12-Quarter Net Futures Realised P&L (After 0.05% Cost) Trend (₹)"
chart.style = 13
...
data = Reference(ws_s, min_col=7, min_row=7, max_row=19)
cats = Reference(ws_s, min_col=1, min_row=8, max_row=19)
chart.add_data(data, titles_from_data=True)
chart.set_categories(cats)
ws_s.add_chart(chart, "N3")

wb_s.save(OUTPUT_005_SUMMARY)
print(f"  ✓ Saved Summary Workbook: {OUTPUT_005_SUMMARY.name}")

print(f"\n🎉 FUTURES 0.05% COST MASTER & SUMMARY CREATED IN {time.time() - t0:.1f}s!", flush=True)
