...
print("==========================================================================================")
print("BUILDING CLEAN 3-SHEET FULL HISTORY WIN RATE RANKING MASTER WORKBOOK")
print("==========================================================================================")
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    wr_long = round((emp_wins_long / total_q) * 100, 2) if total_q > 0 else 50.0
    wr_short = round((emp_wins_short / total_q) * 100, 2) if total_q > 0 else 50.0
    ...
    if wr_long >= wr_short:
        ...
        full_hist_wr = wr_long
        win_count = emp_wins_long
        loss_count = total_q - emp_wins_long
        avg_ret = pred_ret_long
    else:
        optimal_strat = "FUTURE SHORT"
        full_hist_wr = wr_short
        win_count = emp_wins_short
        loss_count = total_q - emp_wins_short
        avg_ret = pred_ret_short

    # Calculate 12-Quarter Net P&L for this stock
    stock_12q_pnl = 0.0
    for q_idx, (q_name, default_d_str) in enumerate(quarters_list_12):
...
# Sort strictly by Full History Win Rate % descending, then Total Quarters descending, then Net P&L descending
df_all = df_all.sort_values(['full_hist_wr', 'total_quarters', 'pnl_12q'], ascending=[False, False, False]).reset_index(drop=True)
...
print(f"Total Target Universe: {len(df_all)} Stocks")
print(f"Total Combined Historical Quarters/Trades Analyzed: {total_all_hist_trades}")
print(f"Qualified Stocks (>=12 Qtrs): {len(df_q)} Stocks")
print(f"Avoid but Monitor (<12 Qtrs): {len(df_avoid)} Stocks")

out_path = ROOT / "Nifty211_All_Stocks_Full_History_WinRate_Ranking_Master_v2.xlsx"
...
def write_ranking_sheet(ws, sheet_title, df_sheet):
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {sheet_title.upper()} RANKING BY FULL HISTORY WIN RATE %"
    ...
    top_wr = df_sheet.iloc[0]['full_hist_wr'] if not df_sheet.empty else 0.0
    avg_wr = round(df_sheet['full_hist_wr'].mean(), 2) if not df_sheet.empty else 0.0
    tot_pnl = round(df_sheet['pnl_12q'].sum(), 2) if not df_sheet.empty else 0.0
    
    cards = [
        ("TOTAL STOCKS RANKED", f"{len(df_sheet)} Stocks", "A3:B4", "A3"),
        ("TOP FULL HISTORY STOCK", f"{top_stock} ({top_wr}%)", "C3:D4", "C3"),
        ("AVERAGE FULL HISTORY WIN RATE %", f"{avg_wr}%", "E3:F4", "E3"),
    ...
    ws.cell(row=6, column=1, value="FULL HISTORY WIN RATE BRACKET DISTRIBUTION SUMMARY").font = hdr_font
    ...
    b_80 = df_sheet[df_sheet['full_hist_wr'] >= 80]
    b_70 = df_sheet[(df_sheet['full_hist_wr'] >= 70) & (df_sheet['full_hist_wr'] < 80)]
    b_60 = df_sheet[(df_sheet['full_hist_wr'] >= 60) & (df_sheet['full_hist_wr'] < 70)]
    b_50 = df_sheet[(df_sheet['full_hist_wr'] >= 50) & (df_sheet['full_hist_wr'] < 60)]
    b_below = df_sheet[df_sheet['full_hist_wr'] < 50]
    ...
    # Main Ranking Table
    rank_start_row = 15
    ws.cell(row=rank_start_row, column=1, value=f"ALL {len(df_sheet)} STOCKS RANKED BY FULL HISTORY WIN RATE %").font = hdr_font
    ...
    headers = [
        "Rank", "Stock Symbol", "Optimal Strategy", "Action Status", "Total Quarters Analyzed",
        "Position Window", "Full History Win Rate %", "Winning Quarters", "Losing Quarters",
        "Average Return %", "Margin Capital (₹)", "12-Quarter Net P&L (₹)"
    ]
    ...
    for r_idx, r in enumerate(df_sheet.to_dict('records'), start=rank_start_row+2):
        ...
        c_wr = ws.cell(row=r_idx, column=7, value=f"{r['full_hist_wr']}%")
        c_wr.border = border_thin; c_wr.alignment = Alignment(horizontal="right")
        c_wr.font = Font(bold=True)
        if r['full_hist_wr'] >= 65.0: c_wr.fill = pos_fill; c_wr.font = pos_font
        elif r['full_hist_wr'] < 50.0: c_wr.fill = neg_fill; c_wr.font = neg_font
        
        ws.cell(row=r_idx, column=8, value=r['win_count']).border = border_thin
        ws.cell(row=r_idx, column=8).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=9, value=r['loss_count']).border = border_thin
        ws.cell(row=r_idx, column=9).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=10, value=f"{r['avg_ret']}%").border = border_thin
...
# Write Sheet 1: All 211 Stocks Full History Ranking
ws1 = wb.active
ws1.title = "All 211 Stocks Ranking"
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_All_Stocks_Full_History_WinRate_Ranking_Master.xlsx")
print("==========================================================================================")
print("CLEAN 3-SHEET FULL HISTORY WIN RATE RANKING MASTER WORKBOOK CREATED SUCCESSFULLY!")
print("==========================================================================================")
