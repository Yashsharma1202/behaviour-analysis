...
# ------------------- 1. Load Daily Futures Price Lookup -------------------
...
# ------------------- 3. Load Lot Sizes from Options Master -------------------
print("Loading stock lot sizes...", flush=True)
lot_size_map = {}
try:
    ...
    print(f"Warning loading lot sizes: {e}", flush=True)
...
# ------------------- 4. Load Trades from REGULARIZED MASTER -------------------
...
print(f"Loaded {len(df_reg_trades)} regularized trade records.", flush=True)
...
for idx, row in df_reg_trades.iterrows():
    ...
    # Equity fallback prices from regularized master
...
print("Building 17-sheet Master Workbook...", flush=True)
wb = Workbook()
...
for qtr in quarters_order:
    ...
    c_b = ws_q['A1']
    c_b.value = f"NIFTY 50 REGULARIZED FUTURES TRADES PERFORMANCE — {qtr.upper()}"
...
print(f"\n🎉 REGULARIZED FUTURES MASTER CREATED IN {time.time() - t0:.1f}s!", flush=True)
