...
OUT_EXCEL  = ROOT / "Nifty50Stocks_QtyResultDates_UPDATED.xlsx"
OUT_HTML   = ROOT / "Nifty50Stocks_QtyResultDates_Report.html"
OUT_PDF    = ROOT / "Nifty50Stocks_QtyResultDates_Report.pdf"
...
def apply_case1(df: pd.DataFrame, entry_dt: pd.Timestamp, exit_dt: pd.Timestamp,
                latest_dt: pd.Timestamp) -> dict:
    res = dict(
        status="Filter Out",
        actual_buy_date=None, buy_price=None,
...
HEADERS = [
    ...
    "Planned Exit Date", "Exit Price", "Realised Return (%)", "Status"
]
...
def write_row(ws, r, data: dict, res: dict, entry_date, exit_date, S: dict):
    ...
    if status in ("Upcoming", "Filter Out"):
        for col in range(9, len(HEADERS) + 1):
            wc(col, "—")
        if status == "Upcoming":
            wc(14, "Upcoming",   fnt=S["ital"])
        else:
            wc(14, "Filter Out", fnt=S["rtxt"], fll=S["rfill"])
        return

    wc(9,  res["actual_buy_date"],  nfmt="yyyy-mm-dd")
    wc(10, res["buy_price"],        nfmt="#,##0.00", aln=S["ar"])
    wc(11, exit_date,               nfmt="yyyy-mm-dd")
    wc(12, res["exit_price"],       nfmt="#,##0.00", aln=S["ar"])

    ret = res["ret"]
    if ret is not None:
        if ret >= 0:
            wc(13, ret, nfmt="0.00", aln=S["ar"], fnt=S["gtxt"], fll=S["gfill"])
        else:
            wc(13, ret, nfmt="0.00", aln=S["ar"], fnt=S["rtxt"], fll=S["rfill"])
    else:
        wc(13, "—")

    if status == "Realised":
        wc(14, "Realised", fnt=S["gtxt"], fll=S["gfill"])
    elif status == "Open":
        wc(14, "Open",     fnt=S["ytxt"], fll=S["yfill"])
    else:
        wc(14, status)
...
def main():
    ...
    print("\nPre-fetching price data for all symbols...")
    price_cache = {}
    ...
    for quarter in qtr_names:
        ...
        cell = ws_dest.cell(row=2, column=1, value=f"  ◆  {quarter}  —  Case 1 Baseline Strategy")
        cell.font = qtr_font
        cell.fill = qtr_fill
        ...
        for r_src in range(2, ws_src.max_row + 1):
            ...
            res = apply_case1(df, entry_dt, exit_dt, latest_dt)
            
            # Statistics tracking
            total_stocks += 1
            if res["status"] in ("Realised", "Open"):
                trades_taken += 1
                ret = res["ret"]
                if ret is not None:
                    total_ret += ret
    ...
    for suffix in ["", "_2", "_3", "_4", "_5", "_6"]:
        try:
            out_path = OUT_EXCEL.parent / (OUT_EXCEL.stem.replace("_UPDATED", "") + "_UPDATED" + suffix + ".xlsx")
    ...
    # ── PDF Compile ───────────────────────────────────────────────────────────
    print("Compiling Case 1 PDF Performance Report...")
    ...
    for row in summary_stats:
        tot_trades_all += row['traded']
        tot_profits_all += int(row['traded'] * row['win_rate'] / 100)
        tot_returns_sum += row['avg_ret'] * row['traded']
    ...
    overall_win_rate = (tot_profits_all / tot_trades_all * 100) if tot_trades_all > 0 else 0.0
    overall_avg_ret = (tot_returns_sum / tot_trades_all) if tot_trades_all > 0 else 0.0
    ...
    for q_name in qtr_names:
        ...
        for ch_idx, chunk in enumerate(chunks, 1):
            ...
            for r in chunk:
                ...
                ex_date = safe_date_str(r["exit_date"], "%d-%b-%Y")
                ...
                <tr class="{status_cls}-row">
                    ...
                    <td class="c">{r['buy_before']}</td>
                    <td class="c">{r['sell_after']}</td>
            ...
            <div class="page">
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 5%;">Before</th>
                            <th style="width: 5%;">After</th>
                            ...
                            <th style="width: 9%;">Realised Return</th>
                ...
                <footer>
                    <span>Nifty 50 Post-Earnings Case 1 Baseline Performance Report</span>
...
<body>
    ...
    <div class="page cover-page">
        <header>
            <h1>Nifty 50 Case 1 Analysis Report</h1>
            <div class="subtitle">Baseline Strategy Performance & Capital Compounding Audit</div>
        </header>
        
        <div class="cover-main">
            <div class="cover-title">Case 1 Baseline Performance Report</div>
            <div class="cover-subtitle">Simulating Unconditional Purchases (No Filter, No SL) Across 7 Quarters (350 Trades)</div>
        ...
        <div class="cover-meta">
            <div class="meta-box">
                ...
                <div class="meta-value">{tot_trades_all}</div>
            </div>
            <div class="meta-box">
                <div class="meta-label">Overall Win Rate</div>
                <div class="meta-value" style="color: { '#15803d' if overall_win_rate >= 50 else '#b91c1c' };">{overall_win_rate:.1f}%</div>
            </div>
            <div class="meta-box">
                <div class="meta-label">Average Return</div>
                <div class="meta-value" style="color: { '#15803d' if overall_avg_ret >= 0 else '#b91c1c' };">{overall_avg_ret:+.2f}%</div>
            </div>
        </div>
