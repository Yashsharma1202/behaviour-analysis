...
OUTPUT_TOTAL_MASTER  = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.5PCT_Total_Turnover_Master.xlsx'
OUTPUT_TOTAL_SUMMARY = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.5PCT_Total_Turnover_Summary.xlsx'
...
print("Loading Source Data for Total 0.5% Cost Model...", flush=True)
...
for qtr in quarters_order:
    ...
    for idx, row in q_tr.iterrows():
        ...
        combined_turnover = buy_turnover + sell_turnover
        
        # EXACT TOTAL 0.5% COST ON TRADE ROUND-TRIP
        trans_cost   = round(buy_turnover * 0.005, 2)  # 0.5% total transaction cost allowance
        ...
        trade_rec = {
            ...
            'Combined Trade Turnover (₹)': round(combined_turnover, 2),
            'Required Margin Outlay (₹)': margin_outlay,
            'Raw Futures P&L (₹)': round(raw_pnl, 2),
            'Transaction Cost (Total 0.5%) (₹)': trans_cost,
        ...
        total_cost_trades.append(trade_rec)

df_all_total_trades = pd.DataFrame(total_cost_trades)

# ------------------- 2. Calculate Day-by-Day Peak Capital & Dual Drawdowns (After Total 0.5% Cost) -------------------
total_summaries = []

for qtr in quarters_order:
    q_tr = df_all_total_trades[df_all_total_trades['Quarter'] == qtr].copy()
    q_tr['Entry Date DT'] = pd.to_datetime(q_tr['Entry Date'])
    ...
    tot_trans_cost    = round(q_tr['Transaction Cost (Total 0.5%) (₹)'].sum(), 2)
...
# ------------------- 3. Build Master 13-Sheet Excel Workbook for Total 0.5% Cost -------------------
print("Building Master 13-Sheet Workbook for Total 0.5% Cost Model...", flush=True)
wb = Workbook()
wb.remove(wb.active)
...
c_t = ws_exec['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY — TOTAL 0.5% TRANSACTION COST MODEL EXECUTIVE SCORECARD"
...
global_tot_comb_turnover = round(df_all_total_trades['Combined Trade Turnover (₹)'].sum(), 2)
global_tot_raw_pnl       = round(df_all_total_trades['Raw Futures P&L (₹)'].sum(), 2)
global_tot_trans         = round(df_all_total_trades['Transaction Cost (Total 0.5%) (₹)'].sum(), 2)
global_tot_net_pnl       = round(df_all_total_trades['Net Futures P&L (After 0.5% Cost) (₹)'].sum(), 2)
global_win_rate          = len(df_all_total_trades[df_all_total_trades['Net Futures P&L (After 0.5% Cost) (₹)'] > 0]) / len(df_all_total_trades)
...
c_sec = ws_exec['A6']
c_sec.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (TOTAL 0.5% TRANSACTION COST MODEL)"
...
for r_idx, row_data in df_total_summary.iterrows():
    row_num = 8 + r_idx
...
ws_exec.cell(row=r_tot, column=9, value=-abs(df_total_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_tot, column=9).font = font_loss; ws_exec.cell(row=r_tot, column=9).fill = fill_loss
ws_exec.cell(row=r_tot, column=10, value=df_total_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_tot, column=11, value=df_total_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=2, value=df_total_summary['Combined Quarterly Turnover (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=3, value=df_total_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=4, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_exec.cell(row=r_avg, column=5, value=df_total_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=6, value=df_total_summary['Total Transaction Cost (0.5%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=6).font = font_loss; ws_exec.cell(row=r_avg, column=6).fill = fill_loss
ws_exec.cell(row=r_avg, column=7, value=df_total_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_exec.cell(row=r_avg, column=7).font = font_win; ws_exec.cell(row=r_avg, column=7).fill = fill_win
ws_exec.cell(row=r_avg, column=8, value=df_total_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_exec.cell(row=r_avg, column=9, value=-abs(df_total_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_exec.cell(row=r_avg, column=9).font = font_loss; ws_exec.cell(row=r_avg, column=9).fill = fill_loss
ws_exec.cell(row=r_avg, column=10, value=df_total_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_exec.cell(row=r_avg, column=11, value=df_total_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
# Chart
chart_f = LineChart()
chart_f.title = "Master 12-Quarter Net Futures Realised P&L (After Total 0.5% Cost) (₹)"
chart_f.style = 13
...
for qtr in quarters_order:
    ...
    q_sum = df_total_summary[df_total_summary['Quarter'] == qtr].iloc[0]
    ...
    c_t = ws_q['A1']
    c_t.value = f"NIFTY 50 FUTURES STRATEGY (TOTAL 0.5% TRANSACTION COST) — {qtr.upper()}"
    c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
    ...
    c_s = ws_q['A2']; c_s.value = f"{qtr.upper()} — PERFORMANCE METRICS & TURNOVER OVERVIEW (AFTER 0.5% TOTAL COST)"; c_s.font = font_section; c_s.fill = fill_section
    ...
    c_s2 = ws_q['A6']; c_s2.value = f"{qtr.upper()} — COMPLETE DETAILED 50-TRADE FUTURES JOURNAL (WITH 0.5% TOTAL TRANSACTION COST)"; c_s2.font = font_section; c_s2.fill = fill_section
    ...
    for r_idx, t_row in q_tr.iterrows():
        ...
        c_cost = ws_q.cell(row=row_num, column=17, value=t_row['Transaction Cost (Total 0.5%) (₹)'])
    ...
    chart_q = LineChart()
    chart_q.title = f"{qtr} — Futures (Total 0.5% Cost) Cumulative P&L Curve (Max DD: -₹{q_sum['Max Drawdown (₹)']:,.2f})"
    chart_q.style = 13
...
try:
    wb.save(OUTPUT_TOTAL_MASTER)
    print(f"  ✓ Saved Master Workbook: {OUTPUT_TOTAL_MASTER.name}")
except Exception as e:
    alt_file = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.5PCT_Total_Turnover_Master_CLEAN.xlsx'
    wb.save(alt_file)
    print(f"  ✓ Saved Master Workbook (Alt): {alt_file.name}")

# --- STANDALONE SUMMARY WORKBOOK ---
print("Writing Standalone Total 0.5% Cost Summary Excel Workbook...", flush=True)
wb_s = openpyxl.Workbook()
wb_s.remove(wb_s.active)

ws_s = wb_s.create_sheet(title='Futures_0.5PCT_Total_Summary')
ws_s.views.sheetView[0].showGridLines = True

ws_s.merge_cells('A1:L1')
c_t = ws_s['A1']
c_t.value = "NIFTY 50 FUTURES STRATEGY (TOTAL 0.5% TRANSACTION COST) — SUMMARY DASHBOARD"
c_t.font = font_title; c_t.fill = fill_title; c_t.alignment = Alignment(horizontal='center', vertical='center')
...
c_h2 = ws_s['D3']; c_h2.value = "GLOBAL WIN RATE (AFTER 0.5% COST)"; c_h2.font = font_card_lbl; c_h2.fill = fill_subheader; c_h2.alignment = Alignment(horizontal='center', vertical='center')
...
c_h3 = ws_s['G3']; c_h3.value = "TOTAL TRANSACTION COST (0.5%)"; c_h3.font = font_card_lbl; c_h3.fill = fill_subheader; c_h3.alignment = Alignment(horizontal='center', vertical='center')
...
c_s = ws_s['A6']
c_s.value = "CONSOLIDATED 12-QUARTER PERFORMANCE SCORECARD (TOTAL 0.5% TRANSACTION COST MODEL)"
...
for r_idx, row_data in df_total_summary.iterrows():
    ...
    c_tc = ws_s.cell(row=row_num, column=6, value=row_data['Total Transaction Cost (0.5%) (₹)'])
    c_tc.number_format = '₹#,##0.00'; c_tc.font = font_loss; c_tc.fill = fill_loss
...
ws_s.cell(row=r_tot, column=9, value=-abs(df_total_summary['Max Drawdown (₹)'].max())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_tot, column=9).font = font_loss; ws_s.cell(row=r_tot, column=9).fill = fill_loss
ws_s.cell(row=r_tot, column=10, value=df_total_summary['DD % on Initial Capital'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_tot, column=11, value=df_total_summary['True DD % on Peak Portfolio'].min()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=2, value=df_total_summary['Combined Quarterly Turnover (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=3, value=df_total_summary['Peak Concurrent Margin (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=4, value="Avg ~17 Trades").alignment = Alignment(horizontal='center')
ws_s.cell(row=r_avg, column=5, value=df_total_summary['Raw Futures P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=6, value=df_total_summary['Total Transaction Cost (0.5%) (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=6).font = font_loss; ws_s.cell(row=r_avg, column=6).fill = fill_loss
ws_s.cell(row=r_avg, column=7, value=df_total_summary['Net Futures Realised P&L (₹)'].mean()).number_format = '₹#,##0.00'
ws_s.cell(row=r_avg, column=7).font = font_win; ws_s.cell(row=r_avg, column=7).fill = fill_win
ws_s.cell(row=r_avg, column=8, value=df_total_summary['Booked Return on Margin (%)'].mean()).number_format = '+0.00%'
ws_s.cell(row=r_avg, column=9, value=-abs(df_total_summary['Max Drawdown (₹)'].mean())).number_format = '-₹#,##0.00'
ws_s.cell(row=r_avg, column=9).font = font_loss; ws_s.cell(row=r_avg, column=9).fill = fill_loss
ws_s.cell(row=r_avg, column=10, value=df_total_summary['DD % on Initial Capital'].mean()).number_format = '-0.00%'
...
ws_s.cell(row=r_avg, column=11, value=df_total_summary['True DD % on Peak Portfolio'].mean()).number_format = '-0.00%'
...
# Chart
chart = LineChart()
chart.title = "Consolidated 12-Quarter Net Futures Realised P&L (After Total 0.5% Cost) Trend (₹)"
chart.style = 13
...
try:
    wb_s.save(OUTPUT_TOTAL_SUMMARY)
    print(f"  ✓ Saved Summary Workbook: {OUTPUT_TOTAL_SUMMARY.name}")
except Exception as e:
    alt_file_s = BASE_DIR / 'Nifty50_12_Quarters_Futures_0.5PCT_Total_Turnover_Summary_CLEAN.xlsx'
    wb_s.save(alt_file_s)
    print(f"  ✓ Saved Summary Workbook (Alt): {alt_file_s.name}")

print(f"\n🎉 PERFECT CLEAN EXCEL WORKBOOKS GENERATED IN {time.time() - t0:.1f}s!", flush=True)
