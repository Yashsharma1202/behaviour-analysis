...
print("==========================================================================================")
print("BUILDING ENHANCED QUALIFIED & MONITOR PORTFOLIO MASTER WITH CLEAR STOCK NAMES")
print("==========================================================================================")
...
# HELPER SIMULATION FUNCTION FOR SPECIFIC STOCKS GROUP
def run_portfolio_simulation(target_symbols, label_name):
    all_q_keys = set()
    ...
    capital_25l = 2500000.0
    capital_50l = 5000000.0
    ...
    for q_k in sorted_q_keys:
        ...
        active_stocks_in_q = [s for s in target_symbols if q_k in stock_quarterly_returns[s]["q_map"]]
        n_active = len(active_stocks_in_q)
        
        if n_active == 0:
            continue

        alloc_25l_per_trade = curr_bal_25l / float(n_active)
        alloc_50l_per_trade = curr_bal_50l / float(n_active)
        ...
        for sym in active_stocks_in_q:
            traded_stock_names.append(sym)
            ret_pct = stock_quarterly_returns[sym]["q_map"][q_k]
            ...
            trade_logs_all.append({
                "quarter": q_disp,
                "sym": sym,
                "sector": stock_quarterly_returns[sym]["sector"],
                "strategy": stock_quarterly_returns[sym]["strategy"],
                "window": stock_quarterly_returns[sym]["window"],
                "ret_pct": ret_pct,
                "pnl_25l": trade_pnl_25l,
                "pnl_50l": trade_pnl_50l
            })
        ...
        traded_symbols_str = ", ".join(traded_stock_names)

        portfolio_sim_25l.append({
            "quarter": q_disp,
            "active_trades": n_active,
            "traded_symbols": traded_symbols_str,
            "start_bal": start_25l,
            "q_pnl": q_pnl_25l,
    ...
    constituent_info = []
    for rank_i, s in enumerate(target_symbols, 1):
        d = stock_quarterly_returns[s]
        constituent_info.append({
            ...
            "window": d["window"],
            "win_rate": d["win_rate"],
            "win_ratio_str": f"{d['wins']} / {d['total_qtrs']} Qtrs",
            "margin": d["margin"],
            "net_pnl": d["net_pnl"]
        })

    return {
        "sim_25l": portfolio_sim_25l,
        "sim_50l": portfolio_sim_50l,
        "tot_pnl_25l": tot_pnl_25l, "tot_ret_25l": tot_ret_25l, "max_dd_25l": max_dd_25l,
        "tot_pnl_50l": tot_pnl_50l, "tot_ret_50l": tot_ret_50l, "max_dd_50l": max_dd_50l,
        "symbols": target_symbols,
        "constituent_info": constituent_info,
        "trade_logs": trade_logs_all
    }

qualified_symbols = sorted(
    [s for s, d in stock_quarterly_returns.items() if d.get("is_available") and d.get("trading_status") == 'QUALIFIED'],
    key=lambda x: (stock_quarterly_returns[x]['net_pnl'], stock_quarterly_returns[x]['win_rate']), reverse=True
)

monitor_symbols = sorted(
    [s for s, d in stock_quarterly_returns.items() if d.get("is_available") and d.get("trading_status") == 'AVOID BUT MONITOR IT'],
    key=lambda x: (stock_quarterly_returns[x]['net_pnl'], stock_quarterly_returns[x]['win_rate']), reverse=True
)

print(f"Total QUALIFIED Stocks Ranked: {len(qualified_symbols)}")
print(f"Total MONITOR Stocks Ranked: {len(monitor_symbols)}")

res_qual = run_portfolio_simulation(qualified_symbols, "QUALIFIED")
res_mon = run_portfolio_simulation(monitor_symbols, "MONITOR")

out_path_qual_mon = ROOT / "Nifty211_Qualified_and_Monitor_Portfolio_Master.xlsx"
out_path_qual_alt = ROOT / "Nifty211_Qualified_Portfolio_Master.xlsx"

wb = openpyxl.Workbook()

title_font = Font(name="Calibri", size=16, bold=True, color="FFFFFF")
title_fill = PatternFill("solid", fgColor="1F4E79")
card_val_font = Font(name="Calibri", size=13, bold=True, color="1F4E79")
...
# ==========================================
# HELPER TO BUILD PORTFOLIO ALLOCATOR SHEET
# ==========================================
def populate_portfolio_sheet(ws_p, res_data, sheet_title_name, sub_header_name):
    ws_p.title = sheet_title_name
    ws_p.views.sheetView[0].showGridLines = True

    # 1. Main Title Block
    ws_p.merge_cells("A1:K1")
    ws_p["A1"] = f"{sub_header_name.upper()} — PORTFOLIO CAPITAL ALLOCATOR SIMULATION"
    ...
    port_cards = [
        ("₹25 LAKHS PORTFOLIO NET P&L", f"₹{res_data['tot_pnl_25l']:,.2f}\n(+{res_data['tot_ret_25l']}%)", "A3:B4", "A3"),
        ("₹25L MAX DRAWDOWN %", f"-{res_data['max_dd_25l']}%", "C3:D4", "C3"),
        ("₹50 LAKHS PORTFOLIO NET P&L", f"₹{res_data['tot_pnl_50l']:,.2f}\n(+{res_data['tot_ret_50l']}%)", "E3:F4", "E3"),
        ("₹50L MAX DRAWDOWN %", f"-{res_data['max_dd_50l']}%", "G3:H4", "G3"),
        (f"TOTAL STOCKS TRADED", f"{len(res_data['symbols'])} Stocks\nEqual Weight Allocation", "I3:K4", "I3")
    ]
    ...
    # 3. TABLE A: CONSTITUENT STOCK NAMES LIST
    start_r_const = 6
    ws_p.cell(row=start_r_const, column=1, value=f"{sub_header_name.upper()} — PORTFOLIO CONSTITUENT STOCKS & PARAMETERS").font = hdr_font
    ws_p.cell(row=start_r_const, column=1).fill = hdr_fill
    ws_p.merge_cells(start_row=start_r_const, start_column=1, end_row=start_r_const, end_column=9)

    c_headers = [
        "Rank", "Stock Symbol / Name", "Industry Sector", "Strategy",
        "Optimal Position Window", "Win Rate %", "Win Ratio", "Margin Capital (₹)", "Standalone Net P&L (₹)"
    ]
    ws_p.row_dimensions[start_r_const+1].height = 26
    for c_i, h in enumerate(c_headers, 1):
        cell = ws_p.cell(row=start_r_const+1, column=c_i, value=h)
        cell.font = hdr_font; cell.fill = subhdr_fill
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

    cur_row = start_r_const + 2
    for c_item in res_data['constituent_info']:
        ws_p.cell(row=cur_row, column=1, value=c_item['rank']).border = border_thin
        ws_p.cell(row=cur_row, column=1).alignment = Alignment(horizontal="center")
        
        c_sym = ws_p.cell(row=cur_row, column=2, value=c_item['sym'])
        c_sym.border = border_thin; c_sym.font = Font(bold=True)
        c_sym.alignment = Alignment(horizontal="left")
        
        ws_p.cell(row=cur_row, column=3, value=c_item['sector']).border = border_thin
        ws_p.cell(row=cur_row, column=3).alignment = Alignment(horizontal="center")

        c_st = ws_p.cell(row=cur_row, column=4, value=c_item['strategy'])
        c_st.border = border_thin; c_st.alignment = Alignment(horizontal="center")
        if c_item['strategy'] == 'FUTURE LONG': c_st.fill = pos_fill; c_st.font = pos_font
        elif c_item['strategy'] == 'FUTURE SHORT': c_st.fill = neg_fill; c_st.font = neg_font

        ws_p.cell(row=cur_row, column=5, value=c_item['window']).border = border_thin
        ws_p.cell(row=cur_row, column=5).alignment = Alignment(horizontal="center")

        ws_p.cell(row=cur_row, column=6, value=f"{c_item['win_rate']}%").border = border_thin
        ws_p.cell(row=cur_row, column=6).alignment = Alignment(horizontal="right")

        ws_p.cell(row=cur_row, column=7, value=c_item['win_ratio_str']).border = border_thin
        ws_p.cell(row=cur_row, column=7).alignment = Alignment(horizontal="center")

        ws_p.cell(row=cur_row, column=8, value=f"₹{c_item['margin']:,.2f}").border = border_thin
        ws_p.cell(row=cur_row, column=8).alignment = Alignment(horizontal="right")

        c_p = ws_p.cell(row=cur_row, column=9, value=f"₹{c_item['net_pnl']:,.2f}")
        c_p.border = border_thin; c_p.alignment = Alignment(horizontal="right")
        if c_item['net_pnl'] >= 0: c_p.fill = pos_fill; c_p.font = pos_font
        else: c_p.fill = neg_fill; c_p.font = neg_font

        cur_row += 1

    # 4. TABLE B: QUARTERLY TIMELINE WITH TRADED STOCK SYMBOLS
    cur_row += 2
    timeline_start_r = cur_row
    ws_p.cell(row=timeline_start_r, column=1, value=f"{sub_header_name.upper()} — QUARTERLY EQUITY TIMELINE & TRADED STOCK NAMES").font = hdr_font
    ws_p.cell(row=timeline_start_r, column=1).fill = hdr_fill
    ws_p.merge_cells(start_row=timeline_start_r, start_column=1, end_row=timeline_start_r, end_column=10)

    p_headers = [
        "Quarter Results Timeline", "Active Stocks Count",
    ...
    ws_p.row_dimensions[timeline_start_r+1].height = 28
    for c_i, h in enumerate(p_headers, 1):
        cell = ws_p.cell(row=timeline_start_r+1, column=c_i, value=h)
        cell.font = hdr_font; cell.fill = subhdr_fill
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

    cur_row = timeline_start_r + 2
    for r25, r50 in zip(res_data['sim_25l'], res_data['sim_50l']):
        ws_p.cell(row=cur_row, column=1, value=r25['quarter']).border = border_thin
        ws_p.cell(row=cur_row, column=1).alignment = Alignment(horizontal="center")
        ws_p.cell(row=cur_row, column=1).font = Font(bold=True)
        
        ws_p.cell(row=cur_row, column=2, value=r25['active_trades']).border = border_thin
        ws_p.cell(row=cur_row, column=2).alignment = Alignment(horizontal="center")
        
        ws_p.cell(row=cur_row, column=3, value=f"₹{r25['start_bal']:,.2f}").border = border_thin
        ws_p.cell(row=cur_row, column=3).alignment = Alignment(horizontal="right")
        
        c_pnl25 = ws_p.cell(row=cur_row, column=4, value=f"₹{r25['q_pnl']:,.2f}")
        ...
        c_ret25 = ws_p.cell(row=cur_row, column=5, value=f"{r25['q_ret_pct']}%")
        ...
        ws_p.cell(row=cur_row, column=6, value=f"₹{r25['end_bal']:,.2f}").border = border_thin
        ws_p.cell(row=cur_row, column=6).alignment = Alignment(horizontal="right")
        ws_p.cell(row=cur_row, column=6).font = Font(bold=True)
        
        ws_p.cell(row=cur_row, column=7, value=f"₹{r50['start_bal']:,.2f}").border = border_thin
        ws_p.cell(row=cur_row, column=7).alignment = Alignment(horizontal="right")
        
        c_pnl50 = ws_p.cell(row=cur_row, column=8, value=f"₹{r50['q_pnl']:,.2f}")
        ...
        c_ret50 = ws_p.cell(row=cur_row, column=9, value=f"{r50['q_ret_pct']}%")
        ...
        ws_p.cell(row=cur_row, column=10, value=f"₹{r50['end_bal']:,.2f}").border = border_thin
        ws_p.cell(row=cur_row, column=10).alignment = Alignment(horizontal="right")
        ws_p.cell(row=cur_row, column=10).font = Font(bold=True)

        cur_row += 1

    # 5. TABLE C: GRANULAR INDIVIDUAL TRADE LOG (QUARTER BY QUARTER BREAKDOWN)
    cur_row += 2
    tradelog_start_r = cur_row
    ws_p.cell(row=tradelog_start_r, column=1, value=f"{sub_header_name.upper()} — GRANULAR INDIVIDUAL STOCK TRADE LOG PER QUARTER").font = hdr_font
    ws_p.cell(row=tradelog_start_r, column=1).fill = hdr_fill
    ws_p.merge_cells(start_row=tradelog_start_r, start_column=1, end_row=tradelog_start_r, end_column=8)

    tl_headers = [
        "Quarter Timeline", "Stock Symbol / Name", "Industry Sector", "Strategy",
        "Position Window", "Trade Return %", "₹25L Trade P&L (₹)", "₹50L Trade P&L (₹)"
    ]
    ws_p.row_dimensions[tradelog_start_r+1].height = 26
    for c_i, h in enumerate(tl_headers, 1):
        cell = ws_p.cell(row=tradelog_start_r+1, column=c_i, value=h)
        cell.font = hdr_font; cell.fill = subhdr_fill
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

    cur_row = tradelog_start_r + 2
    for t_log in res_data['trade_logs']:
        ws_p.cell(row=cur_row, column=1, value=t_log['quarter']).border = border_thin
        ws_p.cell(row=cur_row, column=1).alignment = Alignment(horizontal="center")

        c_sym = ws_p.cell(row=cur_row, column=2, value=t_log['sym'])
        c_sym.border = border_thin; c_sym.font = Font(bold=True); c_sym.alignment = Alignment(horizontal="left")

        ws_p.cell(row=cur_row, column=3, value=t_log['sector']).border = border_thin
        ws_p.cell(row=cur_row, column=3).alignment = Alignment(horizontal="center")

        c_st = ws_p.cell(row=cur_row, column=4, value=t_log['strategy'])
        c_st.border = border_thin; c_st.alignment = Alignment(horizontal="center")
        if t_log['strategy'] == 'FUTURE LONG': c_st.fill = pos_fill; c_st.font = pos_font
        elif t_log['strategy'] == 'FUTURE SHORT': c_st.fill = neg_fill; c_st.font = neg_font

        ws_p.cell(row=cur_row, column=5, value=t_log['window']).border = border_thin
        ws_p.cell(row=cur_row, column=5).alignment = Alignment(horizontal="center")

        c_r = ws_p.cell(row=cur_row, column=6, value=f"{t_log['ret_pct']:.2f}%")
        c_r.border = border_thin; c_r.alignment = Alignment(horizontal="right")
        if t_log['ret_pct'] >= 0: c_r.fill = pos_fill; c_r.font = pos_font
        else: c_r.fill = neg_fill; c_r.font = neg_font

        c_p25 = ws_p.cell(row=cur_row, column=7, value=f"₹{t_log['pnl_25l']:,.2f}")
        c_p25.border = border_thin; c_p25.alignment = Alignment(horizontal="right")
        if t_log['pnl_25l'] >= 0: c_p25.fill = pos_fill; c_p25.font = pos_font
        else: c_p25.fill = neg_fill; c_p25.font = neg_font

        c_p50 = ws_p.cell(row=cur_row, column=8, value=f"₹{t_log['pnl_50l']:,.2f}")
        c_p50.border = border_thin; c_p50.alignment = Alignment(horizontal="right")
        if t_log['pnl_50l'] >= 0: c_p50.fill = pos_fill; c_p50.font = pos_font
        else: c_p50.fill = neg_fill; c_p50.font = neg_font
    ...
    for col in ws_p.columns:
        ...
        if col_letter == 'C':
            ws_p.column_dimensions[col_letter].width = 45
        else:
            ws_p.column_dimensions[col_letter].width = 24

# SHEET 1: QUALIFIED STOCKS PORTFOLIO
ws_q = wb.active
populate_portfolio_sheet(ws_q, res_qual, "QUALIFIED STOCKS PORTFOLIO", "QUALIFIED STOCKS (>=12 Qtrs)")

# SHEET 2: MONITOR STOCKS PORTFOLIO
ws_m = wb.create_sheet(title="MONITOR STOCKS PORTFOLIO")
populate_portfolio_sheet(ws_m, res_mon, "MONITOR STOCKS PORTFOLIO", "MONITOR STOCKS (<12 Qtrs)")

# SHEETS 3-7: ALL QUARTERS COMBINED + Q1, Q2, Q3, Q4
quarter_names = {
...
for q_t in ["ALL", "Q1", "Q2", "Q3", "Q4"]:
    ...
    headers = [
        "Rank", "Stock Symbol / Name", "Industry Sector",
    ...
    for r_idx, r in enumerate(df_sorted.to_dict('records'), start=rank_start_row+2):
        ...
        c_sym = ws.cell(row=r_idx, column=2, value=r['sym'])
        c_sym.border = border_thin; c_sym.font = Font(bold=True)
        c_sym.alignment = Alignment(horizontal="left")
...
saved_paths = []
target_list = [
    out_path_qual_mon, out_path_qual_alt
]
...
print("==========================================================================================")
print(f"ENHANCED PORTFOLIO MASTER WITH CLEAR STOCK NAMES CREATED SUCCESSFULLY!")
print("==========================================================================================")
