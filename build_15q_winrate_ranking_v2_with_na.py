...
print("==========================================================================================")
print("BUILDING 15-QUARTER WIN RATE RANKING MASTER WORKBOOK WITH EXPLICIT 'N/A' HANDLING")
print("==========================================================================================")
...
for idx, sym in enumerate(SYMBOLS, 1):
    ...
    # Evaluate performance SPECIFICALLY ACROSS AVAILABLE QUARTERS IN THE 15-QUARTER WINDOW
    wins_15q = 0
    losses_15q = 0
    available_trades_15q = 0
    missing_trades_15q = 0
    stock_15q_pnl = 0.0
    rets_15q = []

    for q_idx, (q_name, default_d_str) in enumerate(quarters_list_15):
        ...
        if is_avail:
            ...
            available_trades_15q += 1
            rets_15q.append(act_r)
            stock_15q_pnl += net_pnl
            if net_pnl > 0:
                wins_15q += 1
            else:
                losses_15q += 1
        else:
            missing_trades_15q += 1

    win_rate_15q = round((wins_15q / available_trades_15q) * 100, 2) if available_trades_15q > 0 else 0.0
    avg_ret_15q = round(np.mean(rets_15q), 2) if rets_15q else 0.0
    
    trade_ratio_str = f"{wins_15q}/{available_trades_15q}" if available_trades_15q > 0 else "N/A"
    avail_status_str = f"{available_trades_15q}/15 Quarters Available" if available_trades_15q == 15 else f"{available_trades_15q}/15 Available ({missing_trades_15q} N/A)"
    ...
    ranking_data.append({
        ...
        "avail_status_str": avail_status_str,
        "window": window_str,
        "win_rate_15q": win_rate_15q,
        "available_trades_15q": available_trades_15q,
        "missing_trades_15q": missing_trades_15q,
        "wins_15q": wins_15q,
        "losses_15q": losses_15q,
        "trade_ratio_str": trade_ratio_str,
        "avg_ret_15q": avg_ret_15q if available_trades_15q > 0 else "N/A",
        "margin": margin_for_stock,
        "pnl_15q": round(stock_15q_pnl, 2),
        "rom_15q": round((stock_15q_pnl / margin_for_stock) * 100, 2) if margin_for_stock > 0 else 0.0
    })

df_all = pd.DataFrame(ranking_data)

# Sort strictly by 15-Quarter Win Rate % descending, then Available Trades descending, then Net P&L descending
df_all = df_all.sort_values(['win_rate_15q', 'available_trades_15q', 'pnl_15q'], ascending=[False, False, False]).reset_index(drop=True)
...
out_path = ROOT / "Nifty211_All_Stocks_15_Quarters_WinRate_Ranking_Master_v2.xlsx"
...
def write_ranking_sheet(ws, sheet_title, df_sheet):
    ...
    ws["A1"] = f"NIFTY 211 STOCKS - {sheet_title.upper()} RANKING BY 15-QUARTER WIN RATE % (WITH EXPLICIT N/A)"
    ...
    top_wr = df_sheet.iloc[0]['win_rate_15q'] if not df_sheet.empty else 0.0
    avg_wr = round(df_sheet['win_rate_15q'].mean(), 2) if not df_sheet.empty else 0.0
    tot_pnl = round(df_sheet['pnl_15q'].sum(), 2) if not df_sheet.empty else 0.0
    
    cards = [
        ("TOTAL STOCKS RANKED", f"{len(df_sheet)} Stocks", "A3:B4", "A3"),
        ("TOP 15-QTR WIN RATE STOCK", f"{top_stock} ({top_wr}%)", "C3:D4", "C3"),
        ("AVERAGE 15-QTR WIN RATE %", f"{avg_wr}%", "E3:F4", "E3"),
        ("15-QTR NET REALISED P&L", f"₹{tot_pnl:,.2f}", "G3:H4", "G3")
    ]
    ...
    ws.cell(row=6, column=1, value="15-QUARTER WIN RATE BRACKET DISTRIBUTION SUMMARY").font = hdr_font
    ...
    b_80 = df_sheet[df_sheet['win_rate_15q'] >= 80]
    b_70 = df_sheet[(df_sheet['win_rate_15q'] >= 70) & (df_sheet['win_rate_15q'] < 80)]
    b_60 = df_sheet[(df_sheet['win_rate_15q'] >= 60) & (df_sheet['win_rate_15q'] < 70)]
    b_50 = df_sheet[(df_sheet['win_rate_15q'] >= 50) & (df_sheet['win_rate_15q'] < 60)]
    b_below = df_sheet[df_sheet['win_rate_15q'] < 50]
    ...
    for r_idx, (b_lbl, b_df) in enumerate(brackets, start=8):
        ...
        pnl_b = round(b_df['pnl_15q'].sum(), 2)
    ...
    # Main Ranking Table
    rank_start_row = 15
    ws.cell(row=rank_start_row, column=1, value=f"ALL {len(df_sheet)} STOCKS RANKED BY 15-QUARTER WIN RATE %").font = hdr_font
    ws.cell(row=rank_start_row, column=1).fill = hdr_fill
    ws.merge_cells(start_row=rank_start_row, start_column=1, end_row=rank_start_row, end_column=13)

    headers = [
        "Rank", "Stock Symbol", "Optimal Strategy", "Action Status", "Historical Quarters",
        "Quarter Availability Status", "Position Window", "15-Quarter Win Rate %", "15-Quarter Win Ratio",
        "15-Quarter Losses", "15-Quarter Avg Return %", "Margin Capital (₹)", "15-Quarter Net P&L (₹)"
    ]
    ...
    for r_idx, r in enumerate(df_sheet.to_dict('records'), start=rank_start_row+2):
        ...
        ws.cell(row=r_idx, column=6, value=r['avail_status_str']).border = border_thin
        ...
        c_wr = ws.cell(row=r_idx, column=8, value=f"{r['win_rate_15q']}%")
        c_wr.border = border_thin; c_wr.alignment = Alignment(horizontal="right")
        c_wr.font = Font(bold=True)
        if r['win_rate_15q'] >= 65.0: c_wr.fill = pos_fill; c_wr.font = pos_font
        elif r['win_rate_15q'] < 50.0: c_wr.fill = neg_fill; c_wr.font = neg_font
        
        ws.cell(row=r_idx, column=9, value=r['trade_ratio_str']).border = border_thin
        ws.cell(row=r_idx, column=9).alignment = Alignment(horizontal="center")
        
        ws.cell(row=r_idx, column=10, value=r['losses_15q']).border = border_thin
        ws.cell(row=r_idx, column=10).alignment = Alignment(horizontal="center")
        
        val_avg = f"{r['avg_ret_15q']}%" if isinstance(r['avg_ret_15q'], (int, float)) else r['avg_ret_15q']
        ws.cell(row=r_idx, column=11, value=val_avg).border = border_thin
        ws.cell(row=r_idx, column=11).alignment = Alignment(horizontal="right")
        
        ws.cell(row=r_idx, column=12, value=f"₹{r['margin']:,.2f}").border = border_thin
        ws.cell(row=r_idx, column=12).alignment = Alignment(horizontal="right")
        
        c_p = ws.cell(row=r_idx, column=13, value=f"₹{r['pnl_15q']:,.2f}")
        c_p.border = border_thin; c_p.alignment = Alignment(horizontal="right")
        if r['pnl_15q'] >= 0: c_p.fill = pos_fill; c_p.font = pos_font
        else: c_p.fill = neg_fill; c_p.font = neg_font
...
# Write Sheet 1: All 211 Stocks 15Q Ranking
ws1 = wb.active
ws1.title = "All 211 Stocks 15Q Ranking"
...
wb.save(out_path)
print(f"  • Successfully generated: Nifty211_All_Stocks_15_Quarters_WinRate_Ranking_Master_v2.xlsx")
print("==========================================================================================")
print("15-QUARTER WIN RATE RANKING MASTER WORKBOOK WITH EXPLICIT N/A CREATED SUCCESSFULLY!")
print("==========================================================================================")
